package rohstoff.businesslogic.bewegung2.lager.vertragsmengen;
...
public class B2_LAG_KontraktmengenErmittlung {

	private HashMap<String, B2_LAG_KontraktmengenDaten> m_Lagerdaten = null;
	...
	public B2_LAG_KontraktmengenErmittlung() {
	...
	public void ReadVertragsLagerDaten(String sIDLager, String sIDHauptsorte, String sIDSorte ) throws myException{
		...
		String usSqlEK_v2 = 
				new SEL("SUM(A1.MENGE_NETTO)")
				.FROM(_TAB.bg_atom,"A1")
				.INNERJOIN(_TAB.bg_vektor, BG_VEKTOR.id_bg_vektor, BG_ATOM.id_bg_atom.fn("A1"))
				.INNERJOIN(_TAB.bg_station.fullTableName()+" S1", BG_STATION.id_bg_station.fn("S1"), //zwischenlager
						BG_ATOM.id_bg_station_quelle.fn("A1"))
//						"(case "
//						+ "when a1.pos_in_mask = 'A1' then  A1.ID_BG_STATION_QUELLE "
//						+ "when a1.pos_in_mask = 'A2' then  A1.ID_BG_STATION_ZIEL"
//						+ " end)")
				.INNERJOIN(_TAB.bg_station.fullTableName()+" S2", BG_STATION.id_bg_station.fn("S2"), //lager
						BG_ATOM.id_bg_station_ziel.fn("A1"))
//						"(case "
//						+ "when a1.pos_in_mask = 'A1' then  A1.ID_BG_STATION_ZIEL "
//						+ "when a1.pos_in_mask = 'A2' then  A1.ID_BG_STATION_QUELLE"
//						+ " end)")
				.LEFTOUTER(_TAB.vpos_kon.fullTableName() + " K1", "K1.ID_VPOS_KON", "a1.ID_VPOS_KON")
				.INNERJOIN(_TAB.vpos_kon_trakt.fullTableName() + " TRAKT1", "K1.ID_VPOS_KON", "TRAKT1.ID_VPOS_KON")
				.INNERJOIN(_TAB.vpos_kon_lager.fullTableName() + " L1", "L1.ID_VPOS_KON", "K1.ID_VPOS_KON")
				.s()
				+" WHERE "
				+" A1.pos_in_mask='A2'"
				+" AND A1.ID_BG_DEL_INFO IS NULL"  
				+" AND A1.ID_BG_STORNO_INFO IS NULL"  
				+" AND NVL(TRAKT1.ABGESCHLOSSEN,'N') = 'N'" 
				+" AND S2.ID_ADRESSE = L1.ID_ADRESSE_LAGER " 
			    +" AND S2.ID_ADRESSE = L.ID_ADRESSE_LAGER  "
				+" AND A1.ID_ARTIKEL = K1.ID_ARTIKEL" 
				+" AND A1.ID_ARTIKEL = K.ID_ARTIKEL" 
				+" GROUP BY 					" 
			    +"  S2.ID_ADRESSE,			" 
			    +"   A1.ID_ARTIKEL       	" 
				;
...
//						+ " end)")
		...
		String sSqlEK_v2 = 
				"SELECT 'EK', L.ID_ADRESSE_LAGER, K.ID_ARTIKEL, SUM(L.LAGERMENGE), " + 
						" SUM (L.LAGERMENGE) - " + 
					    " NVL( (" +
					    usSqlEK_v2 +"),0) as RESTMENGE_AUF_KONTRAKT, " +
					    " 0 "  +
					    " FROM  JT_VPOS_KON K " +
					    " INNER JOIN " +
					    "JT_VKOPF_KON KOPF  ON ( K.ID_VKOPF_KON = KOPF.ID_VKOPF_KON )  " +
					    " INNER JOIN " +
					    "JT_VPOS_KON_TRAKT TRAKT  ON ( K.ID_VPOS_KON = TRAKT.ID_VPOS_KON )  " +
					    " INNER JOIN " +
					    "JT_VPOS_KON_LAGER L  ON  (  K.ID_VPOS_KON = L.ID_VPOS_KON )  " +
		...
		String sSqlVK_v2 = 
				"SELECT 'VK', L.ID_ADRESSE_LAGER, K.ID_ARTIKEL, SUM(L.LAGERMENGE), " + 
						" SUM (L.LAGERMENGE) - " + 
					    " NVL( (" +
					    usSqlVK_v2 +"),0) as RESTMENGE_AUF_KONTRAKT, " +
					    " 0 "  +
					    " FROM  JT_VPOS_KON K " +
					    " INNER JOIN " +
					    "JT_VKOPF_KON KOPF  ON ( K.ID_VKOPF_KON = KOPF.ID_VKOPF_KON )  " +
...
//			" GROUP BY  L.ID_ADRESSE_LAGER, K.ID_ARTIKEL ";				
		...
		String sSql = sSqlEK_v2  + " UNION " + sSqlVK_v2;
	...
	public void ClearData(){
		...
		m_Lagerdaten = new HashMap<String, B2_LAG_KontraktmengenDaten>();
	}
	 ...
	 * @return
	 */
	public B2_LAG_KontraktmengenDaten getData(String idLager, String idArtikel) {

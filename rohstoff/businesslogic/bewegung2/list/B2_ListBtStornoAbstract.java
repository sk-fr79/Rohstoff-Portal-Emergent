...
public abstract class B2_ListBtStornoAbstract extends E2_Button_DbSimple {

	private enum statusStorno {
		storniert, unstorniert
	}

	private RB_TransportHashMap m_tpHashMap;

	public abstract VEK<String> getIdVectorsToStorno();

	public B2_ListBtStornoAbstract(RB_TransportHashMap p_tpHashMap) {
		super();
		this.m_tpHashMap = p_tpHashMap;

		this._setShapeStornoButton();

		this.setToolTipText(new MyE2_String("Fuhre in dieser Zeile loeschen").CTrans());

		this.add_GlobalValidator(ENUM_VALIDATION.BG_TRANSPORT_STORNO.getValidator());
		...
		this._addValidator(()->{
			...
			for (String id: getIdVectorsToStorno()) {
				...
				if (S.isFull( r.getUfs(BG_VEKTOR.id_bg_del_info))) {
					mv._add(mv._addAlarm(S.ms("Der Stornozustand kann nur bei ungelöschten Warenbewegungen geändert werden! Bitte zuerst das Löschen rückgängig machen !")));
				}
			}
			return mv;
		});
		...
		this._addValidator(()->{ 
			...
			for (String id: getIdVectorsToStorno()) {
				Rec21 r = new Rec21(_TAB.bg_vektor)._fill_id(id);
				mv._add(new Rec21_bgVector(r).checkStornable());
			}
			return mv;
		});
		...
		this._aaa(() -> {
			...
			if (this.getIdVectorsToStorno().size() == 0) {
				bibMSG.MV()._addAlarm(S.ms("Sie müssen Zeilen zum Stornieren / Wiederherstellen markieren !"));
			} else {

				statusStorno status = checkIfAllSameStatus();
				if (status == null) {
					bibMSG.MV()._addAlarm(S.ms("Sie haben sowohl stornierte als auch unstornierte ausgewählt ! "));

				} else if (status == statusStorno.unstorniert) {
					new Popup4EditStornoInfos();
				} else {
					new E2_MessageBox()._setYesText("Wiederherstellen")._setNoText("Abbruch")
							._setTitleOfPopup(S.ms("Stornierte Fuhre(n) wiederherstellen?"))
							._addActionInFrontYesBt(() -> {
								
								
								MyE2_MessageVector mv = new MyE2_MessageVector();
								
								for (String s : this.getIdVectorsToStorno()) {
								...
								this.getTpHashMap().getNavigationList().Mark_ID_IF_IN_Page(this.getIdVectorsToStorno());
							})._addInfo("Bei " + this.getIdVectorsToStorno().size() + " Fuhren Stornomarkierung")
	...
	private statusStorno checkIfAllSameStatus() throws myException {
		VEK<statusStorno> v = new VEK<>();

		VEK<String> ids = this.getIdVectorsToStorno();

		for (String id : ids) {
			Rec21 rv = new Rec21(_TAB.bg_vektor)._fill_id(id);
			if (S.isEmpty(rv.getUfs(BG_VEKTOR.id_bg_storno_info))) {
				v._addIfNotIN(statusStorno.unstorniert);
			} else {
				v._addIfNotIN(statusStorno.storniert);
			}
	...
	private class Popup4EditStornoInfos extends E2_BasicModuleContainer {

		RB_TextArea t = new RB_TextArea()._sizeWH(200, 80);
		E2_Button bSave = new E2_Button()._setShapeStornoButton()._t(S.ms("Stornieren"))._s_BorderText();
		E2_Button bCancel = new E2_Button()._setShapeCancelAndClose()._t(S.ms("Abbruch"))._s_BorderText();

		public Popup4EditStornoInfos() throws myException {
			super();

			E2_Grid g = new E2_Grid();

			g._setSize(200, 200)
					._a(new RB_lab(S.ms("Bitte geben Sie Stornoinformationen ein:")),
							new RB_gld()._span(2)._ins(2, 5, 2, 10))

					._a(new RB_lab(S.ms("Grund für den Stornovorgang:")), new RB_gld()._ins(2, 2, 2, 2))
			...
			bSave._aaa(new ActionExecuteStorno());
			bSave._aaa(() -> {
				CLOSE_AND_DESTROY_POPUPWINDOW(true);
			});
			...
			this.CREATE_AND_SHOW_POPUPWINDOW(new Extent(450), new Extent(250),
					S.ms("Bitte bestätigen Sie das Stornieren der Warenbewegung(en) !"));
		}

		
		private class ActionExecuteStorno extends XX_ActionAgent {
			@Override
			public void executeAgentCode(ExecINFO oExecInfo) throws myException {
				...
				if (S.isEmpty(t.getText())) {
					...
					VEK<String> ids = getIdVectorsToStorno();
					VEK<Rec20> sqls = new VEK<>();

					for (String s : ids) {

						Rec21 recStorno = new Rec21(_TAB.bg_storno_info);
						recStorno._setNewVal(BG_STORNO_INFO.id_user, Long.parseLong(bibALL.get_ID_USER()), bibMSG.MV())
								._setNewVal(BG_STORNO_INFO.storno_datum, new Date(), bibMSG.MV())
								._setNewVal(BG_STORNO_INFO.storno_grund, t.getText(), bibMSG.MV());
						...
						if (allRecordToVektor != null) {
							recV._put_raw_value(BG_VEKTOR.id_bg_storno_info, BG_STORNO_INFO._tab().seq_currval(), false);
							allRecordToVektor.get(RecA1.key)._put_raw_value(BG_ATOM.id_bg_storno_info, BG_STORNO_INFO._tab().seq_currval(), false);
							allRecordToVektor.get(RecA2.key)._put_raw_value(BG_ATOM.id_bg_storno_info, BG_STORNO_INFO._tab().seq_currval(), false);
							allRecordToVektor.get(RecS1.key)._put_raw_value(BG_STATION.id_bg_storno_info, BG_STORNO_INFO._tab().seq_currval(), false);
							allRecordToVektor.get(RecS2.key)._put_raw_value(BG_STATION.id_bg_storno_info, BG_STORNO_INFO._tab().seq_currval(), false);
							allRecordToVektor.get(RecS3.key)._put_raw_value(BG_STATION.id_bg_storno_info, BG_STORNO_INFO._tab().seq_currval(), false);
	...
	public void set_cActual_Formated_DBContent_To_Mask(String cText, String cMASK_STATUS, SQLResultMAP oResultMAP) throws myException {
		try {
			...
			if (S.isEmpty(rec.getUfs(BG_VEKTOR.id_bg_storno_info))) {
				this._setShapeStornoButton();
				this.setToolTipText(new MyE2_String("Fuhre in dieser Zeile stornieren").CTrans());
			} else {
				this._setShapeUnStornoButton();
				SimpleDateFormat sdf = new SimpleDateFormat("dd.MM.yyyy HH:mm:ss");
				Rec21 storno =((B2_ListComponentMap)this.EXT().get_oComponentMAP()).getRec21().get_up_Rec21(BG_STORNO_INFO.id_bg_storno_info);
				Rec21 user   = storno.get_up_Rec21(USER.id_user);
				String tooltips = "Status: Storniert\n"+"von:  "+user.get_ufs_kette(" ", USER.vorname,USER.name1)
									+"\nam:  "+sdf.format((Date)storno.getRawVal(BG_STORNO_INFO.storno_datum))
									+"\nGrund:\n"+storno.getFs(BG_STORNO_INFO.storno_grund)
									+"\n\n"+S.ms("Storno kann mit diesem Button aufgehoben werden").CTrans()
									;

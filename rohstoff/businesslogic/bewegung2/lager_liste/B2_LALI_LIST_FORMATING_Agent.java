...
public class B2_LALI_LIST_FORMATING_Agent extends XX_ComponentMAP_SubqueryAGENT 
{
	Vector<String> m_EigeneLager = null;
	
	public B2_LALI_LIST_FORMATING_Agent() {
		super();
	...
	public void fillComponents(E2_ComponentMAP oMAP, SQLResultMAP oUsedResultMAP) throws myException 
	{
		...
		// Kundeninfo
		MyE2_DB_TextArea o_KundeInfo = (MyE2_DB_TextArea) oMAP.get__Comp_From_RealComponents("NAME_GEGEN");
		

		String sLieferadresse = oMAP.get_oInternalSQLResultMAP().get_UnFormatedValue("NAME_GEGEN");
		String sKundenadresse = oMAP.get_oInternalSQLResultMAP().get_UnFormatedValue("NAME_BASIS_GEGEN");
		if (!bibALL.isEmpty(sKundenadresse) && !sKundenadresse.equals(sLieferadresse) ){
			o_KundeInfo.setText(sLieferadresse + "\n(" + sKundenadresse +  ")");
		}
		
		String idLiefer = oUsedResultMAP.get_UnFormatedValue("ADR_KUNDE_ID_ADRESSE");
		// Läger kennzeichnen
...
//		lbl.setText(sEWFW + " - " + sTemp);
		...
		//
		MyE2_Row_EveryTimeEnabled rowJumperFuhre = (MyE2_Row_EveryTimeEnabled)oMAP.get__Comp_From_RealComponents(B2_LALI_CONST.HASH_BUTTON_SHOW_FUHRE);
		rowJumperFuhre.removeAll();
		String idFuhre = oMAP.get_oInternalSQLResultMAP().get_UnFormatedValue("K_ID_VPOS_TPA_FUHRE");

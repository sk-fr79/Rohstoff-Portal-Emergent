...
@Deprecated
public class BG_Lager_Bewegung_LIST_Summay extends E2_ListSelectorContainer
{
	...
	private BG_Lager_Bewegung_LIST_Selector m_oSelector = null;
...
//	E2_ChartDisplay chart_display = null;
	...
	public BG_Lager_Bewegung_LIST_Summay(E2_NavigationList oNavigationList, BG_Lager_Bewegung_LIST_Selector oSelector) throws myException
	{
...
//		Vector<String> vIDLager = null;
		...
		BG_LagerSaldoErmittlung oSaldo = new BG_LagerSaldoErmittlung();
		boolean bShowLagerEinzel = cbShowLagerEinzel.isSelected();
		if (!bShowLagerEinzel) {
	...
	private void readLagerdatenFromNavigationListEntriesE(E2_NavigationList oNaviList) throws myException {
		...
		cSQL_For_IDs_Navilist = cSQL_For_IDs_Navilist.substring(0, cSQL_For_IDs_Navilist.toUpperCase().indexOf("ORDER BY", 0));
		iSegmente = 1;
		...
		for (int i = 0; i< iSegmente; i++){
			...
			String sSql = "";
			String sSqlSelector = this.oNaviList.get_oComponentMAP__REF().get_oSQLFieldMAP() .get_CompleteSQLQueryFor_ID_VECTOR("",true);
			sSqlSelector = sSqlSelector.substring(sSqlSelector.toUpperCase().indexOf("FROM"));
			
				
			
			sSql = "SELECT " 
					+ " NVL(JT_BG_LADUNG.MENGE,0)  * JT_BG_LADUNG.MENGENVORZEICHEN , "
					+ " nvl(JT_BG_LADUNG.MENGE_NETTO,nvl(JT_BG_LADUNG.MENGE,0)) * JT_BG_LADUNG.MENGENVORZEICHEN , " 
					+ " A.E_PREIS , " 
					+ " JT_BG_LADUNG.E_PREIS_RESULT_NETTO_MGE, "
					+ " JT_BG_LADUNG.MENGENVORZEICHEN, " 
					+ " ART.ID_EINHEIT, " 
					+ " ART.MENGENDIVISOR ,  " 
					+ " EG.EINHEITKURZ, " 
					+ " EP.EINHEITKURZ, " 
					+ " CASE WHEN (JT_BG_LADUNG.ID_ADRESSE_BESITZER = " + sIDAdresseMandant + " OR L2.ID_ADRESSE_BESITZER =" + sIDAdresseMandant + ") "
					+ "															  AND JT_BG_LADUNG.ID_ADRESSE_BESITZER != L2.ID_ADRESSE_BESITZER  "
					+ "     THEN UPPER('N')"
					+ "     ELSE UPPER('Y')"
					+ "  END AS OHNE_ABRECHNUNG,"
					+ " JT_BG_LADUNG.E_PREIS_RESULT_BRUTTO_MGE ,"
					+ " 0 as status_eigen_fremd_abrechenbar ,"
					+ " CASE WHEN JT_BG_LADUNG.ID_ADRESSE_BESITZER = " + bibALL.get_ID_ADRESS_MANDANT() + " "
					+ " 					OR L2.ID_ADRESSE_BESITZER = " + bibALL.get_ID_ADRESS_MANDANT() 
					+ " 				THEN 'Y' "
					+ "                 ELSE 'N' " 
					+ " END  as eigenware "
					+ " "
...
//			Vector<String> vLager			= m_oSelector.getSelectedLager();
	...
	private class cFieldList {
		...
		public cFieldList() {
			m_fieldList = new ArrayList<BG_Lager_Bewegung_LIST_Summay.cField>();
		}

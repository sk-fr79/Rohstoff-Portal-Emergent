...
import rohstoff.businesslogic.bewegung.convert_from_fuhre.EN_VEKTOR_TYP;

@Deprecated
public class BG_Lager_Bewegung_LIST_SelektorWelcheTypenEinblenden extends E2_ListSelektorMultiselektionStatusFeld_STD
{
	/**
...
//		MK,
	...
	private Vector<ENUM_SELEKTOR_BEWEGUNGSTYPEN> vCheckboxen = new Vector<BG_Lager_Bewegung_LIST_SelektorWelcheTypenEinblenden.ENUM_SELEKTOR_BEWEGUNGSTYPEN>(); 
	 ...
	 * @throws myException
	 */
	public BG_Lager_Bewegung_LIST_SelektorWelcheTypenEinblenden(E2_NavigationList oNavigationList) throws myException {
	 ...
	 * @throws myException
	 */
	public BG_Lager_Bewegung_LIST_SelektorWelcheTypenEinblenden(E2_NavigationList oNavigationList,boolean bZeigeBeschriftungAn) throws myException
	{
		super(1,bZeigeBeschriftungAn,new MyE2_String("Zeige: "),new Extent(40));
		
//		String sLagerDef = oNavigationList.get_oComponentMAP__REF().get_hmRealDBComponents().get("LA_ID_ADRESSE_LIEFER").EXT_DB().get_oSQLField().get_cFieldAusdruck();
		...
		String sLagerDef =  
			...
			" UNION " +
			"    SELECT " + bibALL.get_ID_ADRESS_MANDANT() +  " FROM  DUAL"  + 
			" ) " ;
...
//		String s_KH = " ( S2.ID_ADRESSE = " + bibSES.get_ID_ADRESSE_LAGER_KORREKTUR_HAND() + " ) ";
		//String s_UH = " ( S2.ID_ADRESSE = " + bibSES.get_ID_ADRESSE_LAGER_UMBUCHUNG_HAND() + " ) ";
		
		String s_KH = " ( V.EN_VEKTOR_TYP = '" + EN_VEKTOR_TYP.KORR.name() + "' ) ";
		String s_UH = " ( V.EN_VEKTOR_TYP = '" + EN_VEKTOR_TYP.UMB.name() +  "' ) ";
				
		// LL nur noch von eigenem Lager zu eigenem Lager
		String s_LL = "( S2.ID_ADRESSE IN  "+ sLagerDef + " AND S1.ID_ADRESSE IN  " +  sLagerDef +") ";
		// Kunden und MIXED-Fuhren gehen in einem auf, da Mixed eine spezialität von Standardfuhren sind...
		String s_KUNDEN = "( S1.ID_ADRESSE IN (SELECT ID_ADRESSE FROM " +bibE2.cTO() + ".JT_ADRESSE WHERE nvl(SONDERLAGER,'STRECKE') = 'STRECKE' )  AND S2.ID_ADRESSE NOT IN (" + sLagerDef + ")) ";

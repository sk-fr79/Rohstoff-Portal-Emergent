...
public class BG_Util_MultiSelectField_Sorte extends E2_ListSelectorMultiDropDown
{
	
	int 	m_iWidthDropDown = 100;
	String 	m_sLabelText = null;
	
	public BG_Util_MultiSelectField_Sorte( String sLabelText, int WidthDropdown, String sWhereClause) throws myException
	{
		super(new BG_Util_SelectField_Sorte_With_Param() ,sWhereClause); 
		this.fill_Grid4AnzeigeStatusSingle();
		this.get_oSelFieldBasis().setWidth(new Extent(WidthDropdown));
	...
	/**
	 * Aktualisiert die Auswahl im Dropdown
	 * @param sParamValue
	 * @throws myException
	 */
	public void Refresh(String sParamName,String sParamValue) throws myException{
		set_MemStringStatusSelektor("");
		((MyE2_SelectFieldWithParameters)get_oSelFieldBasis()).SetParameter(sParamName, sParamValue);
		get_oSelFieldBasis().RefreshComboboxFromSQL();
	}
	...
	public void fill_Grid4AnzeigeStatusMulti()
	{
		...
		if (bibALL.isEmpty(m_sLabelText) ){
			iSpalten = new int[2];
			iSpalten[0] = m_iWidthDropDown ;
			iSpalten[1] = 30;
		} else {
			iSpalten = new int[3];
			iSpalten[0] = 80;
			iSpalten[1] = m_iWidthDropDown ;
			iSpalten[2] = 30;
		}
		...
		this.get_grid4Anzeige().add(this.get_oButtonOpenMultiSelectPopup(),E2_INSETS.I_2_0_0_0);

...
@Deprecated
public class BG_SALDO_LIST_Summay extends E2_ListSelectorContainer
{
	...
	private  TreeMap<String, cFieldRow>  m_oRows = new TreeMap<String, BG_SALDO_LIST_Summay.cFieldRow>();
	...
	public BG_SALDO_LIST_Summay(E2_NavigationList oNavigationList, BG_LagerSaldoErmittlung oSaldoErmittlung) throws myException
	{
...
//		String sQueryForIDs = 	oNaviList.get_oComponentMAP__REF().get_oSQLFieldMAP().get_CompleteSQLQueryFor_ID_VECTOR("",true);
		...
		for (int i = 0; i< iSegmente; i++){
			...
			sSql = "SELECT s.ID_ADRESSE, a.ID_ARTIKEL, e.EINHEITKURZ "
				+ " FROM " + bibE2.cTO() + ".JT_BG_LADUNG a "
				+ " INNER JOIN " + bibE2.cTO() + ".JT_BG_STATION s on a.ID_BG_STATION = s.ID_BG_STATION "
			...
			for (int j = 0; j < cLagerDaten.length; j++) {
				...
				BG_SaldoDaten oDaten = m_SaldoErmittlung.getData(sID_Adresse, sID_Sorte);
	...
	private class cFieldList {
		...
		public cFieldList() {
			m_fieldList = new ArrayList<BG_SALDO_LIST_Summay.cField>();
		}

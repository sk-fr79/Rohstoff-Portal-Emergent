...
public class BG_LagerSaldoErmittlung {

	private HashMap<String, BG_SaldoDaten> 			m_Lagerdaten = null;
	private HashMap<String, BG_SaldoNulldurchgangDaten> m_Inventurdaten = null;
	
	private BG_Saldo_UnitSummation  					m_UnitSummation = null;
	...
	public BG_LagerSaldoErmittlung() {
...
//		if (enumLagertyp.equals(ENUM_Lagertyp.EIGENWAREN))
	...
	public void readSaldoDaten(
		...
		// LAGERTYP: Eigenwarenlager / FW-Lager / STRECKE
		if (bEigenwarenlager){
			sSqlWhereLagerTyp += " OR ( LA_ID_ADRESSE_FREMD IS NULL and ADR_SONDERLAGER IS NULL ) ";
		}
		if (bFremdwarenlager){
			sSqlWhereLagerTyp += " OR ( LA_ID_ADRESSE_FREMD IS NOT NULL and ADR_SONDERLAGER IS NULL) ";
		}
		if (bIncludeStrecke){
			sSqlWhereLagerTyp +=  " OR ADR_SONDERLAGER ='STRECKE' ";
		}
		sSqlWhereLagerTyp = " AND (  1 = 2  " + sSqlWhereLagerTyp + ")";
		...
		if (sIDSorte != null && sIDSorte.trim().length() > 0) {
			sSqlWhereArtikel 			= " AND L1.ID_ARTIKEL =  " + sIDSorte;
			
		} else if (vIDSorte != null && vIDSorte.size() > 0){
			if (vIDSorte.size()==1){
				sSqlWhereArtikel 			= " AND L1.ID_ARTIKEL =  " + vIDSorte.get(0);
			
			} else {
				sSqlWhereArtikel 			= " AND L1.ID_ARTIKEL IN (" + bibALL.Concatenate(vIDSorte, ",", "-1" )  + ") " ;
			}
		...
		if (sDatum != null && sDatum.trim().length() > 0) {
			...
			if (bTagesbeginn){
				// Saldo Morgens
				sSqlWhereDatum = " AND L1.LEISTUNGSDATUM < " +  sDT + " ";
			} else {
				// Saldo Abends
				sSqlWhereDatum = " AND L1.LEISTUNGSDATUM < (" +  sDT + " + 1 ) ";
			}
		}
		...
		String 	sSqlMenge = " CASE WHEN L1.MENGENVORZEICHEN = -1 " +
					"	THEN (NVL(L1.MENGE,0) ) * L1.MENGENVORZEICHEN " +
					" 	ELSE (NVL(L1.MENGE_NETTO, NVL(L1.MENGE,0)) ) * L1.MENGENVORZEICHEN " +
					" END ";
		...
		String sSql = 
			...
			+  "        L1.ID_ARTIKEL as ID_ARTIKEL, "
			+  "        ART.ID_EINHEIT as ID_EINHEIT, "		   
			+  "        sum( "
			+ 				sSqlMenge
			+  "        ) as SALDO_2 "
			+  "        FROM   		" + bibE2.cTO()+  ".JT_BG_LADUNG L1  " 
			+  "        INNER JOIN  " + bibE2.cTO()+  ".JT_BG_STATION S1 ON L1.ID_BG_STATION = S1.ID_BG_STATION  AND L1.ID_MANDANT = S1.ID_MANDANT "
			+  "        INNER JOIN   " + bibE2.cTO()+  ".JT_ARTIKEL ART ON ART.ID_ARTIKEL = L1.ID_ARTIKEL "
//			+ "			INNER JOIN JT_BG_ATOM A ON A.ID_BG_ATOM = L1.ID_BG_ATOM  "
//			+ " 		INNER JOIN JT_BG_LADUNG L2  ON A.ID_BG_ATOM = L2.ID_BG_ATOM AND L2.MENGENVORZEICHEN = L1.MENGENVORZEICHEN * -1 "

			+  "        INNER JOIN 	"
			+  "            ( 		"
			+  "                    SELECT LA.ID_ADRESSE_LIEFER AS LA_ID_ADRESSE_LIEFER, LA.BESCHREIBUNG AS LA_BESCHREIBUNG , ADRL.SONDERLAGER AS ADR_SONDERLAGER, LA.ID_ADRESSE_FREMDWARE AS LA_ID_ADRESSE_FREMD "
			+  "                	FROM " + bibE2.cTO()+  ".JT_LIEFERADRESSE LA  "
			+  "						INNER JOIN JT_ADRESSE ADRL ON LA.ID_ADRESSE_LIEFER = ADRL.ID_ADRESSE "
			+  "						WHERE LA.ID_ADRESSE_BASIS = 5525 AND NVL(ADRL.SONDERLAGER,'STRECKE') = 'STRECKE'"
			+  "						UNION SELECT 5525,NULL ,NULL ,NULL FROM DUAL"
			+  "						"
			+  "						"
			+  " 			)  "
			+  "        ON LA_ID_ADRESSE_LIEFER = S1.ID_ADRESSE  "
			
			+  " "                    
			+  " "       
			+  " 		WHERE L1.ID_ARTIKEL IS NOT NULL    "
			+  "        AND L1.ID_BG_STORNO_INFO IS NULL "
			+  "        AND L1.ID_BG_DEL_INFO IS NULL    "
			+  "        AND L1.LEISTUNGSDATUM IS NOT NULL "
			+           sSqlWhereArtikel
			+ 			sSqlWhereLager
		...
		m_Lagerdaten = new HashMap<String, BG_SaldoDaten>();
		m_UnitSummation = new BG_Saldo_UnitSummation();

		BG_Saldo_UnitSummationEntry oUnitSum = null;
		...
		for (int i = 0; i < cLagerDaten.length; i++) {
			...
			if (m_Inventurdaten.containsKey(sKey)){
				BG_SaldoNulldurchgangDaten oInventurdaten = m_Inventurdaten.get(sKey);
				dInventurmenge = oInventurdaten.get_Inventurmenge();
				dInventurDate  = oInventurdaten.get_InventurDatum();
			}
			
			BG_SaldoDaten oValue = new BG_SaldoDaten(Lager, Sorte, IDEinheit, dLagermenge, dInventurmenge, dInventurDate);
	 ...
	 * @return
	 */
	public BG_SaldoDaten getData(String idLager, String idSorte) {

		String sKey = idLager + "#" + idSorte;

		// auf jeden Fall ein Leeres Objekt zurückliefern
		BG_SaldoDaten oDaten = new BG_SaldoDaten();
	 ...
	 * @return
	 */
	public HashMap<String, BG_SaldoDaten> getListOfSaldoDaten() {
		return m_Lagerdaten;
	}
	...
	public BigDecimal getSumOfAllSaldoValuesFound() {
		...
		for (BG_SaldoDaten oSaldo : m_Lagerdaten.values()) {
	 ...
	 * @return
	 */
	public HashMap<String, BG_Saldo_UnitSummationEntry> getSummationRowsOfAllSaldoValuesFound() {
	...
	private void readInventurdaten(String sDatum, boolean bTagesbeginn ){
		...
		m_Inventurdaten = new HashMap<String, BG_SaldoNulldurchgangDaten>();
		...
		for (int i = 0; i < cInventurDaten.length; i++) {
			...
			BG_SaldoNulldurchgangDaten oValue = new BG_SaldoNulldurchgangDaten(Lager, Sorte, dInventurmenge, dInventurDate);

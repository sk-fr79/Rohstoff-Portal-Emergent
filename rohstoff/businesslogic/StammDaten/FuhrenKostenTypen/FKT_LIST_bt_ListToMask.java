...
public class FKT_LIST_bt_ListToMask extends RB_bt_List2Mask_V2 {
    private E2_NavigationList  naviList = null;
    
    public FKT_LIST_bt_ListToMask(boolean bEdit, E2_NavigationList p_naviList) {
        super(bEdit,p_naviList);
        this.naviList = p_naviList;
        
        this.add_GlobalValidator(bEdit?ENUM_VALIDATION.FUHREN_KOSTEN_TYP_EDIT.getValidator():ENUM_VALIDATION.FUHREN_KOSTEN_TYP_VIEW.getValidator());
          ...
          this.add_GlobalValidator(new XX_ActionValidator_NG() {
			...
			public MyE2_MessageVector isValid(Component oComponentWhichIsValidated) throws myException {
				...
				if (naviList.get_vSelectedIDs_Unformated().size()==0) {
					mv.add_MESSAGE(new MyE2_Alarm_Message(new MyE2_String("Sie müssen mindestens eine Datenzeile auswÃ¤hlen !")));
				}
				return mv;
			}
		});
    ...
    public RB_ModuleContainerMASK generate_MaskContainer() throws myException {
        return new FKT_MASK_MaskModulContainer();
    }
    @Override
    public RB_hm_multi_DataobjectsCollector generate_DataObjects4Mask(MyE2_MessageVector mv_sammler) throws myException {
        ...
        for (String id: v_ids) {
            collector.put(id, new FKT_MASK_DataObjectCollector(id,aktuellerStatus));
        }
        return collector;
    }
    @Override
    public MyE2_String generate_TitelInfo4MaskWindow(RB_ModuleContainerMASK rb_ModulContainerMask) throws myException {
        return new MyE2_String(this.is_UsedToEdit()?"Bearbeiten von Fuhrenkosten-Varianten":"Anzeige von Fuhrenkosten-Varianten");
    }
    @Override
    public MyE2_String generate_Meldung4SaveRecord(    RB_ModuleContainerMASK rb_ModulContainerMask) throws myException {
        return new MyE2_String("Fuhrenkosten-Variante wurde gespeichert");
    }
    @Override
    public Vector<XX_ActionAgentWhenCloseWindow> generate_WindowCloseActions(RB_ModuleContainerMASK rb_ModulContainerMask,E2_NavigationList navilist) throws myException {
    	...
    	v_rueck.add(new XX_ActionAgentWhenCloseWindow(rb_ModulContainerMask) {
			...
			public void executeAgentCode(ExecINFO oExecInfo) throws myException {
				FKT_LIST_bt_ListToMask.this.get_NaviList()._REBUILD_ACTUAL_SITE("");
			}
		});

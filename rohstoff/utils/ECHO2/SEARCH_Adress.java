...
public class SEARCH_Adress extends MyE2_MaskSearchField
{
	...
	public SEARCH_Adress()throws myException
	{
		this(false);
	}

	
	public SEARCH_Adress(boolean bShowEraser ) throws myException
	{
		super(		"JT_ADRESSE.ID_ADRESSE, NVL(VORNAME,'<vorname>')," +
...
//						" FROM " + bibE2.cTO() + ".JT_ADRESSE WHERE ID_ADRESSE=#WERT#",
					 SEARCH_Adress.cAdressSearchQuery4LabelMainAdresse,
					null, bShowEraser);
	...
	public SEARCH_Adress(boolean MarkLieferanten, boolean MarkAbnehmer, Insets oInsets4Components) throws myException
	{
		super(		"JT_ADRESSE.ID_ADRESSE, NVL(VORNAME,'<vorname>')," +
					"CASE WHEN NAME1 IS NULL THEN CSCONVERT('<name1>','NCHAR_CS') ELSE CSCONVERT(NAME1||' '||NVL(NAME2,''),'NCHAR_CS')  END ," +
					"NVL(PLZ,'<plz>'),NVL(ORT,'<ort>'),'<typ>'", 

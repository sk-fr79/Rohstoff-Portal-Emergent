...
public class LAGER_KONTO_PREIS_Daemon extends XX_SQL_STACK_DAEMON {
	...
	public LAGER_KONTO_PREIS_Daemon() throws myException 
	{
		super();
	}
...
//					&& !test.get_IS_DELETE()) //nur insert und update werden beruecksichtigt !
	...
	public Vector<String> getTriggerStatementsAfterSQL(MyConnection oConn, MyE2_MessageVector oMV) throws myException 
	{
		...
		for (MyDBToolBox_LOG_INFO test: this.get_vLogInfos())
		{
			...
			if (test.get_cTABLENAME().toUpperCase().equals("JT_VPOS_RG"))  {
				LAG_LagerPreisHandler handler = new LAG_LagerPreisHandler(null); 
				handler.pruefePreisaenderungLagerRechnung(cID_TABLE);

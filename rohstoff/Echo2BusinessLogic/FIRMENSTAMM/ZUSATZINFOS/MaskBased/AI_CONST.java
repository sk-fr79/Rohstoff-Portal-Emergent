 
package rohstoff.Echo2BusinessLogic.FIRMENSTAMM.ZUSATZINFOS.MaskBased;
  
import panter.gmbh.Echo2.MyE2_String;
import panter.gmbh.Echo2.RB.BASICS.RB_KM;
import panter.gmbh.basics4project.DB_ENUMS._TAB;
import panter.gmbh.indep.S;
import panter.gmbh.indep.enumtools.IF_enum_4_db;
import panter.gmbh.indep.enumtools.bibENUM;
import panter.gmbh.indep.exceptions.myException;
 
public class AI_CONST {
 	
 	
    public enum AI__NAMES implements IF_enum_4_db {
        
        CHECKBOX_LISTE( S.ms("Auswahl-Checkbox"))
        ,MARKER_LISTE( S.ms("Markierung Liste"))
        ,DIRECT_DEL( S.ms("Loeschbutton in Listenzeile"))
        ,DIRECT_EDIT( S.ms("Editbutton in Listenzeile"))
        ,DIRECT_VIEW( S.ms("Anzeigebutton in Listenzeile"))
        ,DIRECT_UPLOAD( S.ms("Dateien hochladen"))
        
        ,INFO_BLOCK_ERFASSUNG_ANLASS( S.ms("Erfassung/Anlass"))
        ,INFO_BLOCK_DATUM_UND_WV( S.ms("Datum/Wiedervorlage"))
        ,INFO_BLOCK_BENUTZER( S.ms("Mitarbeiter"))
        ,INFO_BLOCK_ERGEBNIS( S.ms("Ergebnisse"))
        
        ,SEND_ADHOC_MESSAGE(S.ms("Sofort-Meldung"))
        ,LAST_USER(S.ms("Geändert von"))
        ,LAST_CHANGE(S.ms("Änd.Dat."))
        
        ;
        
        private MyE2_String userText = null; 
        private String      m_dbVal = null;
        
        private AI__NAMES(MyE2_String p_userText) {
            this.userText = p_userText;
        }
        
        //konstuktor mit abweichenden werten
        private AI__NAMES(MyE2_String p_userText, String dbVal) {
            this.userText = p_userText;
            this.m_dbVal=dbVal;
        }
        
        @Override
        public String db_val() {
         	if (S.isFull(m_dbVal)) {
        		return m_dbVal;
        	}
           return this.name();
        }
        
        @Override
        public String user_text() {
            if (S.isEmpty(this.userText)) {
                return this.name();
            } else {
                return this.userText.CTrans();
            }
        }
        
        @Override
        public String user_text_lang() {
            return this.user_text();
        }
        
        @Override
        public String[][] get_dd_Array(boolean emptyPairInFront) throws myException {
            return null;
        }
        
    }
    
    
    
    /**
     * 
     * @return der ueberall verwendete mask-key
     * @throws myException
     */
    public static _TAB getLeadingTable() throws myException {
    	return _TAB.adresse_info;
    }
    
    
    /**
     * 
     * @return der ueberall verwendete mask-key
     * @throws myException
     */
    public static RB_KM getLeadingMaskKey() throws myException {
    	return _TAB.adresse_info.rb_km();
    }
    
    
    
    
    
    public enum AI__BUTTONS implements IF_enum_4_db  {
        DELETE("LOESCHE_ADRESSE_INFO")
        ,EDIT("BEARBEITE_ADRESSE_INFO")
        ,VIEW("ANZEIGE_ADRESSE_INFO")
        ,NEW("NEUEINGABE_ADRESSE_INFO")
        ,
        
        ;
        
        private String KEY = null;
        
        private AI__BUTTONS() {
            this.KEY=this.name();
        }
        private AI__BUTTONS(String key) {
            this.KEY=key;
        }
        public String     db_val() {    return S.isEmpty(this.KEY)?this.name():this.KEY;}
        @Override
        public String user_text_lang() {return this.name();    }
        @Override
        public String user_text() {        return this.name();        }
        @Override
        public String[][] get_dd_Array(boolean emptyPairInFront) throws myException {
            return bibENUM.dd_array(AI__BUTTONS.values(), emptyPairInFront);
        }
        
    }
    

    
    
    /*
     * enum: hier koennen numerische (long) werte zentral gesteuert werden 
     */
    public static enum AI__NUM_CONST {
         MASKPOPUP_WIDTH(new Integer(800))
        ,MASKPOPUP_HEIGHT(new Integer(650))
        ,MASKPOPUP_DESCRIPTION_COL_SIZE(new Integer(150))
        ,MASKPOPUP_FIELD_COL_SIZE(new Integer(700))
         
    	;
    	
    	private Integer   m_value = null;
    	
    	private AI__NUM_CONST(Integer p_value) {
    		this.m_value=p_value;
    	}
    	
    	public Integer getValue() {
    		return this.m_value;
    	}
    }
    
    
}
 
 

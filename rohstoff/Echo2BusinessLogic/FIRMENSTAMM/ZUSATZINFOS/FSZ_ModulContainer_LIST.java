...
public class FSZ_ModulContainer_LIST extends E2_BasicModuleContainer
{
	
	private E2_NavigationList 				oNavigationList = null;
	private FSZ_LIST_ComponentMAP			oLISTComponentMAP = null;
	private E2_BASIC_EditListButtonPanel 	oBasicBedienpanel = null;
	private E2_BasicModuleContainer_MASK	oOwnE2_ModulContainerMASK  = null;
	
	private boolean                         b_Infos = false;
	 
	public FSZ_ModulContainer_LIST(E2_BasicModuleContainer_MASK	 OwnE2_ModulContainerMASK, boolean bInfos) throws myException
	{
		super();
		this.oOwnE2_ModulContainerMASK = OwnE2_ModulContainerMASK;
		
//		this.set_MODUL_IDENTIFIER(E2_MODULNAMES.NAME_MODUL_FIRMENSTAMM_MASK_INFOLISTE);
		...
		this.oLISTComponentMAP = new FSZ_LIST_ComponentMAP(bInfos);
		...
		//2013-05-13: upload-button fuer zusatzinfos
		E2_ButtonUpDown_NavigationList_to_Archiv oButtonUpload = new E2_ButtonUpDown_NavigationList_to_Archiv(
				this.oNavigationList,
				this.oOwnE2_ModulContainerMASK.get_MODUL_IDENTIFIER(),   //validierung findet auf der Adressmaske statt
				this.oOwnE2_ModulContainerMASK.get_MODUL_IDENTIFIER(),   //validierung findet auf der Adressmaske statt
				null,
				"INFOS_ZUR_ADRESSE_UPLOAD_ZU_INFOTABELLE",null);

		this.oBasicBedienpanel.add(oButtonUpload, E2_INSETS.I_10_0_10_0);
		
		/*
		 * jetzt button-validators einfuegen
		 */
		String cMotherModuleName = OwnE2_ModulContainerMASK.get_MODUL_IDENTIFIER();
		String ButtonsBasis = bInfos?"ZUSATZ_INFO_":"SYSTEM_MELDUNGEN_";
		...
		oButtonUpload.add_GlobalValidator_removeOthers(new E2_ButtonAUTHValidator(cMotherModuleName,ButtonsBasis+"ZUSATZDATEIN_HOCHLADEN"));
	...
	public void set_BASE_ADRESS_ID(String cID_Unformated) throws myException
	{
		try
		{
			...
			if (this.b_Infos)    //dann die nachrichten-ziele vorbesetzen
			{
				RECORD_ADRESSE  recAdresse = new RECORD_ADRESSE(cID_Unformated);
				this.oLISTComponentMAP.get_oSQLFieldMAP().get("ID_USER").set_cDefaultValueFormated(recAdresse.get_ID_USER_cF_NN(""));
				this.oLISTComponentMAP.get_oSQLFieldMAP().get("ID_USER_ERSATZ").set_cDefaultValueFormated(recAdresse.get_ID_USER_ERSATZ_cF_NN(""));
				this.oLISTComponentMAP.get_oSQLFieldMAP().get("ID_USER_SACHBEARBEITER").set_cDefaultValueFormated(recAdresse.get_ID_USER_SACHBEARBEITER_cF_NN(""));
			}
		}
		catch (Exception ex)
		{
	...
	private class ownADDON_Buttons_Status_AGENT extends XX_ADDON_Buttons_Status_AGENT
	{
		...
		{
			FSZ_ModulContainer_LIST oThis = FSZ_ModulContainer_LIST.this;
			...
			MyE2_TabbedPane oTabbedPane = oThis.oOwnE2_ModulContainerMASK.get_oTabbedPane();

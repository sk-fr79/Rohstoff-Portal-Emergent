...
public class FS_Component_MASK_DAUGHTER_MATSPEZ extends XX_FULL_DAUGHTER
{

	private FSMS_ModulContainer_LIST oListContainer = null;


	public FS_Component_MASK_DAUGHTER_MATSPEZ(	SQLFieldMAP 			osqlFieldMap,
												FS_ModulContainer_MASK	oModulContainerMASK) throws myException
	{
		super((SQLFieldForPrimaryKey)osqlFieldMap.get_("ID_ADRESSE"));
		this.oListContainer = new FSMS_ModulContainer_LIST();
	...
	public Component build_content_for_Mask(String cValueFormated,String cValueUnformated, String cMASK_STATUS) throws myException
	{
		...
		{
			return new MyE2_Label(new MyE2_String("Materialspezifikationen können erst erfasst werden, wenn die Firmenadresse gespeichert wurde!"));
		}
	...
	private void setMaskTitle() throws myException
	{
		...
		MyE2_String oString = new MyE2_String("Materialspezifikationen bei der Firma: ");
	...
	{
		this.oListContainer.get_oBT_DELETE_MATSPEZ().set_bEnabled_For_Edit(false);
		this.oListContainer.get_oBT_EDIT_MATSPEZ().set_bEnabled_For_Edit(false);
		this.oListContainer.get_oBT_VIEW_MATSPEZ().set_bEnabled_For_Edit(false);
		this.oListContainer.get_oBT_NEW_MATSPEZ().set_bEnabled_For_Edit(false);
		
		if (cMASK_STATUS.equals(E2_ComponentMAP.STATUS_EDIT))
		{
			this.oListContainer.get_oBT_DELETE_MATSPEZ().set_bEnabled_For_Edit(true);
			this.oListContainer.get_oBT_EDIT_MATSPEZ().set_bEnabled_For_Edit(true);
			this.oListContainer.get_oBT_VIEW_MATSPEZ().set_bEnabled_For_Edit(true);
			this.oListContainer.get_oBT_NEW_MATSPEZ().set_bEnabled_For_Edit(true);
		}
	...
	{
		this.removeAll();
		this.add(new MyE2_Label(new MyE2_String("Materialspezifikationen können erst erfasst werden, wenn die Firmenadresse gespeichert wurde!")));

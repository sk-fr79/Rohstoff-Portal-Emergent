...
public class AI__Const {
	...
	public static TreeMap<BLOCK_TO_SHOW, IF_Saveable>  generate_AI_CheckBoxes() {
		TreeMap<BLOCK_TO_SHOW, IF_Saveable> hm = new TreeMap<AI__Const.BLOCK_TO_SHOW, IF_Saveable>();
		
		for (BLOCK_TO_SHOW block: BLOCK_TO_SHOW.values()) {
			if (!block.name().startsWith("SF_")) {
				
			}
			hm.put(block, new AI__CheckBox(block.label_for_user, block.toolTipText, false, block));
		}

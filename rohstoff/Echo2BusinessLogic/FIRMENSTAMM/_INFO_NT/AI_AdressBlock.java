...
public class AI_AdressBlock extends AR_DataBlock {

	private Vector<String>  						v_id_adresses = new Vector<String>();
	private TreeMap<BLOCK_TO_SHOW, IF_Saveable>  	hm_Steuerpult = null;
	
	
	public AI_AdressBlock(Vector<String> id_adresses, TreeMap<BLOCK_TO_SHOW, IF_Saveable>  	p_hm_CheckBoxToSelectRange) throws myException {
		super();
		this.v_id_adresses.addAll(id_adresses);
		this.hm_Steuerpult = p_hm_CheckBoxToSelectRange;
		
		for (String id: id_adresses) {
			this.add(new AI_AdressRow(new RECORD_ADRESSE(id),true));
		}
	}
	...
	public IF_Saveable get_SteuerpultKnopf(BLOCK_TO_SHOW block) {
	...
	public int get_Abstand() throws myException {
		...
		try {
			abstand = new MyInteger(
					this.get_SteuerpultKnopf(BLOCK_TO_SHOW.SF_SETZE_ABSTAND).get_value_to_save()
					).get_iValue();
		} catch (Exception e) {
			e.printStackTrace();
			abstand = 0;
	...
	public int get_colorDiff() throws myException {
		...
		try {
			diff = new MyInteger(
					this.get_SteuerpultKnopf(BLOCK_TO_SHOW.SF_SETZE_FARBDIFFERENZ).get_value_to_save()
					).get_iValue();
		} catch (Exception e) {
			e.printStackTrace();
			diff = 10;
	...
	public boolean is_set(BLOCK_TO_SHOW block) throws myException {
		boolean rueck = true;
	...
	public boolean zeigeBeschriftung() throws myException {
		return this.is_set(BLOCK_TO_SHOW.BESCHRIFTUNG);
	}

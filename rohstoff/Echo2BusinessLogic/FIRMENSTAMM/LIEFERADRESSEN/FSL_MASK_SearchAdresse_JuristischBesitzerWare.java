 ...
 *
 */
public class FSL_MASK_SearchAdresse_JuristischBesitzerWare extends RB_HL_SearchMainAdress implements MyE2IF__DB_Component, IF_FocusTraversalIndexSetter {
	
	private FSL_MASK_SearchAdresse_JuristischBesitzerWareLager referenzAdresseLager = null;    //die zugeoerige lageradresse wird nach dem suchen mit dem wert der gefundenen hauptadresse gefuellt
	...
	public FSL_MASK_SearchAdresse_JuristischBesitzerWare(SQLField sqlField) throws myException {
		super();
		...
		this.EXT().add_FieldSetters_AND_Validator__BeforeReadInputMAP(
				new XX_FieldSetter_AND_Validator()
				{
					...
					public MyE2_MessageVector isValid(String cstatus_map,MyE2EXT__Component EXT_own) throws myException {
						FSL_MASK_SearchAdresse_JuristischBesitzerWare oThis =  FSL_MASK_SearchAdresse_JuristischBesitzerWare.this;
		...
		this.get_buttonErase()._aaa(()-> {
			executer.execute();
		});
		
//		this._registerAfterSetDbValueManuellListener((sf)-> {
//			MyE2_MessageVector mv = bibMSG.getNewMV();
//			String value = S.NN(sf.get_tf_search_input().getText());
//			if (S.isEmpty(value)) {
//				referenzAdresseLager.rb_set_db_value_manual("");
//			} else {
//				MyLong ml = new MyLong(value); 
//				if (ml.isOK()) {
//					referenzAdresseLager.rb_set_db_value_manual("");
//				}
//			}
...
//		});
	...
	private class ButtonSearchAdressesNearby extends E2_Button {
		...
		public ButtonSearchAdressesNearby() {
			...
			this._style(E2_Button.StyleImageButton())
				._aaa(new ownAction(FSL_MASK_SearchAdresse_JuristischBesitzerWare.this))._ttt(S.ms("Suche nach geografisch in der Nähe befindlichen Hauptadressen"));
		}
		...
		private class ownAction extends RB_SearchField_AbstractActionStartSearch {
			...
			protected MyE2_MessageVector fillResultButtonArray() throws myException {
				...
				Object  oLongitude = new PdServiceReadFromDBFieldMask().getActualRawMaskVal(FSL_MASK_SearchAdresse_JuristischBesitzerWare.this.ext.get_oComponentMAP(), ADRESSE.longitude);
				Object  oLatitude = new PdServiceReadFromDBFieldMask().getActualRawMaskVal(FSL_MASK_SearchAdresse_JuristischBesitzerWare.this.ext.get_oComponentMAP(), ADRESSE.latitude);
		...
		private class ownResultButton extends RB_ResultButtonWithRec21 {
			...
			public ownResultButton(Rec21 rec21, String cText) throws myException {
				super(FSL_MASK_SearchAdresse_JuristischBesitzerWare.this, rec21, cText, E2_Button.StyleTextButton());
				this.add_oActionAgent(new RB_ResultButton_action(rec21, FSL_MASK_SearchAdresse_JuristischBesitzerWare.this));
				this._aaa(()->{
					executer.execute();
				});
				this.sortText = cText;
	...
	//executer-interface um nach einer aktion das lager evtl. leer zu machen
	private IF_Executer executer = ()-> {
		
		try {
			boolean mustClearLager = true;
			//nachsehen, ob das alte lager passt zur gerade gewaehlten adresse passt
			MyLong mlBesitzerWare      = new MyLong(get_cActualDBValueFormated());
			MyLong mlBesitzerWareLager = new MyLong(referenzAdresseLager.get_cActualDBValueFormated());
			if (mlBesitzerWareLager.isOK() && mlBesitzerWare.isOK()) {
				//hauptadresse raussuchen
				Rec21_adresse adresseLager = new Rec21_adresse()._fill_id(mlBesitzerWareLager.get_lValue());
				if (adresseLager._getMainAdresse().getId()==mlBesitzerWare.get_lValue()) {
					mustClearLager = false;
				}
				
			}
			if (mustClearLager) {	
				if (mlBesitzerWare.isOK()) {
					referenzAdresseLager.rb_set_db_value_manual(""+mlBesitzerWare.get_lValue());
				} else {
					referenzAdresseLager.rb_set_db_value_manual("");
				}
			}
			...
			e.printStackTrace();
			bibMSG.getNewMV()._addAlarmUT(e.ErrorMessage);
		}
	...
	public MyE2_MessageVector  execute_searchquery_and_fill_resultbutton_array(String search_text) throws myException {
		MyE2_MessageVector mv = super.execute_searchquery_and_fill_resultbutton_array(search_text);
		
		for ( RB_ResultButton_IF[] arr: this.get_rb_ResultButtonArray()) {
			for (RB_ResultButton_IF rb: arr) {
				if (rb instanceof RB_ResultButton) {
					((RB_ResultButton)rb)._aaa(()->{
						executer.execute();
					});
				}
			}
	 ...
	 */
	public FSL_MASK_SearchAdresse_JuristischBesitzerWare _setReferenzAdresseLager(FSL_MASK_SearchAdresse_JuristischBesitzerWareLager referenzAdresse) {
		this.referenzAdresseLager = referenzAdresse;
		return this;
	}

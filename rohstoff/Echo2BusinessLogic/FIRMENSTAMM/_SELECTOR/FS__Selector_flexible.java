...
public class FS__Selector_flexible extends SF__Filter {
	
	public FS__Selector_flexible() throws myException {
		super();
		this.add_table(_TAB.adresse);
		this.add_table(_TAB.firmeninfo);
		this.add_table(_TAB.user);
		this.add_table(_TAB.land);
	}

	@Override
	public void fill_components_4_selector(E2_Grid4MaskSimple grid4selektor, MyE2_Button showActualSelection,MyE2_Button bt_clear_anzeige, MyE2_Button bt_start_filterselectdefinition) throws myException {
		grid4selektor.removeAll();
		
		
		String c_where = this.get_WhereBlock();
		if (S.isEmpty(c_where)) {
			showActualSelection.setText("<leer>");
			bt_start_filterselectdefinition.setText("Filter definieren");
		} else {
			if (c_where.length()>100) {
				showActualSelection.setText(c_where.substring(0,100));
			}else {
				showActualSelection.setText(c_where);
			}
			showActualSelection.setToolTipText(c_where);
			showActualSelection.setLineWrap(true);
		...
		MyE2_Grid help = showActualSelection.in_border(new E2_ColorDDark(), MyE2_Grid.LAYOUT_LEFT_CENTER(E2_INSETS.I(0,0,0,0)), new Extent(200), new Extent(15));

 ...
 *
 */
public class FSI_ZDR_Container extends E2_Grid {
	...
	public FSI_ZDR_Container(Vector<String> id_adresses) throws myException {
		super();
		this._s(1)._bo_no()._w100();
		this.v_id_adressen.clear();
		this.v_id_adressen._a(id_adresses);
		
		this.gridSteuerbuttons._s(2)._bo_no()._w100();
		...
		AI__SelectSettings_NG abstand_box = new AI__SelectSettings_NG(
				ENUM_Selectfield_Datei.SF_INSETS.get_lbl_4_user(),		
				new String[]  {"0","1","2","3","4","5","6","7","8"}, 
				"1", 
				new Extent(60),
				ENUM_Selectfield_Datei.SF_INSETS.getTooltiptext());
		this.hm_Steuerpult.put(ENUM_Selectfield_Datei.SF_INSETS.name(), 	abstand_box);
		this.gridSteuerbuttons._a(abstand_box.get_Comp(), gld);
		
		
		AI__SelectSettings_NG farbdiff_box = new AI__SelectSettings_NG(ENUM_Selectfield_Datei.SF_FARBDIFF.get_lbl_4_user(),
				new String[]{"0","5","10","15","20","25","30"}, 
				"1", 
				new Extent(60),
				ENUM_Selectfield_Datei.SF_FARBDIFF.getTooltiptext());		
		this.hm_Steuerpult.put(ENUM_Selectfield_Datei.SF_FARBDIFF.name(), 	farbdiff_box);
		this.gridSteuerbuttons._a(farbdiff_box.get_Comp(), gld);
		
		this.container = new MyE2_ContainerEx(infoGrid);

		//settings laden
      	new FSI_ZDR_SaveSettings(hm_Steuerpult).RESTORE();
	...
	public void build() throws myException {
		...
		FSI_ZDR_DataBlock fi  = new FSI_ZDR_DataBlock(this.v_id_adressen, this.hm_Steuerpult);
	...
	private class ownActionAgentSaveSettings extends XX_ActionAgent {
		@Override
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			new FSI_ZDR_SaveSettings(FSI_ZDR_Container.this.hm_Steuerpult).SAVE();
		}
	}
}

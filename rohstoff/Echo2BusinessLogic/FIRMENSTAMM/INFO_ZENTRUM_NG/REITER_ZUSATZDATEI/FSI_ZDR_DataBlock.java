 ...
 *
 */
public class FSI_ZDR_DataBlock extends AR_DataBlock {
	...
	public FSI_ZDR_DataBlock(Vector<String> id_adresses, TreeMap<String, IF_Saveable>  	p_hm_CheckBoxToSelectRange) throws myException{
		super();
		this.v_id_adresses.addAll(id_adresses);
		this.hm_Steuerpult = p_hm_CheckBoxToSelectRange;
		
		for (String id: id_adresses) {
			this.add(new FSI_ZDR_AdressRow(new RECORD_ADRESSE(id),true));
		}
	}
	...
	public int get_Abstand() throws myException {
		...
		try {
			abstand = new MyInteger(
					this.get_SteuerpultKnopf(ENUM_Selectfield_Datei.SF_INSETS.name()).get_value_to_save()
					).get_iValue();
		} catch (Exception e) {
			e.printStackTrace();
			abstand = 0;
	...
	public int get_colorDiff() throws myException {
		...
		try {
			diff = new MyInteger(
					this.get_SteuerpultKnopf(ENUM_Selectfield_Datei.SF_FARBDIFF.name()).get_value_to_save()
					).get_iValue();
		} catch (Exception e) {
			e.printStackTrace();
			diff = 10;

...
public class AI__Container_NG extends MyE2_Grid {
	...
	private TreeMap<AI__Const_NG.BLOCK_TO_SHOW, IF_Saveable>  	hm_Steuerpult = new TreeMap<AI__Const_NG.BLOCK_TO_SHOW, IF_Saveable>();
	...
	public AI__Container_NG(Vector<String> id_adresses) throws myException {
		...
		for (AI__Const_NG.BLOCK_TO_SHOW block: AI__Const_NG.BLOCK_TO_SHOW.values()) {
			if (!block.name().startsWith("SF_")) {
				hm_Steuerpult.put(block, new AI__CheckBox_NG(block.label4User(), block.toolTipText(), false, block));
			}
		}
		hm_Steuerpult.put(AI__Const_NG.BLOCK_TO_SHOW.SF_SETZE_ABSTAND, 		new AI__SelectSettings_NG(	BLOCK_TO_SHOW.SF_SETZE_ABSTAND.label4User(),		
																						abstaende, 
																						"1", 
																						new Extent(60),
																						BLOCK_TO_SHOW.SF_SETZE_ABSTAND.toolTipText()));
		hm_Steuerpult.put(AI__Const_NG.BLOCK_TO_SHOW.SF_SETZE_FARBDIFFERENZ, new AI__SelectSettings_NG(	BLOCK_TO_SHOW.SF_SETZE_FARBDIFFERENZ.label4User(),
																						farbdiff, 
																						"1", 
																						new Extent(60),
...
//      	gridSteuerbuttons.add(new MyE2_Label(""),E2_INSETS.I(2,0,2,0));
     	...
     	gridSteuerbuttons.add(this.hm_Steuerpult.get(BLOCK_TO_SHOW.SF_SETZE_ABSTAND).get_Comp(),E2_INSETS.I(2,0,2,0));
      	...
      	//settings laden
      	new AI__SaveSetting_NG(hm_Steuerpult).RESTORE();
	...
	public void _build() throws myException {
		...
		AI_AdressBlock_NG fi  = new AI_AdressBlock_NG(this.v_id_adressen, this.hm_Steuerpult);
      	fi.fill(infoGrid);
	...
	private class ownActionAgentSaveSettings extends XX_ActionAgent {
		@Override
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			new AI__SaveSetting_NG(AI__Container_NG.this.hm_Steuerpult).SAVE();
		}
	}

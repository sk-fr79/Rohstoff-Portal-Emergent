...
public class AI_MitarbeiterAdressDataBlock_NG extends AR_DataBlock {
	
	private RECORD_ADRESSE_extend 	rec_Adresse   = null;
	private RECLIST_MITARBEITER  	rl_ma= 			null;
	public AI_MitarbeiterAdressDataBlock_NG(RECORD_ADRESSE ra) throws myException {
		super();
	...
	public boolean _must_be_filled() throws myException {
		...
		if (startBlock!=null) {
			IF_Saveable steuerknopf = ((AI_AdressBlock_NG)startBlock).get_SteuerpultKnopf(BLOCK_TO_SHOW.MITARBEITER);
			if (steuerknopf!=null && steuerknopf instanceof AI__CheckBox_NG) {
				return ((AI__CheckBox_NG)steuerknopf).isSelected() && this.rl_ma.size()>0;
			}
		}
...
//		return comps;
	...
	private class MitarbeiterAdress extends AR_DataRow {
		...
		public MitarbeiterAdress(RECORD_ADRESSE p_recADRESS) throws myException {
			...
			this.add_daughter_DataBlock(new AI_TelefonFaxDataBlock_NG(this.recADRESS, new MyE2_String("Mitarbeiteradresse").CTrans(),BLOCK_TO_SHOW.MITARBEITER_TELEFON));
			this.add_daughter_DataBlock(new AI_eMailDataBlock_NG(this.recADRESS, new MyE2_String("Mitarbeiteradresse").CTrans(),BLOCK_TO_SHOW.MITARBEITER_MAIL));
		...
		public boolean _must_be_filled() throws myException {
			...
			IF_Saveable  steuerknopf = ((AI_AdressBlock_NG)startBlock).get_SteuerpultKnopf(BLOCK_TO_SHOW.INAKTIVE_MITARBEITER_AUSBLENDEN);
			return this.recADRESS.is_AKTIV_YES() || (!((AI__CheckBox_NG)steuerknopf).isSelected());
		}

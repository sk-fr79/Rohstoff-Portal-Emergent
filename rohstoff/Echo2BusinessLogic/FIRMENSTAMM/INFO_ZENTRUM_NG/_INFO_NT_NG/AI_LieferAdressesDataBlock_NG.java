...
public class AI_LieferAdressesDataBlock_NG extends AR_DataBlock {

	private RECORD_ADRESSE_extend 	rec_Adresse   = null;
	private RECLIST_LIEFERADRESSE  	rl_la = null;  
	public AI_LieferAdressesDataBlock_NG(RECORD_ADRESSE ra) throws myException {
		super();
	...
	public boolean _must_be_filled() throws myException {
		...
		if (startBlock!=null) {
			IF_Saveable steuerknopf = ((AI_AdressBlock_NG)startBlock).get_SteuerpultKnopf(BLOCK_TO_SHOW.LIEFERADRESSEN);
			if (steuerknopf!=null && steuerknopf instanceof AI__CheckBox_NG) {
				return ((AI__CheckBox_NG)steuerknopf).isSelected() && this.rl_la.size()>0;
			}
		}
...
//		comps[0][0] = new AR_Label(" ",null,new AR_LayoutData(19,E2_INSETS.I(1,5,1,5),null,0));
	...
	private class LieferAdress extends AR_DataRow {
		...
		public LieferAdress(RECORD_ADRESSE p_recADRESS) throws myException {
			...
			this.add_daughter_DataBlock(new AI_TelefonFaxDataBlock_NG(this.recADRESS, new MyE2_String("Lieferadresse").CTrans(),BLOCK_TO_SHOW.LIEFERADRESSEN_TELEFON));
			this.add_daughter_DataBlock(new AI_eMailDataBlock_NG(this.recADRESS, new MyE2_String("Lieferadresse").CTrans(),BLOCK_TO_SHOW.LIEFERADRESSEN_MAIL));
		}
		...
		public boolean _must_be_filled() throws myException {
			...
			IF_Saveable  steuerknopf = ((AI_AdressBlock_NG)startBlock).get_SteuerpultKnopf(BLOCK_TO_SHOW.INAKTIVE_LIEFERADRESSEN_AUSBLENDEN);

...
public class FSI_MR_MeldungenDataBlock extends AR_DataBlock {
	...
	public FSI_MR_MeldungenDataBlock(RECORD_ADRESSE recAdresse, LinkedHashMap<String, IF_Saveable> p_steuerung_hashmap) throws myException {
		super();
		
		this.steuerung_hashmap = p_steuerung_hashmap;
		
		this.rec_Adresse = new RECORD_ADRESSE_extend(recAdresse);

		this.rl_ai = recAdresse.get_DOWN_RECORD_LIST_ADRESSE_INFO_id_adresse(new vgl(ADRESSE_INFO.ist_message,"Y").s(),ADRESSE_INFO.message_typ.fn() ,true);
		
		if (rl_ai.size()>0) {
			Vector<RECORD_ADRESSE_INFO> v_ai = new Vector<RECORD_ADRESSE_INFO>();
			v_ai.addAll(rl_ai.GET_SORTED_VECTOR(bibALL.get_Vector(ADRESSE_INFO.message_typ.fn(),ADRESSE_INFO.datumeintrag.fn()), true));
	...
	public IF_AR_Component[][] _generate_titelComponentsInFrontOfBlock() throws myException {
		if (this.get_topAdressBlock().zeigeBeschriftung()) {
			...
			comp[0][0] = new AR_Label(new AR_LayoutData(20-this.get_complete_insets(),get_topAdressBlock().get_insets(),-1*0), 
										new MyE2_String("Programm-Meldungen zur Adresse",true," <",false,this.rec_Adresse.get__FullNameAndAdress_Typ1(),false,">",false), new AR_StyleLabel(false, true, true, -2));
			return comp;
		}
		return null;
	...
	private class ZusatzInfosRow extends AR_DataRow {
		...
		public ZusatzInfosRow(RECORD_ADRESSE_INFO ai) throws myException {
			super();
			this.rec_ai = ai;
			this.messagetyp = rec_ai.get_MESSAGE_TYP_cF_NN("");
		}
		...
		public IF_AR_Component[][] _generate_Components() throws myException {
			...
			String infoTypLesbar = "";
			String infoTyp = this.rec_ai.get_MESSAGE_TYP_cUF_NN("");

			if (!S.isEmpty(infoTyp)) {
				infoTypLesbar = S.NN(myCONST.HM_ADRESS_INFO_TYP.get(infoTyp));
			}
		...
		@Override
		public boolean _must_be_filled() throws myException {
			return steuerung_hashmap.get(messagetyp).get_value_to_save().equals("Y");
		}
	}
}

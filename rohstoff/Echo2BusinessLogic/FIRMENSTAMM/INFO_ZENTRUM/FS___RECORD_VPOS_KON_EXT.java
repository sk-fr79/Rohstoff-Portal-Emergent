...
public class FS___RECORD_VPOS_KON_EXT extends __RECORD_VPOS_KON_EXT
{	


	public FS___RECORD_VPOS_KON_EXT(RECORD_VPOS_KON recordOrig)
	{
		super(recordOrig);
	}
	...
	public FSI_SortObject get_View_and_Sort_Pair(String cKenner) throws myException
	{
		...
		if (cKenner.equals(FSI_CONST.KON_BUCHNUMMER))
		{
			return new  FSI_SortObject(this.get_UP_RECORD_VKOPF_KON_id_vkopf_kon().get_BUCHUNGSNUMMER_cUF_NN("<ID:"+this.get_ID_VKOPF_KON_cF()+">")+"-"+this.get_POSITIONSNUMMER_cF_NN("??"));
		}
		else if (cKenner.equals(FSI_CONST.KON_FR_BEST_NUMMER))
		{
			return new  FSI_SortObject(this.get_BESTELLNUMMER_cUF_NN("<Best-Nr.>"));
		}
		else if (cKenner.equals(FSI_CONST.KON_DRUCKDATUM))
		{
			return  new FSI_SortObject(this.get_UP_RECORD_VKOPF_KON_id_vkopf_kon().get_DRUCKDATUM_cF_NN("??.??.????"),
						this.get_UP_RECORD_VKOPF_KON_id_vkopf_kon().get_DRUCKDATUM_VALUE_FOR_SQLSTATEMENT());
		}
		else if (cKenner.equals(FSI_CONST.KON_GUELTIG))
		{
...
//			cGueltig += (" - "+this.get_DOWN_RECORD_LIST_VPOS_KON_TRAKT_id_vpos_kon().get(0).get_GUELTIG_BIS_cF_NN("??.??.????").substring(0,6));
			...
			return  new FSI_SortObject(	cGueltig,
										this.get_DOWN_RECORD_LIST_VPOS_KON_TRAKT_id_vpos_kon().get(0).get_GUELTIG_VON_VALUE_FOR_SQLSTATEMENT());
		}
		else if (cKenner.equals(FSI_CONST.KON_MENGE))
		{
			return new FSI_SortObject(	MyNumberFormater.formatDez( this.get_ANZAHL_bdValue(BigDecimal.ZERO),0,true),
										this.get_ANZAHL_bdValue(BigDecimal.ZERO));
		}
		else if (cKenner.equals(FSI_CONST.KON_FU_PLAN_MENGE))
		{
			return new FSI_SortObject(	MyNumberFormater.formatDez( bdPlan_oder_Geladen,0,true),
										bdPlan_oder_Geladen);
		}
		else if (cKenner.equals(FSI_CONST.KON_FU_LADE_MENGE))
		{
			return new FSI_SortObject(	MyNumberFormater.formatDez( bdGeladen,0,true),
										bdGeladen);
		}
		else if (cKenner.equals(FSI_CONST.KON_REST_MENGE))
		{
			return new FSI_SortObject(	MyNumberFormater.formatDez( bdRestMenge,0,true),
										bdRestMenge);
		}
		else if (cKenner.equals(FSI_CONST.KON_RECH_GUT))
		{
			return new FSI_SortObject(	MyNumberFormater.formatDez( bdBerechnet,0,true),
										bdBerechnet);
		}
		else if (cKenner.equals(FSI_CONST.KON_ANR1_2))
		{
			return new FSI_SortObject(this.get_ANR1_cUF_NN("")+"-"+this.get_ANR2_cUF_NN(""));
		}
		else if (cKenner.equals(FSI_CONST.KON_ARTBEZ1))
		{
			return new FSI_SortObject(this.get_ARTBEZ1_cF_NN("<Artbez1>"));
		}
		else if (cKenner.equals(FSI_CONST.KON_ARTBEZ2))
		{
			return new FSI_SortObject(this.get_ARTBEZ2_cF_NN("<Artbez2>"));
		}
		else if (cKenner.equals(FSI_CONST.KON_PREIS))
		{
			return new FSI_SortObject(	MyNumberFormater.formatDez( this.get_EINZELPREIS_FW_bdValue(BigDecimal.ZERO),2,true),

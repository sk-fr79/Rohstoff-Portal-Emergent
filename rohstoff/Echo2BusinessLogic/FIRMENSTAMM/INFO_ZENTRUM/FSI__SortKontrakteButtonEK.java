...
public class FSI__SortKontrakteButtonEK extends MyE2_Grid
{
	...
	private int                       			iActualSortStatus = FSI__SortKontrakteButtonEK.SORTSTATUS_UNSORTED;
	
	private String                              cSortKenner = null;


	private INFO_BLOCK_Kontrakte                ownInfoBlockKontrakte = null;
	 ...
	 * @throws myException
	 */
	public FSI__SortKontrakteButtonEK( String cText, String SortKenner, boolean bLeft, INFO_BLOCK_Kontrakte own_InfoBlockKontrakte) throws myException
	{
		...
		if (S.isFull(this.cSortKenner) && this.cSortKenner.equals(S.NN(this.ownInfoBlockKontrakte.get_ActualSortKennerEK())))
		{
			//dieser button muss in den status sort versetzt werden
			if (this.ownInfoBlockKontrakte.get_bActualSort_UP_EK())
			{
				this.set_StatusUP();
			}
			else
	...
	private void SetToNextSortStatus()
	{
		...
		if (iOldSortStatus==FSI__SortKontrakteButtonEK.SORTSTATUS_UNSORTED)
		{
			this.iActualSortStatus = FSI__SortKontrakteButtonEK.SORTSTATUS_UP;
		}
		else if (iOldSortStatus==FSI__SortKontrakteButtonEK.SORTSTATUS_UP)
		{
			this.iActualSortStatus = FSI__SortKontrakteButtonEK.SORTSTATUS_DOWN;
		}
		else
		{
			this.iActualSortStatus = FSI__SortKontrakteButtonEK.SORTSTATUS_UP;     //status UP springt nach DOWN, nicht nach unsorted
		}
	}
	...
	{
		return this.iActualSortStatus==FSI__SortKontrakteButtonEK.SORTSTATUS_UNSORTED;
	}
	
	public boolean get_bIsSortedUP()
	{
		return this.iActualSortStatus==FSI__SortKontrakteButtonEK.SORTSTATUS_UP;
	}
	
	
	public void Reset()
	{
		this.iActualSortStatus = FSI__SortKontrakteButtonEK.SORTSTATUS_UNSORTED;
		this.BaueSortButtonNeu();
	}
	...
	private class actionSortList extends XX_ActionAgent
	{
		...
		{
			FSI__SortKontrakteButtonEK oThis = FSI__SortKontrakteButtonEK.this;

			oThis.SetToNextSortStatus();

			//jetzt den status abspeichern
			FSI__SortKontrakteButtonEK_SaveStatus oSaver = new FSI__SortKontrakteButtonEK_SaveStatus();
			...
			oThis.ownInfoBlockKontrakte.set_ActualSortKennerEK(this.cSortKenner);
			oThis.ownInfoBlockKontrakte.set_bActualSort_UP_EK(oThis.get_bIsSortedUP());

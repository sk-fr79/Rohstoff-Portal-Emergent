...
public class INFO_BLOCK_SORTEN extends E2_SortGrid
{
	...
	private Vector<ownARTIKEL_BEZ>      vArtikelBez = new Vector<INFO_BLOCK_SORTEN.ownARTIKEL_BEZ>();
	...
	private MyE2_TextField_DatePOPUP_OWN   oDateBetrachtungVon = new MyE2_TextField_DatePOPUP_OWN("", 70, true, true);
	private MyE2_TextField_DatePOPUP_OWN   oDateBetrachtungBis = new MyE2_TextField_DatePOPUP_OWN("", 70, true, true);
	
	
	public INFO_BLOCK_SORTEN(FS__Adress_Info_Zentrum oInfoZentrum, boolean bAufbau, String EK_VK) throws myException 
	{
		super(INFO_BLOCK_SORTEN.arraySpaltenBreite, MyE2_Grid.STYLE_GRID_DDARK_BORDER_W100());
		...
		this.oDateBetrachtungVon.get_vActionAgentsZusatz().add(new actionSaveStatus_Suche_und_datum());
		this.oDateBetrachtungVon.get_vActionAgentsZusatz().add(new actionKompletterNeuAufbau());
		
		this.oDateBetrachtungBis.get_vActionAgentsZusatz().add(new actionSaveStatus_Suche_und_datum());
		this.oDateBetrachtungBis.get_vActionAgentsZusatz().add(new actionKompletterNeuAufbau());
		
		this.oDateBetrachtungVon.get_oButtonEraser().add_oActionAgent(new actionSaveStatus_Suche_und_datum());
		this.oDateBetrachtungVon.get_oButtonEraser().add_oActionAgent(new actionKompletterNeuAufbau());
		
		this.oDateBetrachtungBis.get_oButtonEraser().add_oActionAgent(new actionSaveStatus_Suche_und_datum());
		this.oDateBetrachtungBis.get_oButtonEraser().add_oActionAgent(new actionKompletterNeuAufbau());
	...
	{
		return INFO_BLOCK_SORTEN.iAnzahlSpalten;
	}
	...
	public Component get_TitelComponent(int iSpalte) throws myException
	{
		E2_SortButtonInList oSortButton = new E2_SortButtonInList(
												iSpalte, 
												this, 
												new MyE2_String(INFO_BLOCK_SORTEN.arraySpaltenTitel[iSpalte]), 
												null, 
												null, 
												E2_SortButtonInList.SORT_BUTTON_STYLE_SMALL(),
												FS__Adress_Info_Zentrum.STORE_MODUL_KENNER);
		
		if (INFO_BLOCK_SORTEN.arraySpaltenTitel[iSpalte].length()<3)         //die sonder-buttons sind nicht sortierbar
		{
		...
		oSortButton.setLayoutData(new FSI___GridLayoutList(INFO_BLOCK_SORTEN.arrAusrichtung_left_center_right_012[iSpalte],new E2_ColorDDDark()));
	...
	{
		return INFO_BLOCK_SORTEN.SAVE_SORT_INFOBLOCK_SORTEN+this.cEK_VK;
	}
	...
	private class ownListenZeile extends E2_SortGridListenZeile
	{
		...
		{
			INFO_BLOCK_SORTEN oThis = INFO_BLOCK_SORTEN.this;
			
			Component[] arrComp = new Component[INFO_BLOCK_SORTEN.iAnzahlSpalten];
			
			arrComp[0]=new FSI_Label(this.REC_ART_BEZ.get_UP_RECORD_ARTIKEL_id_artikel().get_ANR1_cUF_NN("<anr1>")+"-"+this.REC_ART_BEZ.get_ANR2_cUF_NN("<anr2>"),
														INFO_BLOCK_SORTEN.arrAusrichtung_left_center_right_012[0]);
			arrComp[1]=new FSI_Label(this.REC_ART_BEZ.get_ARTBEZ1_cUF_NN("<artbez1>"),INFO_BLOCK_SORTEN.arrAusrichtung_left_center_right_012[1]);
			arrComp[2]=new FSI_Label(this.REC_ART_BEZ.ARTBEZLIEF.get_ARTBEZ2_ALTERNATIV_cF_NN("<artbez2_alternativ>"),INFO_BLOCK_SORTEN.arrAusrichtung_left_center_right_012[2]);
			...
			arrComp[3]=new FSI_Label(recEAK==null?"<eak-code>":recEAK.get_AVV_Code_Gesamt(),INFO_BLOCK_SORTEN.arrAusrichtung_left_center_right_012[3]);

			arrComp[4]=new FSI_Label(this.REC_ART_BEZ.ARTBEZLIEF.is_ANGEBOT_YES()?"X":"",INFO_BLOCK_SORTEN.arrAusrichtung_left_center_right_012[4]);
			arrComp[5]=new FSI_Label(this.REC_ART_BEZ.get_BEMERKUNG_INTERN_cUF_NN("<bemerkung>"),INFO_BLOCK_SORTEN.arrAusrichtung_left_center_right_012[5]);
			
			arrComp[6]=new FSI_Label(MyNumberFormater.formatDez(this.REC_ART_BEZ.bdMengenSumme,0,true),INFO_BLOCK_SORTEN.arrAusrichtung_left_center_right_012[6]);
			((FSI_Label)arrComp[6]).setToolTipText(new MyE2_String("Abgerechnete Menge dieser Sorten-Bezeichnung im angegebenen Betrachtungszeitraum").CTrans());
			
		
			arrComp[7] = new FS__Adress_Info_Zentrum_JUMP_ADRESSE_NAVI_POPUP_TO_FUHRE(	
			...
			{
				((FS__Adress_Info_Zentrum_JUMP_ADRESSE_NAVI_POPUP_TO_FUHRE)arrComp[7]).set_bEnabled_For_Edit(false);
			}
				
			
			this.ownArrayComp = arrComp;
			
			new FSI_HighLighter(oThis.oTF_Suche, this.ownArrayComp);
	...
	private class actionNeueAnzeigeDerListe extends XX_ActionAgent
	{
		...
		{
			INFO_BLOCK_SORTEN.this.BaueListeAuf();
		}
	}
	...
	private class actionKompletterNeuAufbau extends XX_ActionAgent
	{
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException 
		{
			INFO_BLOCK_SORTEN.this.__query_und_baue_auf();
		}
	}
	...
	private class E2_UserSettings_SAVE_SUCHFELD_UND_DATUMSBEREICH extends XXX_UserSetting 
	{
		...
		{
			return this.cSessionHash+INFO_BLOCK_SORTEN.this.cEK_VK;
		}
	...
	private class actionSaveStatus_Suche_und_datum extends XX_ActionAgent
	{
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException
		{
			new E2_UserSettings_SAVE_SUCHFELD_UND_DATUMSBEREICH().STORE(FS__Adress_Info_Zentrum.STORE_MODUL_KENNER, INFO_BLOCK_SORTEN.this.get_Status_Suche_und_Datum());
		}
	}
	...
	public String get_Status_Suche_und_Datum() throws myException
	{
		...
		cRueck += "|";
		cRueck += (S.NN(this.oDateBetrachtungVon.get_oFormatedDateFromTextField()));
		cRueck += "|";
		cRueck += (S.NN(this.oDateBetrachtungBis.get_oFormatedDateFromTextField()));
	...
	private void restore_status_Suche_und_Datum() throws myException
	{
		...
		String cDatabaseSetting = (String)new E2_UserSettings_SAVE_SUCHFELD_UND_DATUMSBEREICH().get_Settings(FS__Adress_Info_Zentrum.STORE_MODUL_KENNER);
		...
		if (cDatabaseSetting!=null)
		{
			...
			if (vWerte.size()>1) this.oDateBetrachtungVon.get_oTextField().setText(vWerte.get(1));
			if (vWerte.size()>2) this.oDateBetrachtungBis.get_oTextField().setText(vWerte.get(2));
		}
	}
	...
	private class ownARTIKEL_BEZ extends RECORD_ARTIKEL_BEZ
	{
		...
		private BigDecimal   get_bdMengeEK_VK() throws myException
		{
			...
			MyDate oDatumStart = oThis.oDateBetrachtungVon.get_oDateFromTextField_even_when_not_OK();
			MyDate oDatumEnde  = oThis.oDateBetrachtungBis.get_oDateFromTextField_even_when_not_OK();
			...
			if (oDatumEnde.get_bOK())
			{
			 ...
			 String cQuery = "SELECT NVL(SUM(NVL(VP.ANZAHL,0)-NVL(VP.ANZAHL_ABZUG_LAGER,0)),0) AS NETTOMENGE " +
			 					...
			 					" AND   LAGER_VORZEICHEN="+(INFO_BLOCK_SORTEN.this.cEK_VK.equals("EK")?"1":"-1");
		...
		private Vector<String>   get_vIDs_Fuhren() throws myException
		{
			...
			String cADRESSFELD=(INFO_BLOCK_SORTEN.this.cEK_VK.equals("EK")?"FU.ID_ADRESSE_START":"FU.ID_ADRESSE_ZIEL");
			
			String cARTIKELBEZ_FELD = (INFO_BLOCK_SORTEN.this.cEK_VK.equals("EK")?"FU.ID_ARTIKEL_BEZ_EK":"FU.ID_ARTIKEL_BEZ_VK");
			
			String cLADEDAT_FELD=		(INFO_BLOCK_SORTEN.this.cEK_VK.equals("EK")?"NVL(FU.DATUM_AUFLADEN,FU.DATUM_ABHOLUNG)":"NVL(FU.DATUM_ABLADEN,FU.DATUM_ANLIEFERUNG)");
			String cLADEDAT_FELD_FUO=	(INFO_BLOCK_SORTEN.this.cEK_VK.equals("EK")?"NVL(FUO.DATUM_LADE_ABLADE,FU2.DATUM_ABHOLUNG)":"NVL(FUO.DATUM_LADE_ABLADE,FU2.DATUM_ANLIEFERUNG)");
			...
			String cQuery = "SELECT FU.ID_VPOS_TPA_FUHRE FROM " +bibE2.cTO()+".JT_VPOS_TPA_FUHRE FU "+
			 				...
			 				" SELECT FUO.ID_VPOS_TPA_FUHRE_ORT FROM " +bibE2.cTO()+".JT_VPOS_TPA_FUHRE_ORT FUO " +
			 				    ...
			 				    " AND   FUO.DEF_QUELLE_ZIEL="+bibALL.MakeSql(INFO_BLOCK_SORTEN.this.cEK_VK);

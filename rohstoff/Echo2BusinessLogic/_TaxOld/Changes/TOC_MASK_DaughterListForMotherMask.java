 ...
 */
public class TOC_MASK_DaughterListForMotherMask extends RB_MaskDaughterBasedOnBasicModulContainer {
 	
 	//fieldKey fuer das einbinden dieser komponente in einer mother-mask
	public static RB_KF  keyForMotherMask = (RB_KF)new RB_KF()._setHASHKEY("TOC_MWSTSCHLUESSEL_OLD_AENDERUNGEN_MASK_DaughterListForMotherMask")._setREALNAME("TOC_MWSTSCHLUESSEL_OLD_AENDERUNGEN_MASK_DaughterListForMotherMask");
    ...
    private TOC_LIST_BedienPanel 	bedienPanel = null;
     ...
     * @throws myException
     */
    public TOC_MASK_DaughterListForMotherMask(RB_TransportHashMap p_tpHashMap, boolean useCompactList) throws myException {
        super();
	...
	public IF_Field getLinkfieldToMotherTable() {
		return MWSTSCHLUESSEL_AENDERUNGEN.id_mwstschluessel;
	}
	...
	public class InternalLIST_BasicModuleContainer extends Project_BasicModuleContainer    {
	    ...
	    public InternalLIST_BasicModuleContainer(RB_TransportHashMap p_params, boolean useCompactList) throws myException    {
	        ...
	        super(E2_MODULNAME_ENUM.MODUL.MWSTSCHLUESSEL_OLD_AENDERUNGEN_LIST.get_callKey());
	        
	        TOC_MASK_DaughterListForMotherMask oThis = TOC_MASK_DaughterListForMotherMask.this;
	        ...
	        oThis.m_tpHashMap._setLeadingMaskKey(TOC_CONST.getLeadingMaskKey());
	        oThis.m_tpHashMap._setLeadingTableOnMask(TOC_CONST.getLeadingTable());
 
	        this.set_bVisible_Row_For_Messages(false);
 	        
 	        
	        TOC_LIST_ComponentMap listComponentMap  = null;
 	        
	        if (useCompactList) {
                oThis.m_tpHashMap.put(RB_TransportHashMap_ENUM.NAVILIST,new E2_NavigationListCompact());
 
                 
                TOC_LIST_bt_New btNew = new TOC_LIST_bt_New(oThis.m_tpHashMap);
                TOC_LIST_bt_Copy btCopy = new TOC_LIST_bt_Copy(oThis.m_tpHashMap);
                ...
                listComponentMap = new TOC_LIST_ComponentMap(oThis.m_tpHashMap);
                ...
                oThis.bedienPanel = new TOC_LIST_BedienPanel(TOC_MASK_DaughterListForMotherMask.this.m_tpHashMap);
		...
		public void fillComponents(E2_ComponentMAP oMAP, SQLResultMAP oUsedResultMAP) throws myException {
			if (!m_enabled) {
				...
				for (MyE2IF__Component c: oMAP.get_hmRealComponents().values()) {
					if (c instanceof TOC_LIST_bt_DeleteInListRow) {
						((TOC_LIST_bt_DeleteInListRow)c).setEnabledForEditOfSuperClass(false);
					}
					if (c instanceof TOC_LIST_bt_ListToMaskInListRow) {
						if ( ((TOC_LIST_bt_ListToMaskInListRow)c).isUsedToEdit()) {
							((TOC_LIST_bt_ListToMaskInListRow)c).setEnabledForEditOfSuperClass(false);
						}
					}
	...
	/* (non-Javadoc)
	 * @see panter.gmbh.Echo2.RB.COMP.MaskDaughter.RB_MaskDaughterBasedOnBasicModulContainer#getSearcher()
	 */
	@Override

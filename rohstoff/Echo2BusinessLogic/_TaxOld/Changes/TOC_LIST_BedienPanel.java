...
public class TOC_LIST_BedienPanel extends E2_Grid {
    
    private TOC_LIST_Selector  MWSTSCHLUESSEL_OLD_AENDERUNGEN_LIST_Selector = null;
    ...
    public TOC_LIST_BedienPanel(RB_TransportHashMap  p_params) throws myException     {
        super();
        this.setSize(2);
        ...
        this.MWSTSCHLUESSEL_OLD_AENDERUNGEN_LIST_Selector = new TOC_LIST_Selector(tpHashMap);
        this._a(MWSTSCHLUESSEL_OLD_AENDERUNGEN_LIST_Selector,   	new RB_gld()._ins(0,2,10,2)._span(2)._left_mid());
        ...
        grid4Components._a((Component)this.toDisableAtMask._ar(new TOC_LIST_bt_ListToMask(true,tpHashMap)),   new RB_gld()._ins(2,2,10,2)._left_mid());
        grid4Components._a(new TOC_LIST_bt_ListToMask(false,tpHashMap),   new RB_gld()._ins(2,2,10,2)._left_mid());
        
        grid4Components._a((Component)this.toDisableAtMask._ar(new TOC_LIST_bt_New(tpHashMap)),  		new RB_gld()._ins(2,2,10,2)._left_mid());
        grid4Components._a((Component)this.toDisableAtMask._ar(new TOC_LIST_bt_Copy(tpHashMap)),   	new RB_gld()._ins(2,2,10,2)._left_mid());
        grid4Components._a((Component)this.toDisableAtMask._ar(new TOC_LIST_bt_multiDelete(tpHashMap)),new RB_gld()._ins(2,2,10,2)._left_mid());
         
        //benutzerdefinierte Reports anzeigen  
        grid4Components._a((Component)new REP__POPUP_Button(E2_MODULNAME_ENUM.MODUL.MWSTSCHLUESSEL_OLD_AENDERUNGEN_LIST.get_callKey(),this.tpHashMap.getNavigationList()),new RB_gld()._ins(2,2,10,2)._left_mid());
        ...
        uploadBt.add_GlobalValidator(TOC_VALIDATORS.ATTACHMENT.getValidator());
    ...
    public TOC_LIST_Selector get_list_Selector() 
    {
        return MWSTSCHLUESSEL_OLD_AENDERUNGEN_LIST_Selector;
    }

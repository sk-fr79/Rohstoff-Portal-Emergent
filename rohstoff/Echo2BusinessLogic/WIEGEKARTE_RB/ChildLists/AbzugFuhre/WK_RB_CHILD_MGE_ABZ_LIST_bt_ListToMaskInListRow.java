...
public class WK_RB_CHILD_MGE_ABZ_LIST_bt_ListToMaskInListRow extends RB_BtV4_List2Mask {
    
    public WK_RB_CHILD_MGE_ABZ_LIST_bt_ListToMaskInListRow(boolean bEdit, RB_TransportHashMap  p_tpHashMap) {
        super(bEdit);
        this._setTransportHashMap(p_tpHashMap);
        this._setShowSaveAndReOpenButton(false);

        this.add_GlobalValidator(bEdit?WK_RB_CHILD_MGE_ABZ_VALIDATORS.EDIT.getValidator():WK_RB_CHILD_MGE_ABZ_VALIDATORS.EDIT.getValidator());
        
    }
    @Override
    public RB_ModuleContainerMASK generateMaskContainer() throws myException {
    	this.getTransportHashMap()._setRBModulContainerMask(
    			new WK_RB_CHILD_MGE_ABZ_MASK_MaskModulContainer(this.getTransportHashMap()));
    ...
    public RB_hm_multi_DataobjectsCollector generateDataObjects4Mask(MyE2_MessageVector mv_sammler) throws myException {
    	...
    	WK_RB_CHILD_MGE_ABZ_LIST_bt_ListToMaskInListRow oThis = WK_RB_CHILD_MGE_ABZ_LIST_bt_ListToMaskInListRow.this;
		...
		if (this.EXT().get_oComponentMAP()!=null) {
			...
			if (id.isOK()) {
	            ...
	            collector.put(id.get_cUF_LongString(), new WK_RB_CHILD_MGE_ABZ_MASK_DataObjectCollector(oThis.getTransportHashMap(), id.get_cUF_LongString(),aktuellerStatus));
	            return collector;
			} else {
    ...
    public MyE2_String generateTitelInfo4MaskWindow() throws myException {
       	MyE2_String bearbeiten = S.ms("Bearbeiten eines Datensatzes vom Typ: ").ut(WK_RB_CHILD_MGE_ABZ_CONST.WK_RB_CHILD_MGE_ABZ_NAMES.DATASET_NAME.getUserText().CTrans());
    	MyE2_String anzeigen  = S.ms("Anzeige eines Datensatzes vom Typ: ").ut(WK_RB_CHILD_MGE_ABZ_CONST.WK_RB_CHILD_MGE_ABZ_NAMES.DATASET_NAME.getUserText().CTrans());
    ...
    public MyE2_String generateMessagetextForSaveRecord() throws myException {
         return S.ms("Datensatzes vom Typ: ").ut(WK_RB_CHILD_MGE_ABZ_CONST.WK_RB_CHILD_MGE_ABZ_NAMES.DATASET_NAME.getUserText().CTrans()).t(" wurde gespeichert");
    }
	...
	public Object get_Copy(Object objHelp) throws myExceptionCopy {
		WK_RB_CHILD_MGE_ABZ_LIST_bt_ListToMaskInListRow copy= new WK_RB_CHILD_MGE_ABZ_LIST_bt_ListToMaskInListRow(this.isUsedToEdit(), this.getTransportHashMap());

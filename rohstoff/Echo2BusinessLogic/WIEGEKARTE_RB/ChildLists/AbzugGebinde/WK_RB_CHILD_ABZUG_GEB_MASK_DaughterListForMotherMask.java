...
import rohstoff.Echo2BusinessLogic.WIEGEKARTE_RB.WK_RB_MASK_DataObjectCollector;
 
/**
 * @author martin
 * Komponente fuer eine maske eine mothertable, die diese als fullDaughter einblendet
 */
public class WK_RB_CHILD_ABZUG_GEB_MASK_DaughterListForMotherMask extends RB_MaskDaughterBasedOnBasicModulContainer {
 	
 	//fieldKey fuer das einbinden dieser komponente in einer mother-mask
	public static RB_KF  keyForMotherMask = (RB_KF)new RB_KF()._setHASHKEY("WK_RB_CHILD__WIEGEKARTE_ABZUG_GEB_MASK_DaughterListForMotherMask")._setREALNAME("WK_RB_CHILD__WIEGEKARTE_ABZUG_GEB_MASK_DaughterListForMotherMask");
    ...
    private WK_RB_CHILD_ABZUG_GEB_LIST_BedienPanel 	bedienPanel = null;
    private VEK<MyE2IF__Component>  toDisableAtMask = new VEK<MyE2IF__Component>();
    private WK_RB_CHILD_ABZUG_GEB_LIST_DATASEARCH  	searcher = null;
     ...
     * @throws myException
     */
    public WK_RB_CHILD_ABZUG_GEB_MASK_DaughterListForMotherMask(RB_TransportHashMap p_tpHashMap, boolean useCompactList) throws myException {
        super();
    ...
    /* (non-Javadoc)
     * @see panter.gmbh.Echo2.RB.COMP.MaskDaughter.RB_MaskDaughterBasedOnBasicModulContainer#rb_Datacontent_to_Component(panter.gmbh.Echo2.RB.DATA.RB_Dataobject)
     */
    @Override
	...
	public IF_Field getLinkfieldToMotherTable() {
		return WIEGEKARTE_ABZUG_GEB.id_wiegekarte;
	...
	public class InternalLIST_BasicModuleContainer extends Project_BasicModuleContainer    {
	    ...
	    public InternalLIST_BasicModuleContainer(RB_TransportHashMap p_params, boolean useCompactList) throws myException    {
	        ...
	        WK_RB_CHILD_ABZUG_GEB_MASK_DaughterListForMotherMask oThis = WK_RB_CHILD_ABZUG_GEB_MASK_DaughterListForMotherMask.this;
	        ...
	        oThis.m_tpHashMap._setLeadingMaskKey(WK_RB_CHILD_ABZUG_GEB_CONST.getLeadingMaskKey());
	        oThis.m_tpHashMap._setLeadingTableOnMask(WK_RB_CHILD_ABZUG_GEB_CONST.getLeadingTable());
 
	        this.set_bVisible_Row_For_Messages(false);
 	        
 	        
	        WK_RB_CHILD_ABZUG_GEB_LIST_ComponentMap listComponentMap  = null;
 	        
	        if (useCompactList) {
                oThis.m_tpHashMap.put(RB_TransportHashMap_ENUM.NAVILIST,new E2_NavigationListCompact());
 
                 
                WK_RB_CHILD_ABZUG_GEB_LIST_bt_New btNew = new WK_RB_CHILD_ABZUG_GEB_LIST_bt_New(oThis.m_tpHashMap);
                WK_RB_CHILD_ABZUG_GEB_LIST_bt_Copy btCopy = new WK_RB_CHILD_ABZUG_GEB_LIST_bt_Copy(oThis.m_tpHashMap);
...
//                oThis.searcher = new WK_RB_CHILD__LIST_DATASEARCH(oThis.m_tpHashMap);
...
//                oThis.m_tpHashMap.getNaviListCompact()._addRightComponent(oThis.searcher._setLDC(new RB_gld()._ins(20, 0, 10, 0)._left_mid()));
                ...
                listComponentMap = new WK_RB_CHILD_ABZUG_GEB_LIST_ComponentMap(oThis.m_tpHashMap);
                ...
                oThis.bedienPanel = new WK_RB_CHILD_ABZUG_GEB_LIST_BedienPanel(WK_RB_CHILD_ABZUG_GEB_MASK_DaughterListForMotherMask.this.m_tpHashMap);
    ...
    private class mapSettingAgent extends XX_ComponentMAP_SubqueryAGENT {
		...
		public void PrepareComponents_For_NEW(E2_ComponentMAP oMAP) throws myException {
			oMAP.set_bEnabled_For_Edit_ALL(false);
		...
		public void fillComponents(E2_ComponentMAP oMAP, SQLResultMAP oUsedResultMAP) throws myException {
			if (!m_enabled) {
				...
				for (MyE2IF__Component c: oMAP.get_hmRealComponents().values()) {
					if (c instanceof WK_RB_CHILD_ABZUG_GEB_LIST_bt_DeleteInListRow) {
						((WK_RB_CHILD_ABZUG_GEB_LIST_bt_DeleteInListRow)c).setEnabledForEditOfSuperClass(m_enabled);
					}
					if (c instanceof WK_RB_CHILD_ABZUG_GEB_LIST_bt_ListToMaskInListRow) {
						if ( ((WK_RB_CHILD_ABZUG_GEB_LIST_bt_ListToMaskInListRow)c).isUsedToEdit()) {
							((WK_RB_CHILD_ABZUG_GEB_LIST_bt_ListToMaskInListRow)c).setEnabledForEditOfSuperClass(m_enabled);
						}
					}
	...
	private class oActionSaveAndResumeEdit extends RB_actionStandardSaveAndReopen_Rec20 {
		...
		public oActionSaveAndResumeEdit() throws myException {
			super();
			this._set_leading_table_on_mask(_TAB.wiegekarte)._set_mask_key_main(new RB_KM(_TAB.wiegekarte));
		}
		...
		public RB_DataobjectsCollector generate_dataObjectsCollector_4_edit(
			...
			return new WK_RB_MASK_DataObjectCollector(m_tpHashMap, id_record, RB__CONST.MASK_STATUS.EDIT);		
		}

		@Override
		public RB_ComponentMapCollector get_componentMapCollector() throws myException {
			WK_RB_CHILD_ABZUG_GEB_MASK_DaughterListForMotherMask oThis = WK_RB_CHILD_ABZUG_GEB_MASK_DaughterListForMotherMask.this;
 			return oThis.rb_ComponentMap_this_belongsTo().rb_get_belongs_to(); 
		}

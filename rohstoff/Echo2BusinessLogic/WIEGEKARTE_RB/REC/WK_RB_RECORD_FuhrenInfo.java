...
public class WK_RB_RECORD_FuhrenInfo {
	// die Lageradresse, die in der Maske gesetzt wurde(ohne die man keine Wiegekarte anlegen kann)
	...
	/**
	 * prüft, ob das Lager / der Lieferant oder der Empfänger in der Liste der gültigen Lager steht, 
	 * falls für ein fremdes Lager verwogen werden soll. (Ausnahmeprüfung) 
	 * @author manfred
	 * @date 05.08.2020
	 *
	 * @return
	 */
	public boolean isLagerInListe() {
		boolean bRet = true;
		
		boolean bLief 		= getIST_LIEFERANT().equalsIgnoreCase("Y");
		boolean bIstStrecke = getIST_STRECKE().equalsIgnoreCase("Y");
		
		if (bLief) {
			if (!getLagerListe().contains(getID_ADRESSE_ZIEL()) && !bIstStrecke ) {
				bRet = false;
			}
		} else {
			if (!getLagerListe().contains(getID_ADRESSE_START())) {
				bRet = false;
			}
		}
		return bRet;
	...
	/**
	 * prüft, ob beide Lagerorte (Start und Ziel) WaageLager  sind
	 * @author manfred
	 * @date 13.07.2021
	 *
	 * @return
	 */
	public boolean isWaageLagerOnly() {
		return ( getLagerListe().contains(getID_ADRESSE_START()) && getLagerListe().contains(getID_ADRESSE_ZIEL()) );
	}

	
	
	/**
	 * die Adresse des Lagers bei dem Verwogen wird
	 * @author manfred
	 * @date 13.07.2021
	 *
	 * @return
	 */
	public String getID_ADRESSE_LAGER_MASK() {
	 ...
	 */
	public void setID_ADRESSE_LAGER_MASK(String IDAdresseLager) {

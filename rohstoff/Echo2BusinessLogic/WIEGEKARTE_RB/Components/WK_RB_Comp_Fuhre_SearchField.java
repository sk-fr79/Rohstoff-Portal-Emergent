...
public class WK_RB_Comp_Fuhre_SearchField extends MyE2_MaskSearchField
{
	private XX_ActionAgent ActionAfterThisActionFound = null;
	private XX_ActionAgent ActionAfterThisActionDeleted = null;	
	
	private RB_TextField  oTF_Fuhre = null;
	private RB_TextField  oTF_Fuhre_ort = null;
	
	private RB_TransportHashMap  _tpHashMap = null;
	

	public WK_RB_Comp_Fuhre_SearchField(
			RB_TransportHashMap   p_tpHashMap,
			RB_TextField  TF_Fuhre,
			RB_TextField  TF_Fuhre_ort, 
			String idLager, 
...
//				  " FROM "+bibE2.cTO()+".V"+bibALL.get_RECORD_MANDANT().get_ID_MANDANT_cUF()+"_FUHREN WHERE TO_CHAR(id_vpos_tpa_fuhre) ||'-'|| to_CHAR(id_vpos_tpa_fuhre_ort) = '#WERT#'",
				  " FROM "+bibE2.cTO()+".V"+bibALL.get_RECORD_MANDANT().get_ID_MANDANT_cUF()+"_FUHREN WHERE "
				  		+ " ID_VPOS_TPA_FUHRE =substr('#WERT#',0,instr('#WERT#','-')-1) "
				  	    + " AND nvl(ID_VPOS_TPA_FUHRE_ORT,-1) = nvl(substr('#WERT#',instr('#WERT#','-')+1),-1)"
				  ,
	...
	private class ownActionAfterFound extends XX_MaskActionAfterFoundNonDB
	{
		...
		public void doMaskSettingsAfterValueWrittenInMaskField(String maskValue, MyE2_MaskSearchField searchField, boolean afterAction) throws myException
		{
			...
			WK_RB_Comp_Fuhre_SearchField oThis = WK_RB_Comp_Fuhre_SearchField.this;

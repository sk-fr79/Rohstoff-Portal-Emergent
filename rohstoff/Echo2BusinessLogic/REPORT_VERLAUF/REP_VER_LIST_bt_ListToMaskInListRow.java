...
public class REP_VER_LIST_bt_ListToMaskInListRow extends RB_BtV4_List2Mask {
    
    public REP_VER_LIST_bt_ListToMaskInListRow(boolean bEdit, RB_TransportHashMap  p_tpHashMap) {
        super(bEdit);
        this._setTransportHashMap(p_tpHashMap);
   
        this.add_GlobalValidator(REP_VER_VALIDATORS.VIEW.getValidator());
        
        this._ttt(S.ms("Report anzeigen"));
        
    }
    @Override
    public RB_ModuleContainerMASK generateMaskContainer() throws myException {
    	this.getTransportHashMap()._setRBModulContainerMask(
    			new REP_VER_MASK_MaskModulContainer(this.getTransportHashMap()));
    ...
    public RB_hm_multi_DataobjectsCollector generateDataObjects4Mask(MyE2_MessageVector mv_sammler) throws myException {
    	...
    	REP_VER_LIST_bt_ListToMaskInListRow oThis = REP_VER_LIST_bt_ListToMaskInListRow.this;
		...
		if (this.EXT().get_oComponentMAP()!=null) {
			...
			if (id.isOK()) {
	            ...
	            collector.put(id.get_cUF_LongString(), new REP_VER_MASK_DataObjectCollector(oThis.getTransportHashMap(), id.get_cUF_LongString(),aktuellerStatus));
	            return collector;
			} else {
    ...
    public MyE2_String generateTitelInfo4MaskWindow() throws myException {
        return new MyE2_String(this.isUsedToEdit()?"Bearbeiten eines Datensatzes vom Typ: Reportverlauf":"Anzeige eines Datensatzes vom Typ: Reportverlauf");
    }
    
    @Override
    public MyE2_String generateMessagetextForSaveRecord() throws myException {
        return new MyE2_String("Datensatzes vom Typ: Reportverlauf wurde gespeichert");
    }
	...
	public Object get_Copy(Object objHelp) throws myExceptionCopy {
		REP_VER_LIST_bt_ListToMaskInListRow copy= new REP_VER_LIST_bt_ListToMaskInListRow(this.isUsedToEdit(), this.getTransportHashMap());

...
public class B_LIST_SQLFIELD_NAME_1ST_ATOM_1ST_VEKTOR extends SQLField {

	public B_LIST_SQLFIELD_NAME_1ST_ATOM_1ST_VEKTOR() throws myException {
		super("(SELECT NVL(S.ORT,AD.NAME1) FROM JT_BEWEGUNG_VEKTOR V "+ 
				...
				"               (SELECT MIN(VV.ID_BEWEGUNG_VEKTOR) FROM JT_BEWEGUNG_VEKTOR VV WHERE NVL(VV.DELETED,'N')='N' AND  VV.ID_BEWEGUNG=V.ID_BEWEGUNG)"+
				" AND A.ID_BEWEGUNG_ATOM =  " +
				"               (SELECT MIN(AA.ID_BEWEGUNG_ATOM)    FROM JT_BEWEGUNG_ATOM AA      WHERE NVL(AA.DELETED,'N')='N' AND AA.ID_BEWEGUNG_VEKTOR=V.ID_BEWEGUNG_VEKTOR)"+
                " AND S.MENGENVORZEICHEN= -1 "+
                " AND NVL(A.DELETED,'N')='N' "+
                " AND NVL(V.DELETED,'N')='N' "+
                " AND  V.ID_BEWEGUNG = JT_BEWEGUNG.ID_BEWEGUNG )", 
                B__CONST.COLKEY_AUSGANGSORT, new MyE2_String("1. Ladeort") , bibE2.get_CurrSession());
	}

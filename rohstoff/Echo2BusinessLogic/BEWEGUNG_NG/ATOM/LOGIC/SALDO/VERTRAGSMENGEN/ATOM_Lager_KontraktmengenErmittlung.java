...
public class ATOM_Lager_KontraktmengenErmittlung {

	private HashMap<String, ATOM_Lager_KontraktmengenDaten> m_Lagerdaten = null;
	...
	public ATOM_Lager_KontraktmengenErmittlung() {
	...
	public void ReadVertragsLagerDaten(String sIDLager, String sIDHauptsorte, String sIDSorte ) throws myException{
		...
		String sSqlEK = 
			...
			" SUM (L.LAGERMENGE) - " + 
		    "    NVL( " +
		    "	 (SELECT SUM( A1.MENGE - nvl(A1.ABZUG_MENGE,0) ) AS SUM_ABLADEMENGE " +
		    "	 FROM  " +   m_TableOwner + ".JT_BEWEGUNG_ATOM A1 " + 
		    "	 INNER JOIN  " +   m_TableOwner + ".JT_BEWEGUNG_STATION S1 ON (A1.ID_BEWEGUNG_ATOM = S1.ID_BEWEGUNG_ATOM) " +
		    "	 LEFT OUTER JOIN  " +   m_TableOwner + ".JT_VPOS_KON K1 ON  (K1.ID_VPOS_KON = A1.ID_VPOS_KON)  " +
		    ...
		    "	 WHERE NVL(A1.DELETED,'N') = 'N' " +
		    "	 AND S1.MENGENVORZEICHEN = 1 " +
			"	 AND NVL(A1.STORNIERT,'N') = 'N' " +
		    "	 AND NVL(TRAKT1.ABGESCHLOSSEN,'N') = 'N' " +
		    "	 AND S1.ID_ADRESSE = L1.ID_ADRESSE_LAGER " +
		    "	 AND S1.ID_ADRESSE = L.ID_ADRESSE_LAGER " +
...
String sSqlVK = 
		...
		for (int i = 0; i < cLagerDaten.length; i++) {
			...
			ATOM_Lager_KontraktmengenDaten oDaten  = null;

			// Datenobjekt holen bzw. erzeugen
			if (m_Lagerdaten.containsKey(sKey)){
				oDaten = m_Lagerdaten.get(sKey);
			} else {
				oDaten = new ATOM_Lager_KontraktmengenDaten(Lager,Sorte);
	...
	public void ClearData(){
		...
		m_Lagerdaten = new HashMap<String, ATOM_Lager_KontraktmengenDaten>();
	}
	 ...
	 * @return
	 */
	public ATOM_Lager_KontraktmengenDaten getData(String idLager, String idArtikel) {

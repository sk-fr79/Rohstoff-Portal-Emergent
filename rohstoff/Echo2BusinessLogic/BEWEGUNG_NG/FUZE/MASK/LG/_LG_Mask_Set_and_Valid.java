...
public class _LG_Mask_Set_and_Valid extends RB_Mask_Set_And_Valid {

	private __LG_MASTER_KEY masterKey = null;
	...
	public MyE2_MessageVector make_Interactive_Set_and_Valid(RB_ComponentMap rbMASK, VALID_TYPE ActionType, ExecINFO oExecInfo) throws myException {
		//zuerst den LG_CM__Collector beschaffen
		
		MyE2_MessageVector mv = new MyE2_MessageVector();
		
		CM_Bewegung  cm_bewegung = (CM_Bewegung) rbMASK;
		
		LG_CM__Collector cm_collector = (LG_CM__Collector)cm_bewegung.rb_get_belongs_to();
		...
		if (ActionType==VALID_TYPE.USE_IN_MASK_VALID_ACTION_BEFORE_FIELDVALIDATION) {
			...
			MyLong l_ziel_lager_start = new MyLong(cm_collector.get_zielStationRight().getRbComponentSavable(BEWEGUNG_STATION.id_adresse).rb_readValue_4_dataobject());
			if(l_ziel_lager_start.get_bOK()){
				RECORD_ADRESSE ra = new RECORD_ADRESSE(l_ziel_lager_start.get_lValue());
				cm_collector.get_zielStationRight().getRbComponentSavable(BEWEGUNG_STATION.id_adresse).rb_set_db_value_manual(ra.ufs(ADRESSE.id_adresse));
			}
			
			new _LG_Controller_Leistungsdatum(cm_collector).fill_datum(mv);
			
			new _LG_Controller_Menge(cm_collector).fill_menge(mv);
		}
		...
		if(ActionType == VALID_TYPE.USE_IN_MASK_VALID_ACTION){
			...
			String id_artikel_bez		= 	rbMASK._find_component_in_neighborhood(this.masterKey.k_atom_left(), 					BEWEGUNG_ATOM.id_artikel_bez.fk()).get__actual_maskstring_in_view();
			...
			String virtual_lager_id 	= 	FZ_SONDERLAGER_ENUM.LG_ZWISCHENLAGER.get_zwischenlager_id();
			id_artikel 					= 	bibALL.convertID2UnformattedID(id_artikel);
			id_artikel_bez 				= 	bibALL.convertID2UnformattedID(id_artikel_bez);
			...
			//atom links
			rbMASK._find_component_in_neighborhood(this.masterKey.k_atom_left(), 				BEWEGUNG_ATOM.id_artikel.fk()				).rb_set_db_value_manual(id_artikel);
			rbMASK._find_component_in_neighborhood(this.masterKey.k_atom_left(), 				BEWEGUNG_ATOM.id_artikel_bez.fk()			).rb_set_db_value_manual(id_artikel_bez);
			...
			//atom recht
//			rbMASK._find_component_in_neighborhood(this.masterKey.k_atom_ziel(),				BEWEGUNG_ATOM.leistungsdatum.fk()				).rb_set_db_value_manual(datum);
			rbMASK._find_component_in_neighborhood(this.masterKey.k_atom_right(),				BEWEGUNG_ATOM.id_artikel.fk()				).rb_set_db_value_manual(id_artikel);
			rbMASK._find_component_in_neighborhood(this.masterKey.k_atom_right(),				BEWEGUNG_ATOM.id_artikel_bez.fk()			).rb_set_db_value_manual(id_artikel_bez);
			...
			rbMASK._find_component_in_neighborhood(this.masterKey.k_atom_right__station_ziel(),BEWEGUNG_STATION.id_adresse_besitzer.fk()	).rb_set_db_value_manual(id_mandant);
		}

...
public class LG_CO_search_artbez extends RB_HL_SearchArtbez  {
	
	private E2_Grid4MaskSimple  add_on_grid = new E2_Grid4MaskSimple();
	private ENUM_VEKTORPOS_TYP pos_typ = null;
	
	public LG_CO_search_artbez(ENUM_VEKTORPOS_TYP typ, int width) throws myException {
		super();
	...
	private class own_validator extends XX_ActionValidator_NG {
		...
		public MyE2_MessageVector isValid(Component oComponentWhichIsValidated) throws myException {
			...
			LG_CO_search_artbez oThis = LG_CO_search_artbez.this;
			
			LG_CM__Collector cm_collector = (LG_CM__Collector)oThis._find_componentMapCollector_i_belong_to();
	...
	public MyE2_MessageVector do_mask_settings_after_search(String id_artBez, boolean aktiv) throws myException {
		...
		if (S.isFull(id_artBez)) {
			if (this.pos_typ==ENUM_VEKTORPOS_TYP.LG_MAIN) {
				//dann wird die rechte seite der bewegung vorbesetzt
				Component artibelbez_ziel=((LG_CM__Collector)this._find_componentMapCollector_i_belong_to()).get_start_Atom().getComp(BEWEGUNG_ATOM.id_artikel_bez);
				((LG_CO_search_artbez)artibelbez_ziel).rb_set_db_value_manual(id_artBez);
				((LG_CO_search_artbez)artibelbez_ziel).set_c_vergleichswert_dbfeld(id_artBez);;
			}
		}

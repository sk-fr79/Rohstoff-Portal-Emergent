...
public class LG_CM_Station extends RB_ComponentMap {

	private ENUM_STATION_TYP  stations_typ = null;
	private boolean      is_physical = true;

	public LG_CM_Station(ENUM_STATION_TYP p_stationstyp, boolean b_is_physical) throws myException {
		super();
		...
		this.registerComponent(BEWEGUNG_STATION.id_adresse,			new LG_CO_SearchStation(this.stations_typ, 	FZ__CONST.FIELD_WIDTH.ADRESSSUCHE.get_pixel()));
		
		this.registerComponent(BEWEGUNG_STATION.id_adresse_besitzer,  new RB_besitzer_label(false));

//		this.rb_register(BEWEGUNG_STATION.id_adresse_besitzer,  new LG_CO_SearchBesitzer(this.stations_typ, FZ__CONST.FIELD_WIDTH.ADRESSSUCHE.get_pixel()));
		...
		this.registerComponent(BEWEGUNG_STATION.id_bewegung_station, 	new RB_TextField_resizable(100));
	...
	public MyE2_MessageVector maskSettings_define_own_pre_settings(RB_PreSettingsContainer SurfaceSettingsContainer,MASK_STATUS status) throws myException {
		...
		if(status==MASK_STATUS.NEW){
			Rec20 recAdr = new Rec20(_TAB.adresse)._fill_id(bibALL.get_ID_ADRESS_MANDANT());
//			Rec20 recLaender = recAdr.get_up_Rec20(ADRESSE.id_land, LAND.id_land, false);

...
import rohstoff.Echo2BusinessLogic.BEWEGUNG_NG.FUZE.INTERFACES.IF_mapCollector_4_FZ_MaskModulContainer_v2;
...
public class LG_CM__Collector extends RB_ComponentMapCollector implements IF_mapCollector_4_FZ_MaskModulContainer_v2 {
	...
	private __LG_MASTER_KEY 				master_key = null;
	 ...
	 * @throws myException
	 */
	public LG_CM__Collector(E2_NavigationList p_naviList, __LG_MASTER_KEY p_key) throws myException {
		super();

		this.master_key = p_key;
		this.naviList 	= p_naviList;

		this.registerComponent(this.master_key, 									new LG_CM_Bewegung());
		this.registerComponent(this.master_key.k_vektor(), 						new CM_Vektor(p_key));
		this.registerComponent(this.master_key.k_vektor_pos(), 					new LG_CM_VectorPos(ENUM_VEKTORPOS_TYP.LG_MAIN));

		this.registerComponent(this.master_key.k_atom_left(), 					new LG_CM_Atom(this, ENUM_VEKTORPOS_TYP.LG_MAIN));
		this.registerComponent(this.master_key.k_atom_left__station_start(), 		new LG_CM_Station(ENUM_STATION_TYP.START_STATION,true));
		this.registerComponent(this.master_key.k_atom_left__station_ziel(), 		new LG_CM_Station(ENUM_STATION_TYP.ZIEL_STATION,false));

		this.registerComponent(this.master_key.k_atom_right(), 					new LG_CM_Atom(this, ENUM_VEKTORPOS_TYP.LG_MAIN));
		this.registerComponent(this.master_key.k_atom_right__station_start(), 	new LG_CM_Station(ENUM_STATION_TYP.START_STATION,false));
		this.registerComponent(this.master_key.k_atom_right__station_ziel(), 		new LG_CM_Station(ENUM_STATION_TYP.ZIEL_STATION,true));
		...
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Leistungsdatum"), 		new LG_CO_Leistungsdatum(this), 										FZ__CONST.COLUMN_WIDTH.LDATUM.get_column_span(), 	ld,ld),	new LG_CM__column_extenders.ExtDatum(maskContainer, this));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Startstation"), 		this.get_startStationLeft().getComp(BEWEGUNG_STATION.id_adresse), 		FZ__CONST.COLUMN_WIDTH.STATION.get_column_span(), 	ld,ld),	new LG_CM__column_extenders.Station_extenders(maskContainer,this, true));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Zielstation"), 			this.get_zielStationRight().getComp(BEWEGUNG_STATION.id_adresse), 		FZ__CONST.COLUMN_WIDTH.STATION.get_column_span(), 	ld,ld),	new LG_CM__column_extenders.Station_extenders(maskContainer,this, false));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Sorte"), 				this.get_start_Atom().getComp(BEWEGUNG_ATOM.id_artikel_bez), 			FZ__CONST.COLUMN_WIDTH.SORTE.get_column_span(), 	ld,ld),	new LG_CM__column_extenders.ExtSorte(maskContainer,this));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("--leer--"), 			new RB_lab("-"), 														FZ__CONST.COLUMN_WIDTH.KONTRAKT.get_column_span(),	ld,ld), null);
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Menge"), 				new FZ_CO_Mengegrid(this.get_start_Atom().getComp(BEWEGUNG_ATOM.menge)
				,this.get_ziel_Atom().getComp(BEWEGUNG_ATOM.menge)),FZ__CONST.COLUMN_WIDTH.MENGE.get_column_span(), 	ld,ld),	null);
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("EH"), 					this.get_start_Atom().getComp(FZ__CONST.f_keys.EINHEIT.k()), 			FZ__CONST.COLUMN_WIDTH.EINHEIT.get_column_span(), 	ld,ld),	null);
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Preis/EH"), 			new RB_lab("-"), 														FZ__CONST.COLUMN_WIDTH.PREIS.get_column_span(), 	ld,ld),	null);
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("--leer--"), 			new RB_lab("-"), 														FZ__CONST.COLUMN_WIDTH.BETRAG.get_column_span(), 	ld,ld), null);
	...
	public Component render_type_label() throws myException {
		return ENUM_VEKTOR_TYP.LG.get_vektor_typ_label();
	}
	...
	public Component fill_button_grid() throws myException {
		...
		if (stat==MASK_STATUS.NEW) {
			...
			this.button_grid._a(this.bt_abbruch,new RB_gld()._ins(2));
		}
	...
	private class ownActionCloseLine extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			LG_CM__Collector  oThis = LG_CM__Collector.this;
	...
	public void re_open(MASK_STATUS reopen_status) throws myException {
		...
		__LG_MASTER_KEY my_key = this.master_key;
		my_key.set_mask_status(reopen_status);
		LG_DO__Collector  dataColl = new LG_DO__Collector(id_vektor_to_reload, master_key);
	...
	private class ownActionAddNewLine extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			...
			FZ_MASK_MaskModulContainer cont = ((FZ_MASK_MaskModulContainer)LG_CM__Collector.this.rb_get_belongs_to());

			LG_CM__Collector col = cont.add_new_LG();
			cont.rebuild_container_grid();
	...
	private class ownActionFocusLastComponentMapCollector extends XX_ActionAgent {
		@Override
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			FZ_MASK_MaskModulContainer mask = (FZ_MASK_MaskModulContainer)LG_CM__Collector.this.rb_get_maskcontainer_i_belong_to();
			((LG_CM__Collector)mask.get_last_ComponentMapCollector()).set_focus_to_first_in_line();
		}
	}
	...
	public void set_focus_to_first_in_line() throws myException {
		...
		LG_CM_Atom   		mapAtom =  		(LG_CM_Atom) 		this.get(oThis.master_key.k_atom_left());
	...
	@Override
	public __LG_MASTER_KEY get_master_key() {
		return master_key;
	}


	public LG_CM_Station get_startStationLeft() {
		return  (LG_CM_Station)this.rb_get_ComponentMAP(this.master_key.k_atom_left__station_start());
	}

	public LG_CM_Station get_zielStationRight() {
		return  (LG_CM_Station)this.rb_get_ComponentMAP(this.master_key.k_atom_right__station_ziel());
	}

	public LG_CM_Atom get_start_Atom() {
		return  (LG_CM_Atom)this.rb_get_ComponentMAP(this.master_key.k_atom_left());
	}

	public LG_CM_Atom get_ziel_Atom() {
		return  (LG_CM_Atom)this.rb_get_ComponentMAP(this.master_key.k_atom_right());
	}

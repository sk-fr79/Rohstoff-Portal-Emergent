...
public class LG_CM__column_extenders {

	public static class ExtDatum extends FZ_MASK_ContainerSegmentExtender {

		private LG_CM__Collector cm_collector = null;

		public ExtDatum(FZ_MASK_MaskModulContainer fz_mask_modulContainer, LG_CM__Collector p_cm_collector)
				throws myException {
	...
	public static class ExtMenge extends FZ_MASK_ContainerSegmentExtender {
		...
		public ExtMenge(FZ_MASK_MaskModulContainer fz_mask_modulContainer, LG_CM__Collector p_cm_collector)
				throws myException {
			super(p_cm_collector);
		...
		public void open_extension() throws myException {
			...
			baseGrid._startLine(new RB_gld()._col(col_closed)._span(this.get_left_offset()));
			baseGrid._a(new RB_lab_resizable()._tr("Menge netto"),
					new RB_gld()._span(3)._col(col_open)._ins(1));
			baseGrid._endLine(cm_collector.get_start_Atom().getComp(BEWEGUNG_ATOM.menge_netto),
					new RB_gld()._span(5)._col(col_open)._ins(1));

			baseGrid._startLine(new RB_gld()._col(col_closed)._span(this.get_left_offset()));
			baseGrid._a(new RB_lab_resizable()._tr("Plane Menge"),
					new RB_gld()._span(3)._col(col_open)._ins(1));
			baseGrid._endLine(cm_collector.get_start_Atom().getComp(BEWEGUNG_ATOM.planmenge),
					new RB_gld()._span(5)._col(col_open)._ins(1));

			baseGrid._startLine(new RB_gld()._col(col_closed)._span(this.get_left_offset()));
			baseGrid._a(new RB_lab_resizable()._tr("Abzug menge"),
					new RB_gld()._span(3)._col(col_open)._ins(1));
			baseGrid._endLine(cm_collector.get_start_Atom().getComp(BEWEGUNG_ATOM.abzug_menge),
					new RB_gld()._span(5)._col(col_open)._ins(1));
	...
	public static class Station_extenders extends FZ_MASK_ContainerSegmentExtender {

		private LG_CM__Collector cm_collector = null;

		private boolean bStartStation = false;

		public Station_extenders(FZ_MASK_MaskModulContainer fz_mask_modulContainer, LG_CM__Collector p_cm_collector,
				boolean true_start__or__false_ziel) throws myException {
			super(p_cm_collector);
			this.cm_collector = p_cm_collector;
		...
		public void open_extension() throws myException {
			...
			RB_gld endLayoutOpen = new RB_gld()._al(E2_ALIGN.LEFT_MID)._ins(E2_INSETS.I(1, 0, 1, 0))._span(3)
					._col(col_open);
			RB_gld startLayoutOpen = new RB_gld()._col(col_closed)._span(this.get_left_offset());

			baseGrid._startLine(new RB_gld()._col(col_closed)._span(this.get_left_offset()));
			baseGrid._a(new RB_lab_resizable("Besitzer (Leergut)")._lw(),new RB_gld()._span(i_lbl_span)._col(col_open)._ins(1));
	...
	public static class ExtSorte extends FZ_MASK_ContainerSegmentExtender {
		...
		public ExtSorte(FZ_MASK_MaskModulContainer fz_mask_modulContainer, LG_CM__Collector p_cm_collector)
				throws myException {
			super(p_cm_collector);
			this.cm_collector = p_cm_collector;
		}
		...
		@Override
		public void open_extension() throws myException {
			LG_CM_Atom atom_ziel = cm_collector.get_start_Atom();

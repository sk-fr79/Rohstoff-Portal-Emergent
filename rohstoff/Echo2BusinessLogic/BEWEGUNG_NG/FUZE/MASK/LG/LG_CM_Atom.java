...
public class LG_CM_Atom extends RB_ComponentMap {

	private ENUM_VEKTORPOS_TYP 	pos_typ 	= null;
	
	public LG_CM_Atom(LG_CM__Collector component_map_collector, ENUM_VEKTORPOS_TYP typ) throws myException {
		super();
		...
		this.registerComponent(BEWEGUNG_ATOM.id_artikel_bez, 					new LG_CO_search_artbez(this.pos_typ,FZ__CONST.FIELD_WIDTH.ARTIKEL_SUCHE.get_pixel()));
	...
	public MyE2_MessageVector maskSettings_define_own_pre_settings(RB_PreSettingsContainer SurfaceSettingsContainer,MASK_STATUS status) throws myException {
		...
		RECORD_ARTIKEL_BEZ recArt = new RECORD_ARTIKEL_BEZ(2118);
		...
		SurfaceSettingsContainer.rb_get(BEWEGUNG_ATOM.id_artikel_bez)	.set_Enabled(false);
    	...
    	if(status==MASK_STATUS.NEW){
			SurfaceSettingsContainer.rb_get( _TS_CONST._TS_KEYS.ANR12.k())	.set_rb_Default(recArt.get_ID_ARTIKEL_BEZ_cUF_NN(""));
			SurfaceSettingsContainer.rb_get(BEWEGUNG_ATOM.id_artikel_bez)	.set_rb_Default(recArt.get_ID_ARTIKEL_BEZ_cF_NN(""));
			SurfaceSettingsContainer.rb_get(BEWEGUNG_ATOM.id_artikel)		.set_rb_Default(recArt.get_ID_ARTIKEL_BEZ_cF_NN(""));
			SurfaceSettingsContainer.rb_get(BEWEGUNG_ATOM.artbez1)			.set_rb_Default(recArt.get_ARTBEZ1_cF_NN(""));
			SurfaceSettingsContainer.rb_get(BEWEGUNG_ATOM.artbez2)			.set_rb_Default(recArt.get_ARTBEZ2_cF_NN(""));

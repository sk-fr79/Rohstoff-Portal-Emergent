...
public class TS_CM_Station extends RB_ComponentMap {

	private boolean 	is_physical;

	private ENUM_STATION_TYP stations_typ;

	public TS_CM_Station(ENUM_STATION_TYP p_stationstyp, boolean b_is_physical) throws myException {
		super();
		...
		this.registerComponent(BEWEGUNG_STATION.id_adresse , 			new TS_CO_SearchStation(stations_typ, FZ__CONST.FIELD_WIDTH.ADRESSSUCHE.get_pixel()));
		
		this.registerComponent(BEWEGUNG_STATION.id_adresse_besitzer,  new RB_besitzer_label(false));
		
//		this.rb_register(BEWEGUNG_STATION.id_adresse_besitzer, 	new TS_CO_SearchBesitzer(stations_typ));
		...
		this.registerComponent(BEWEGUNG_STATION.kontrollmenge,    	new RB_TextField_resizable(150));
...
//		SurfaceSettingsContainer.rb_get(BEWEGUNG_STATION.mengenvorzeichen).set_Enabled(false);
...
//			SurfaceSettingsContainer.rb_get_forcedDB(BEWEGUNG_STATION.id_adresse_besitzer).rb_set_formated_value_forced_at_save(bibALL.get_ID_ADRESS_MANDANT());
		...
		if(!is_physical){
			SurfaceSettingsContainer.rb_get(BEWEGUNG_STATION.id_adresse)		.set_rb_Default(FZ_SONDERLAGER_ENUM.TS_ZWISCHENLAGER.get_zwischenlager_id());
			SurfaceSettingsContainer.rb_get(BEWEGUNG_STATION.id_adresse_basis)	.set_rb_Default(FZ_SONDERLAGER_ENUM.TS_ZWISCHENLAGER.get_zwischenlager_id());

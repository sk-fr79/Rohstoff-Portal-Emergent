...
public class TS_CM__Collector extends RB_ComponentMapCollector implements IF_mapCollector_4_FZ_MaskModulContainer {
	...
	private __TS_MASTER_KEY 				master_key = 			null;
	...
	public TS_CM__Collector(E2_NavigationList p_naviList, __TS_MASTER_KEY p_key) throws myException {
		super();

		this.master_key = p_key;
		this.naviList 	= p_naviList;
		
		this.registerComponent(this.master_key, 								new TS_CM_Bewegung());
		...
		this.registerComponent(this.master_key.k_atom_left(), 				new TS_CM_Atom(ENUM_VEKTORPOS_TYP.TS_MAIN));
		this.registerComponent(this.master_key.k_atom_left__station_start(), 	new TS_CM_Station(ENUM_STATION_TYP.START_STATION, true));
		this.registerComponent(this.master_key.k_atom_left__station_ziel(), 	new TS_CM_Station(ENUM_STATION_TYP.ZIEL_STATION, false));
		
		this.registerComponent(this.master_key.k_atom_right(),				new TS_CM_Atom(ENUM_VEKTORPOS_TYP.TS_MAIN));
		this.registerComponent(this.master_key.k_atom_right__station_start(), new TS_CM_Station(ENUM_STATION_TYP.START_STATION, false));
		this.registerComponent(this.master_key.k_atom_right__station_ziel(), 	new TS_CM_Station(ENUM_STATION_TYP.ZIEL_STATION, true));
		...
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Leistungsdatum"), 		new TS_CO_Leistungsdatum(this), 											FZ__CONST.COLUMN_WIDTH.LDATUM.get_column_span(), ld,ld),	new TS_CM__column_extenders.ExtDatum(maskContainer, this));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Startstation"), 		this.get_atom_left__station_start().getComp(BEWEGUNG_STATION.id_adresse),	FZ__CONST.COLUMN_WIDTH.STATION.get_column_span(), ld,ld),	new TS_CM__column_extenders.ExtLadestation(maskContainer,this, true));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Zielstation"), 			this.get_atom_right__station_ziel().getComp(BEWEGUNG_STATION.id_adresse), 	FZ__CONST.COLUMN_WIDTH.STATION.get_column_span(), ld,ld),	new TS_CM__column_extenders.ExtLadestation(maskContainer,this, false));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Sorte"), 				this.get_atom_left().getComp(BEWEGUNG_ATOM.id_artikel_bez), 				FZ__CONST.COLUMN_WIDTH.SORTE.get_column_span(), ld,ld),		new TS_CM__column_extenders.ExtSorte(maskContainer,this));
		...
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Lademenge"), 			new FZ_CO_Mengegrid(this.get_atom_left().getComp(BEWEGUNG_ATOM.menge)
		...
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Preis/EH"), 			this.get_atom_left().getComp(BEWEGUNG_ATOM.e_preis), 						FZ__CONST.COLUMN_WIDTH.PREIS.get_column_span(), ld,ld),		null);
	...
	public void set_focus_to_first_in_line() throws myException {
		TS_CM__Collector oThis = TS_CM__Collector.this;
		TS_CM_Atom   		mapAtom =  		(TS_CM_Atom) 		this.get(oThis.master_key.k_atom_left());
	...
	public void re_open(MASK_STATUS reopen_status) throws myException {
		...
		__TS_MASTER_KEY my_key = this.master_key;
		my_key.set_mask_status(reopen_status);
		TS_DO__Collector  dataColl = new TS_DO__Collector(id_vektor_to_reload, master_key);
	...
	//keys accessor
	@Override
	public __TS_MASTER_KEY get_master_key() {
	...
	public TS_CM_Atom get_atom_left() {
		return  (TS_CM_Atom)this.rb_get_ComponentMAP(this.master_key.k_atom_left());
	}
	
	public TS_CM_Atom get_atom_right() {
		return  (TS_CM_Atom)this.rb_get_ComponentMAP(this.master_key.k_atom_right());
	}

	public TS_CM_Station get_atom_left__station_start() {
		return  (TS_CM_Station)this.rb_get_ComponentMAP(this.master_key.k_atom_left__station_start());
	}

	public TS_CM_Station get_atom_left__station_ziel() {
		return  (TS_CM_Station)this.rb_get_ComponentMAP(this.master_key.k_atom_left__station_ziel());
	}

	public TS_CM_Station get_atom_right__station_start() {
		return  (TS_CM_Station)this.rb_get_ComponentMAP(this.master_key.k_atom_right__station_start());
	}
	
	public TS_CM_Station get_atom_right__station_ziel() {
		return  (TS_CM_Station)this.rb_get_ComponentMAP(this.master_key.k_atom_right__station_ziel());
	}
	...
	public Component render_type_label() throws myException {
		return ENUM_VEKTOR_TYP.TS.get_vektor_typ_label();
	}
...
//	}
	...
	private class ownActionAddNewLine extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			...
			FZ_MASK_MaskModulContainer cont = ((FZ_MASK_MaskModulContainer)TS_CM__Collector.this.rb_get_belongs_to());

			TS_CM__Collector col = cont.add_new_TS();
			cont.rebuild_container_grid();
	...
	private class ownActionFocusLastComponentMapCollector extends XX_ActionAgent {
		@Override
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			FZ_MASK_MaskModulContainer mask = (FZ_MASK_MaskModulContainer)TS_CM__Collector.this.rb_get_maskcontainer_i_belong_to();
			((TS_CM__Collector)mask.get_last_ComponentMapCollector()).set_focus_to_first_in_line();
		}
	}

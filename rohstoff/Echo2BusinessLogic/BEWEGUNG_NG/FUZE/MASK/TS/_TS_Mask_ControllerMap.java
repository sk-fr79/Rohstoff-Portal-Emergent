...
public class _TS_Mask_ControllerMap extends RB_MaskControllerMap {

	private __TS_MASTER_KEY masterKey;

	public _TS_Mask_ControllerMap(RB_ComponentMapCollector p_componentMapCollector) throws myException {
		super(p_componentMapCollector);
		this.masterKey = ((TS_CM__Collector)p_componentMapCollector).get_master_key();
	}

	public _TS_Mask_ControllerMap(IF_RB_Component p_component) throws myException {
		super(p_component);
		this.masterKey=((TS_CM__Collector)p_component._find_componentMapCollector_i_belong_to()).get_master_key();	
	}
	...
	public void _fill_artikel_feldern(MyE2_MessageVector mv) throws myException{
		MyLong l_id_artBez = this.get_MyLong_maskVal(this.masterKey.k_atom_left(), BEWEGUNG_ATOM.id_artikel_bez);

		if(l_id_artBez.get_bOK()){
			Rec20_artikel recArtikel = new Rec20_artikel(new Rec20(_TAB.artikel_bez)._fill_id(l_id_artBez.get_cUF_LongString()));

			this.set_maskVal(this.masterKey.k_atom_left(), _TS_CONST._TS_KEYS.ANR12.k(),	l_id_artBez.get_cUF_LongString()						, mv);
			this.set_maskVal(this.masterKey.k_atom_left(), _TS_CONST._TS_KEYS.EINHEIT.k(),	l_id_artBez.get_cUF_LongString()						, mv);
	...
	public void _fill_lager_feldern(MyE2_MessageVector mv) throws myException{
		...
		if ((l_id_adresse != null) && l_id_adresse.get_bOK()) {
			...
			this.set_maskVal(key_left_start, BEWEGUNG_STATION.id_adresse, 			"", 								mv);
			...
			this.set_maskVal(key_left_start, BEWEGUNG_STATION.ortzusatz, 			"", 								mv);
	...
	public void _fill_adresse_feldern(MyE2_MessageVector mv, ENUM_STATION_TYP typ_station) throws myException{
		...
		if(l_id_adresse!= null && l_id_adresse.isOK()){
			...
			if(typ_station == ENUM_STATION_TYP.START_STATION){
				...
				this.set_maskVal(this.masterKey.k_atom_left__station_ziel(), BEWEGUNG_STATION.id_adresse_besitzer, 		id_adresse_basis, mv);
			}else{
				key = this.masterKey.k_atom_right__station_ziel();
				this.set_maskVal(this.masterKey.k_atom_right__station_start(), BEWEGUNG_STATION.id_adresse_besitzer, 	id_adresse_basis, mv);
			}
			this.set_maskVal(key, BEWEGUNG_STATION.id_adresse_besitzer, 	id_adresse_basis,							mv);
	...
	public void _erase_artikel_feldern(MyE2_MessageVector mv){

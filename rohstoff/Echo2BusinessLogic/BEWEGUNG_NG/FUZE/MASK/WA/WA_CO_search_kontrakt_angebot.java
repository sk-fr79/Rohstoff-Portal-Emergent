...
public class WA_CO_search_kontrakt_angebot extends FZ_MASK_Search_Kontrakt_Angebot_in_ONE  {

	public WA_CO_search_kontrakt_angebot(SEARCH_EK_OR_VK p_searchTyp, RB_TextField _tf_kontrakt, RB_TextField _tf_angebot, int i_width) throws myException {
	...
	public MyE2_MessageVector do_mask_settings_after_search(String unformated_MaskValue, boolean aktiv) throws myException {
		...
		new _WA_Controller_kontrakt(this._find_componentMapCollector_i_belong_to()).fill_angebot_kontrakt(mv, unformated_MaskValue);
	...
	protected MyLong get_id_adresse_station_in_search_moment() throws myException {
		...
		WA_CO_SearchStation lieferant = (WA_CO_SearchStation)
								this.rb_ComponentMap_this_belongsTo().rb_get_belongs_to().get(key.zielstation()).getRbComponent(BEWEGUNG_STATION.id_adresse);
	...
	protected MyLong get_id_artikel_bez_in_searchmoment() throws myException {
		...
		MyLong l_id_artbez = new _WA_Controller_kontrakt(this._find_componentMapCollector_i_belong_to()).get_MyLong_maskVal(key, BEWEGUNG_ATOM.id_artikel_bez);

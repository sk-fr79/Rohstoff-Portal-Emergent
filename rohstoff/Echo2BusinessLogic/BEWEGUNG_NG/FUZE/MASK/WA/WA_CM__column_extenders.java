...
public class WA_CM__column_extenders {

	public static class ExtDatum extends FZ_MASK_ContainerSegmentExtender {

		private  WA_CM__Collector cm_collector=null;

		public ExtDatum(FZ_MASK_MaskModulContainer fz_mask_modulContainer, WA_CM__Collector p_cm_collector) throws myException {
	...
	public static class ExtMenge extends FZ_MASK_ContainerSegmentExtender {
		...
		public ExtMenge(FZ_MASK_MaskModulContainer fz_mask_modulContainer, WA_CM__Collector p_cm_collector) throws myException {
			super(p_cm_collector);
		...
		public void open_extension() throws myException {
			...
			baseGrid._startLine(new RB_gld()._col(col_closed)._span(this.get_left_offset()));
			baseGrid._a(new RB_lab_resizable()._tr("Menge netto"), new RB_gld()._span(i_lbl_span)._col(col_open)._ins(2));
			baseGrid._endLine(cm_collector.get_atom_left().getComp(BEWEGUNG_ATOM.menge_netto),new RB_gld()._span(5)._col(col_open)._ins(2));

			baseGrid._startLine(new RB_gld()._col(col_closed)._span(this.get_left_offset()));
			baseGrid._a(new RB_lab_resizable()._tr("Plane Menge"),new RB_gld()._span(i_lbl_span)._col(col_open)._ins(2));
			baseGrid._endLine(cm_collector.get_atom_left().getComp(BEWEGUNG_ATOM.planmenge),new RB_gld()._span(5)._col(col_open)._ins(2));

			baseGrid._startLine(new RB_gld()._col(col_closed)._span(this.get_left_offset()));
			baseGrid._a(new RB_lab_resizable()._tr("Abzug menge"),new RB_gld()._span(i_lbl_span)._col(col_open)._ins(2));
			baseGrid._endLine(cm_collector.get_atom_left().getComp(BEWEGUNG_ATOM.abzug_menge),new RB_gld()._span(5)._col(col_open)._ins(2));
		}
	...
	public static class ExtLadestation extends FZ_MASK_ContainerSegmentExtender {
		...
		public ExtLadestation(FZ_MASK_MaskModulContainer fz_mask_modulContainer, WA_CM__Collector p_cm_collector, boolean true_start__or__false_ziel) throws myException {
			super(p_cm_collector);
			this.cm_collector = p_cm_collector;
	...
	public static class ExtSorte extends FZ_MASK_ContainerSegmentExtender {
		...
		public ExtSorte(FZ_MASK_MaskModulContainer fz_mask_modulContainer, WA_CM__Collector p_cm_collector) throws myException {
			super(p_cm_collector);
			this.cm_collector = p_cm_collector;
		}
		...
		@Override
		public void open_extension() throws myException {
			WA_CM_Atom atom_ziel 	= cm_collector.get_atom_left();
			...
			._endLine(atom_ziel.getRbComponent(BEWEGUNG_ATOM.id_artikel),new RB_gld()._span(5)._col(col_open)._ins(2));
			
			baseGrid._startLine(new RB_gld()._col(col_closed)._span(this.get_left_offset()))
			._a(new RB_lab_resizable("ANR1-2")._lw(), new RB_gld()._span(i_lbl_span)._col(col_open)._ins(2))
			._endLine(atom_ziel.getRbComponent(_TS_CONST._TS_KEYS.ANR12.k()),new RB_gld()._span(2)._col(col_open)._ins(2)._al(E2_ALIGN.LEFT_MID));

			baseGrid._startLine(new RB_gld()._col(col_closed)._span(this.get_left_offset()))
			._a(new RB_lab_resizable("Art bez 1")._lw(), new RB_gld()._span(i_lbl_span)._col(col_open)._ins(2))
			._endLine(atom_ziel.getRbComponent(BEWEGUNG_ATOM.artbez1),new RB_gld()._span(5)._col(col_open)._ins(2));
			
			baseGrid._startLine(new RB_gld()._col(col_closed)._span(this.get_left_offset()))
			._a(new RB_lab_resizable("Art bez 2")._lw(), new RB_gld()._span(i_lbl_span)._col(col_open)._ins(2))
			._endLine(atom_ziel.getRbComponent(BEWEGUNG_ATOM.artbez2),new RB_gld()._span(5)._col(col_open)._ins(2));
...
public static class ExtKontraktAngebot extends FZ_MASK_ContainerSegmentExtender {
		...
		public ExtKontraktAngebot(FZ_MASK_MaskModulContainer fz_mask_modulContainer, WA_CM__Collector p_cm_collector) throws myException {
			super(p_cm_collector);
			this.cm_collector = p_cm_collector;
		}
		...
		@Override
		public void open_extension() throws myException {
			WA_CM_Atom 			atom = 			cm_collector.get_atom_right();
			
			RB_gld glstd_label= new RB_gld()._ins(E2_INSETS.I(2,2,2,2))._col(this.col_High())._span(5);
			
			RB_gld glstd_data = new RB_gld()._ins(E2_INSETS.I(2,2,2,2))._col(this.col_High())._span(3);
			...
			this.mask_Grid()._startLine(gls)._a(new RB_lab_resizable("Kontrakt (ID)"),glstd_label)
											._endLine(sf_id_vpos_kon,glstd_data)
//											._endLine(glstd2)
											;

			this.mask_Grid()._startLine(gls)._a(new RB_lab_resizable("Angebot (ID)"),glstd_label)
											._endLine(sf_id_vpos_std,glstd_data)
//											._endLine(glstd2)
											;

...
import _Test.Test_SEBASTIEN.Test_Umbuchung_Sebastien;
...
public class WE_CM__Collector extends RB_ComponentMapCollector implements IF_mapCollector_4_FZ_MaskModulContainer  {
	...
	private __WE_MASTER_KEY 				master_key = null;
	 ...
	 * @throws myException
	 */
	public WE_CM__Collector(E2_NavigationList p_naviList, __WE_MASTER_KEY p_key) throws myException {
		super();
		...
		this.registerComponent(this.master_key, new WE_CM_Bewegung());
		...
		this.registerComponent(this.master_key.k_atom_left(), 					new WE_CM_Atom(ENUM_VEKTORPOS_TYP.WE_MAIN));
		this.registerComponent(this.master_key.k_atom_left__station_start(), 		new WE_CM_Station(ENUM_STATION_TYP.START_STATION, true));
		this.registerComponent(this.master_key.k_atom_left__station_ziel(), 		new WE_CM_Station(ENUM_STATION_TYP.ZIEL_STATION, false));

		this.registerComponent(this.master_key.k_atom_right(), 					new WE_CM_Atom(ENUM_VEKTORPOS_TYP.WE_MAIN));
		this.registerComponent(this.master_key.k_atom_right__station_start(), 	new WE_CM_Station(ENUM_STATION_TYP.START_STATION, false));
		this.registerComponent(this.master_key.k_atom_right__station_ziel(), 		new WE_CM_Station(ENUM_STATION_TYP.ZIEL_STATION, true));
		...
		FZ_MASK_MaskModulContainer mask = this.get_my_maskContainer();
		...
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Leistungsdatum"), 			new WE_CO_Leistungsdatum(this), 										FZ__CONST.COLUMN_WIDTH.LDATUM.get_column_span(), ld,ld),	new WE_CM__column_extenders.ExtDatum(mask, this));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Startstation"), 			this.get_startStation().getComp(BEWEGUNG_STATION.id_adresse), 			FZ__CONST.COLUMN_WIDTH.STATION.get_column_span(), ld,ld),	new WE_CM__column_extenders.ExtStartlager(mask, this, true));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Unser Lager"), 				this.get_zielStation().getComp(BEWEGUNG_STATION.id_adresse), 			FZ__CONST.COLUMN_WIDTH.STATION.get_column_span(), ld,ld),	new WE_CM__column_extenders.ExtStartlager(mask, this, false));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Sorte"), 					this.get_left_atom().getComp(BEWEGUNG_ATOM.id_artikel_bez), 			FZ__CONST.COLUMN_WIDTH.SORTE.get_column_span(), ld,ld),		new WE_CM__column_extenders.ExtSorte(mask, this));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Kontrakt/Angebot"), 		this.get_left_atom().getComp(FZ__CONST.f_keys.KOMBI_ANG_KON_LEFT.k()), 	FZ__CONST.COLUMN_WIDTH.KONTRAKT.get_column_span(),ld,ld), 	new WE_CM__column_extenders.ExtKontraktAngebot(mask, this));
		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Menge"), 					new FZ_CO_Mengegrid(this.get_left_atom().getComp(BEWEGUNG_ATOM.menge)
				,this.get_right_atom().getComp(BEWEGUNG_ATOM.menge)),FZ__CONST.COLUMN_WIDTH.MENGE.get_column_span(), ld,ld),	null);

		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("EH"), 						this.get_left_atom().getComp(FZ__CONST.f_keys.EINHEIT.k()), 			FZ__CONST.COLUMN_WIDTH.EINHEIT.get_column_span(), ld,ld),	null);

		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Preis/EH"), 				new E2_Grid()._s(2)
				._a(this.get_left_atom().getComp(BEWEGUNG_ATOM.e_preis), new RB_gld()._ins(2,0,2,1)._span(2))
				._a(new RB_lab("Pr. manuell"), ld)
				._a(this.get_left_atom().getComp(BEWEGUNG_ATOM.manuell_preis), ld)	, 	FZ__CONST.COLUMN_WIDTH.PREIS.get_column_span(), ld, ld),	null); 					

		this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Wert"), 					this.get_left_atom().getComp(BEWEGUNG_ATOM.gesamtpreis), 				FZ__CONST.COLUMN_WIDTH.BETRAG.get_column_span(), ld,ld), 	null);
	...
	private void init_pop_up(){
		this.bt_PopUpZusatz.addButton(new Test_Umbuchung_Sebastien(this), true);
	}


	//holt immer das zu bearbeitende bewegungsatom, nicht die automatischen
	public WE_CM_Station  get_startStation() throws myException {
		return (WE_CM_Station)this.get(this.master_key.k_atom_left__station_start());
	}

	//holt immer das zu bearbeitende bewegungsatom, nicht die automatischen
	public WE_CM_Station  get_zielStation() throws myException {
		return (WE_CM_Station)this.get(this.master_key.k_atom_right__station_ziel());
	}


	//holt immer das zu bearbeitende bewegungsatom, nicht die automatischen
	public WE_CM_Atom  get_left_atom() throws myException {
		return (WE_CM_Atom)this.get(this.master_key.k_atom_left());
	}

	public WE_CM_Atom  get_right_atom() throws myException {
		return (WE_CM_Atom)this.get(this.master_key.k_atom_right());
	}


	public CM_Vektor  get_vektor() throws myException {
		return (CM_Vektor)this.get(this.master_key.k_vektor());
	}
	...
	public Component render_type_label() throws myException {
		return ENUM_VEKTOR_TYP.WE.get_vektor_typ_label();
	}
	...
	public Component fill_button_grid() throws myException {
		...
		if (stat==MASK_STATUS.NEW) {
			this.button_grid
			._a(this.bt_PopUpZusatz)
			._a(this.bt_save_and_next)
			._a(this.bt_save_and_close)
			._a(this.bt_abbruch);
			...
			._a(this.bt_PopUpZusatz)
			._a(this.bt_save_and_close)
			._a(this.bt_save_and_reopen_as_view)
			._a(this.bt_abbruch);

		} else if (stat==MASK_STATUS.VIEW) {
			this.button_grid
			._a(new MyE2_Label(E2_ResourceIcon.get_RI("empty25.png")))
			._a(this.bt_open4Edit)
			._a(new MyE2_Label(E2_ResourceIcon.get_RI("empty25.png")))
			._a(this.bt_abbruch);
		} else {
	...
	private class ownActionCloseLine extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			WE_CM__Collector  oThis = WE_CM__Collector.this;
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			...
			if (stat==MASK_STATUS.NEW) {
				...
				if (bibMSG.get_bIsOK()) {
					...
					if (S.isFull(id_vektor_to_reload)) {
						oThis.naviList.ADD_NEW_ID_TO_ALL_VECTORS_IN_FRONT(bibVECTOR.get_Vector(id_vektor_to_reload));
						oThis.naviList._REBUILD_ACTUAL_SITE(id_vektor_to_reload);
					} 
				}
			} else {
				bibMSG.add_MESSAGE(oThis.rb_COMPLETE_SAVE_CYCLE(true));
				...
				//liste aktualisieren
				String id_vektor = oThis.get(oThis.master_key.k_vektor()).getRbDataObjectActual().rec20().get_ufs_dbVal(BEWEGUNG_VEKTOR.id_bewegung_vektor);
				oThis.naviList.Refresh_ComponentMAP(id_vektor, E2_ComponentMAP.STATUS_VIEW);
	...
	public void re_open(MASK_STATUS status_new) throws myException {
		...
		__WE_MASTER_KEY my_key = this.master_key;
		my_key.set_mask_status(status_new);
		WE_DO__Collector  dataColl = new WE_DO__Collector(id_vektor_to_reload, master_key);
	...
	private class ownActionAddNewLine extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			...
			FZ_MASK_MaskModulContainer cont = ((FZ_MASK_MaskModulContainer)WE_CM__Collector.this.rb_get_belongs_to());

			WE_CM__Collector col = cont.add_new_WE();
			cont.rebuild_container_grid();
	...
	private class ownActionFocusLastComponentMapCollector extends XX_ActionAgent {
		@Override
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			FZ_MASK_MaskModulContainer mask = (FZ_MASK_MaskModulContainer)WE_CM__Collector.this.rb_get_maskcontainer_i_belong_to();
			((WE_CM__Collector)mask.get_last_ComponentMapCollector()).set_focus_to_first_in_line();
		}
	}



	/*  private class ownActionInterneVerteilung extends XX_ActionAgent {
			...
			public void executeAgentCode(ExecINFO oExecInfo) throws myException {
				...
				WE_CM_Atom  ba = (WE_CM_Atom)WE_CM__Collector.this.get(oThis.master_key.k_atom());
				String wert = ((RB_TextField)ba.rb_Component(BEWEGUNG_ATOM.menge)).getText();
				String c_message = "";

				if (ba.rb_Mask_Dataobject_actual().get_RecORD()==null) {
					throw new myException("Error finding dataObject !!");   			//sollte nicht vorkommen
				}

//				RECORD_BEWEGUNG_ATOM  atom = (RECORD_BEWEGUNG_ATOM)ba.rb_Mask_Dataobject_actual().get_RecORD();


				if (S.isFull(wert)) {
					MyBigDecimal bd_menge = new MyBigDecimal(wert);
					if (bd_menge.get_bOK()) {
						new SEP_HiddenMASK(ba, mask)._add_new_Row();
					} else {
						c_message="Die Mengenangabe ist nicht interpretierbar";
					}
				} else{
					c_message="Es ist keine Menge vorhanden";
				}

				if (S.isFull(c_message)) {
					bibMSG.add_MESSAGE(new MyE2_Alarm_Message(c_message));
				}
	    ...
	    private class ownActionOffeneVerteilung extends XX_ActionAgent {
			...
			public void executeAgentCode(ExecINFO oExecInfo) throws myException {
				...
				if (S.isFull(wert)) {
					MyBigDecimal bd_menge = new MyBigDecimal(wert);
					if (bd_menge.get_bOK()) {
						new SEP_OpenMASK(atom, oThis)._add_new_Row();
					} else {
						c_message="Die Mengenangabe ist nicht interpretierbar";
					}
				} else{
	...
	public void set_focus_to_first_in_line() throws myException {
		...
		WE_CM_Atom   		mapAtom =  		(WE_CM_Atom) 		this.get(oThis.master_key.k_atom_left());
	...
	@Override
	public __WE_MASTER_KEY get_master_key() {
		return master_key;
	}
	...
	//			FZ_Grid_4_anzeigeInternBuchung  addons = (FZ_Grid_4_anzeigeInternBuchung)object_from_check;
	//

...
import panter.gmbh.indep.dataTools.RecFactory;
...
public class WE_CO_SearchStation_customer extends RB_HL_SearchAdressStation  {

	private ENUM_STATION_TYP  typ_station = null;
	
	
	
	public WE_CO_SearchStation_customer(ENUM_STATION_TYP start_or_ziel,int i_width) throws myException {
		super(true);
		this.typ_station = start_or_ziel;
		
		this.get_tf_search_input()._w(60);
...
//		FZ_MaskFormatter.ArrangeAdressSearchField.arange(this);
	...
	public MyE2_MessageVector do_mask_settings_after_search(String id_adresse, boolean aktiv) throws myException {
		...
		_WE_MASK_Controller controller =new _WE_MASK_Controller(this);
		
		controller.fill_lieferant_adress((RECORD_ADRESSE)RecFactory.gen(_TAB.adresse, id_adresse), mv);

		if (aktiv) {
			controller.set_maskVal(controller.get_masterKey().k_atom_left(),FZ__CONST.f_keys.KOMBI_ANG_KON_LEFT.k(),"", mv);
			controller.set_maskVal(controller.get_masterKey().k_atom_left(),BEWEGUNG_ATOM.id_artikel_bez,"", mv);
			
//			controller._refresh().fill_kontrakt_kombi_followers( mv);
			controller._refresh().fill_sorten_bez(mv);
		}

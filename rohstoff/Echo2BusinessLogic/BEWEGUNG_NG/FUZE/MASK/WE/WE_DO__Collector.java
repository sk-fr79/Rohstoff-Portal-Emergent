...
public class WE_DO__Collector extends FZ_dataObjectsCollector {

	private __WE_MASTER_KEY key = null;

	public WE_DO__Collector(__WE_MASTER_KEY p_key) throws myException {
		super();
		this.key = p_key;

		this.registerComponent(this.key, 									new WE_DO_Bewegung());
		this.registerComponent(this.key.k_vektor(), 						new WE_DO_Vektor());

		this.registerComponent(this.key.k_vektor_pos(), 					new WE_DO_VektorPos());

		this.registerComponent(this.key.k_atom_left(), 					new WE_DO_Atom());
		this.registerComponent(this.key.k_atom_left__station_start(), 	new WE_DO_Station());
		this.registerComponent(this.key.k_atom_left__station_ziel(), 		new WE_DO_Station());

		this.registerComponent(this.key.k_atom_right(), 					new WE_DO_Atom());
		this.registerComponent(this.key.k_atom_right__station_start(), 	new WE_DO_Station());
		this.registerComponent(this.key.k_atom_right__station_ziel(), 	new WE_DO_Station());
	}

	public WE_DO__Collector(String id_bewegung_vektor, IF_MasterKey p_key) throws myException {
		super();

		this.key = (__WE_MASTER_KEY)p_key;
		...
		RecList20  rlp = rv.get_down_reclist20(BEWEGUNG_VEKTOR_POS.id_bewegung_vektor, new vgl(BEWEGUNG_VEKTOR_POS.pos_typ, ENUM_VEKTORPOS_TYP.WE_MAIN.db_val()).s(), null);
		...
		this.registerComponent(this.key, 									new WE_DO_Bewegung(rb,			this.key.get_mask_status()));
		this.registerComponent(this.key.k_vektor(),  						new WE_DO_Vektor(rv, 			this.key.get_mask_status()));
		this.registerComponent(this.key.k_vektor_pos(),  					new WE_DO_VektorPos(rlp.get(0), this.key.get_mask_status()));

		this.registerComponent(this.key.k_atom_left(),  					new WE_DO_Atom(rla.get(0), 								this.key.get_mask_status()));
		this.registerComponent(this.key.k_atom_left__station_start(), 	new WE_DO_Station(record_atom_START.__station_start(),	this.key.get_mask_status()));
		this.registerComponent(this.key.k_atom_left__station_ziel(), 		new WE_DO_Station(record_atom_START.__station_ziel(),	this.key.get_mask_status()));

		this.registerComponent(this.key.k_atom_right(),  					new WE_DO_Atom(rla.get(1), 								this.key.get_mask_status()));
		this.registerComponent(this.key.k_atom_right__station_start(), 	new WE_DO_Station(record_atom_ZIEL.__station_start(),	this.key.get_mask_status()));
		this.registerComponent(this.key.k_atom_right__station_ziel(), 	new WE_DO_Station(record_atom_ZIEL.__station_ziel(),	this.key.get_mask_status()));
	}

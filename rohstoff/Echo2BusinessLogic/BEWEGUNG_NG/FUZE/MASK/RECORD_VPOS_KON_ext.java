...
public class RECORD_VPOS_KON_ext extends RECORD_VPOS_KON {
	...
	//hilfsmethode um fuellen der suchergebnisse in einem RB_SearchField
	public void fill_grid_with_infos(E2_Grid grid) throws myException {
		...
		RECORD_VPOS_KON_TRAKT	vkt = this.get_DOWN_RECORD_LIST_VPOS_KON_TRAKT_id_vpos_kon().get(0);
		RECORD_ADRESSE   		ad  = this.get_UP_RECORD_VKOPF_KON_id_vkopf_kon().get_UP_RECORD_ADRESSE_id_adresse();
		...
		grid._a(new RB_lab("Kontrakt (ID): " + this.ufs(VPOS_KON.id_vpos_kon))._b()._fsa(-2),gl1);
...
//		
//		RB_gld gl  = new RB_gld()._ins(E2_INSETS.I(1,0,2,1))._left_mid();
//
//
//		grid.removeAll();
//		grid.setStyle(MyE2_Grid.STYLE_GRID_NO_BORDER_NO_INSETS());
//		grid._setSize(80,60,60);
//				
//		RECORD_VPOS_KON_TRAKT	vkt = this.get_DOWN_RECORD_LIST_VPOS_KON_TRAKT_id_vpos_kon().get(0);
//		RECORD_ADRESSE   		ad  = this.get_UP_RECORD_VKOPF_KON_id_vkopf_kon().get_UP_RECORD_ADRESSE_id_adresse();
//		RECORD_WAEHRUNG         waeh = this.get_UP_RECORD_WAEHRUNG_id_waehrung_fremd();

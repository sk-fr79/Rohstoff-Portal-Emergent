...
public class LL_CM__Collector extends RB_ComponentMapCollector implements IF_mapCollector_4_FZ_MaskModulContainer {
	...
	private __LL_MASTER_KEY 				master_key = null;
	 ...
	 * @throws myException
	 */
    public LL_CM__Collector(E2_NavigationList p_naviList, __LL_MASTER_KEY p_key) throws myException {
        super();
        
        this.master_key = p_key;
        this.naviList 	= p_naviList;
        
        this.registerComponent(this.master_key, new LL_CM_Bewegung());
      	...
      	this.registerComponent(this.master_key.k_vektor(), 					new CM_Vektor(p_key));
       	...
       	this.registerComponent(this.master_key.k_atom_left(), 				new LL_CM_Atom(ENUM_VEKTORPOS_TYP.LL_MAIN));
       	this.registerComponent(this.master_key.k_atom_left__lager_start(), 	new LL_CM_Station(ENUM_STATION_TYP.START_STATION,true));
       	this.registerComponent(this.master_key.k_atom_left__lager_ziel(), 	new LL_CM_Station(ENUM_STATION_TYP.ZIEL_STATION,false));
       	
       	this.registerComponent(this.master_key.k_atom_right(), 				new LL_CM_Atom(ENUM_VEKTORPOS_TYP.LL_MAIN));
       	this.registerComponent(this.master_key.k_atom_right__lager_start(), 	new LL_CM_Station(ENUM_STATION_TYP.START_STATION,false));
       	this.registerComponent(this.master_key.k_atom_right__lager_ziel(), 	new LL_CM_Station(ENUM_STATION_TYP.ZIEL_STATION,true));
        ...
        this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Leistungsdatum"), 		new LL_CO_Leistungdatum(this), 												FZ__CONST.COLUMN_WIDTH.LDATUM.get_column_span(), ld,ld),	new LL_CM__column_extenders.ExtDatum(maskContainer,this));
        this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Startlager"), 			this.get_atom_left__lager_left().getComp(BEWEGUNG_STATION.id_adresse), 		FZ__CONST.COLUMN_WIDTH.STATION.get_column_span(), ld,ld),	new LL_CM__column_extenders.Ext_Station(maskContainer,this, true));
        this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Ziellager"), 			this.get_atom_right__lager_ziel().getComp(BEWEGUNG_STATION.id_adresse), 	FZ__CONST.COLUMN_WIDTH.STATION.get_column_span(), ld,ld),	new LL_CM__column_extenders.Ext_Station(maskContainer,this, false));
        this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("Sorte (Startlager)"), 	this.get_start_atom().getComp(BEWEGUNG_ATOM.id_artikel_bez), 				FZ__CONST.COLUMN_WIDTH.SORTE.get_column_span(), ld,ld),		new LL_CM__column_extenders.ExtSorte(maskContainer,this));
        ...
        this.add_segment_in_maskline(new FZ_MASK_ContainerSegmentInLine(this, new RB_lab()._tr("--leer--"), 			new RB_lab("-"), 															FZ__CONST.COLUMN_WIDTH.PREIS.get_column_span(), ld,ld),		null);
	...
	public Component render_type_label() throws myException {
		return ENUM_VEKTOR_TYP.LL.get_vektor_typ_label();
	}
    ...
    private class ownActionCloseLine extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			LL_CM__Collector  oThis = LL_CM__Collector.this;
	   ...
	   public void re_open(MASK_STATUS reopen_status) throws myException {
			...
			__LL_MASTER_KEY my_key = this.master_key;
			my_key.set_mask_status(reopen_status);
			LL_DO__Collector  dataColl = new LL_DO__Collector(id_vektor_to_reload, master_key);
	    ...
	    private class ownActionAddNewLine extends XX_ActionAgent {
			...
			public void executeAgentCode(ExecINFO oExecInfo) throws myException {
				...
				FZ_MASK_MaskModulContainer cont = ((FZ_MASK_MaskModulContainer)LL_CM__Collector.this.rb_get_belongs_to());
				
				LL_CM__Collector col = cont.add_new_LL();
				cont.rebuild_container_grid();
	    ...
	    private class ownActionFocusLastComponentMapCollector extends XX_ActionAgent {
			@Override
			public void executeAgentCode(ExecINFO oExecInfo) throws myException {
				FZ_MASK_MaskModulContainer mask = (FZ_MASK_MaskModulContainer)LL_CM__Collector.this.rb_get_maskcontainer_i_belong_to();
				((LL_CM__Collector)mask.get_last_ComponentMapCollector()).set_focus_to_first_in_line();
			}
	    }
		...
		public void set_focus_to_first_in_line() throws myException {
		    ...
		    LL_CM_Atom   		mapAtom =  		(LL_CM_Atom) 		this.get(oThis.master_key.k_atom_left());
		...
		@Override
		public __LL_MASTER_KEY get_master_key() {
			return master_key;
		}

		public LL_CM_Station get_atom_left__lager_right() {
			return  (LL_CM_Station)this.rb_get_ComponentMAP(this.master_key.k_atom_left__lager_start());
		}
		
		public LL_CM_Station get_atom_left__lager_left() {
			return  (LL_CM_Station)this.rb_get_ComponentMAP(this.master_key.k_atom_left__lager_start());
		}
		
		public LL_CM_Station get_atom_right__lager_ziel() {
			return  (LL_CM_Station)this.rb_get_ComponentMAP(this.master_key.k_atom_right__lager_ziel());
		}
	
		public LL_CM_Station get_atom_right__lager_start() {
			return  (LL_CM_Station)this.rb_get_ComponentMAP(this.master_key.k_atom_right__lager_start());
		}
		
		public LL_CM_Atom get_start_atom() {
			return  (LL_CM_Atom)this.rb_get_ComponentMAP(this.master_key.k_atom_left());
		}
		
		public LL_CM_Atom get_ziel_atom() {
			return  (LL_CM_Atom)this.rb_get_ComponentMAP(this.master_key.k_atom_right());
		}
...
////			Color col_high = 		this.get_my_maskContainer().get_color_for_expanded_block();

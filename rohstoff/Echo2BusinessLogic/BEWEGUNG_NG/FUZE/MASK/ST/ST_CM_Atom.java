...
public class ST_CM_Atom extends RB_ComponentMap {
	
	private ENUM_VEKTORPOS_TYP pos_typ = null;

    public ST_CM_Atom(ENUM_VEKTORPOS_TYP typ) throws myException {
        super();
        ...
        RB_TextField  tf_angebot = new RB_TextField_resizable(118);
        RB_TextField  tf_kontrakt = new RB_TextField_resizable(118);
        
       	this.registerComponent(BEWEGUNG_ATOM.id_artikel_bez, 					new ST_CO_search_artbez(this.pos_typ,90));   
        
        this.registerComponent(BEWEGUNG_ATOM.id_vpos_std,  					tf_angebot);
        this.registerComponent(BEWEGUNG_ATOM.id_vpos_kon,  					tf_kontrakt);
        this.registerComponent(BEWEGUNG_ATOM.manuell_preis,  					new RB_cb());
		...
		this.registerComponent(BEWEGUNG_ATOM.artbez1, 						new RB_TextField_resizable(327));
		...
		this.registerComponent(BEWEGUNG_ATOM.id_artikel, 						new RB_TextField_resizable(125));
		...
		this.registerComponent(BEWEGUNG_ATOM.leistungsdatum,  				new ST_CO_Date_Selektor(80,pos_typ));		
		...
		this.registerComponent(BEWEGUNG_ATOM.artbez2, 						new RB_TextArea(325,5));
        ...
        if (this.pos_typ==ENUM_VEKTORPOS_TYP.ST_MAIN_LEFT) {
	        this.registerComponent(FZ__CONST.f_keys.EINHEIT_LEFT.k(),  		new RB_label_einheit()._fsa(2));
	        this.registerComponent(FZ__CONST.f_keys.EINHEIT_PREIS_LEFT.k(),  	new RB_label_einheitpreis());
	        this.registerComponent(FZ__CONST.f_keys.KOMBI_ANG_KON_LEFT.k(),  	new ST_CO_search_kontrakt_angebot(SEARCH_EK_OR_VK.EK, 
	        															tf_kontrakt, tf_angebot, pos_typ, 
	        															FZ__CONST.FIELD_WIDTH.KONTRAKT_SUCHE.get_pixel()));
	       
	        this.registerSetterValidators(BEWEGUNG_ATOM.manuell_preis.fk(), new _ST_SAV_Manuell_Epreis_Quelle());
	        
        } else {
	        this.registerComponent(FZ__CONST.f_keys.EINHEIT_RIGHT.k(),  		new RB_label_einheit()._fsa(2));
	        this.registerComponent(FZ__CONST.f_keys.EINHEIT_PREIS_RIGHT.k(),  new RB_label_einheitpreis());
	        this.registerComponent(FZ__CONST.f_keys.KOMBI_ANG_KON_RIGHT.k(),  new ST_CO_search_kontrakt_angebot(SEARCH_EK_OR_VK.VK, 
	        															tf_kontrakt, tf_angebot, pos_typ, 
	        															FZ__CONST.FIELD_WIDTH.KONTRAKT_SUCHE.get_pixel()));
	        
	        this.registerSetterValidators(BEWEGUNG_ATOM.manuell_preis.fk(), new _ST_SAV_Manuell_Epreis_Ziel());

...
public class ST_CO_search_kontrakt_angebot extends FZ_MASK_Search_Kontrakt_Angebot_in_ONE  {

	private ENUM_VEKTORPOS_TYP pos_typ = null;

	public ST_CO_search_kontrakt_angebot(SEARCH_EK_OR_VK p_searchTyp, RB_TextField _tf_kontrakt, RB_TextField _tf_angebot, ENUM_VEKTORPOS_TYP typ,int i_width) throws myException {
	...
	public MyE2_MessageVector do_mask_settings_after_search(String unformated_MaskValue, boolean aktiv) throws myException {
		...
		if(pos_typ==ENUM_VEKTORPOS_TYP.ST_MAIN_LEFT){
			new _ST_Controller_kontrakt_Quelle(this._find_componentMapCollector_i_belong_to()).fill_angebot_kontrakt(mv, unformated_MaskValue);
		}else{
			new _ST_Controller_kontrakt_Ziel(this._find_componentMapCollector_i_belong_to()).fill_angebot_kontrakt(mv, unformated_MaskValue);
		}
	...
	public void render_search_result_visible_on_mask(E2_Grid grid_result_container, String c_result_value_4_db) throws myException {
		...
		if (l_angebot.get_bOK()) {
			grid_result_container._a(new RB_lab("Angebot: " + l_angebot.get_cUF_LongString())._b());
			...
			grid_result_container._a(new RB_lab("Kontrakt: " + l_kontrakt.get_cUF_LongString())._b());
	...
	protected MyLong get_id_adresse_station_in_search_moment() throws myException {
		...
		KEY_STATION lief_abn = this.get_typ()==SEARCH_EK_OR_VK.EK ? key.startstation() : key.zielstation();

		ST___MASK_Controller  cont = new ST___MASK_Controller(this._find_componentMapCollector_i_belong_to());

		if (cont != null) {
			MyLong l_id_adresse = cont.get_MyLong_maskVal(lief_abn, BEWEGUNG_STATION.id_adresse);
	...
	protected MyLong get_id_artikel_bez_in_searchmoment() throws myException {
		...
		MyLong l_id_artbez = new ST___MASK_Controller(this._find_componentMapCollector_i_belong_to()).get_MyLong_maskVal(key, BEWEGUNG_ATOM.id_artikel_bez);

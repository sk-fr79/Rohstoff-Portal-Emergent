...
import rohstoff.Echo2BusinessLogic.BEWEGUNG_NG.FUZE.MASK.FZ_MaskShowAdresse;
...
public class ST_CM_Station extends RB_ComponentMap {
	
	private ENUM_STATION_TYP  stationstyp = null;
	private boolean      is_physical = true;
	
    public ST_CM_Station(ENUM_STATION_TYP p_stationstyp, boolean b_is_physical) throws myException {
    	super();
    	...
    	if (this.is_physical) {
           	this.registerComponent(new RB_KF(BEWEGUNG_STATION.id_adresse),			new ST_CO_SearchStation_customer(this.stationstyp, FZ__CONST.FIELD_WIDTH.ADRESSSUCHE.get_pixel()));
    	} else {
    		this.registerComponent(new RB_KF(BEWEGUNG_STATION.id_adresse),			new RB_TextField_resizable(35));
    	}

    	this.registerComponent(BEWEGUNG_STATION.id_adresse_besitzer,  new RB_besitzer_label(false));
    	
       	this.registerComponent(BEWEGUNG_STATION.id_adresse_basis,   	new FZ_MaskShowAdresse());
	    ...
	    this.registerComponent(BEWEGUNG_STATION.kontrollmenge,    	new RB_TextField_resizable(100));
	    ...
	    this.registerComponent(BEWEGUNG_STATION.wiegekartenkenner,    new RB_TextField_resizable(100));
    ...
    public MyE2_MessageVector maskSettings_define_own_pre_settings(RB_PreSettingsContainer SurfaceSettingsContainer,MASK_STATUS status) throws myException {
    	...
    	SurfaceSettingsContainer.rb_get(BEWEGUNG_STATION.kontrollmenge).set_Enabled(false);
    	if (!this.is_physical) {

...
public class FZ_LIST_btDeleteVektor4Develop extends E2_Button {

	private E2_NavigationList  f_navilist = null;
	
	public FZ_LIST_btDeleteVektor4Develop(E2_NavigationList  navilist) throws myException {
		super();
		this._image(E2_ResourceIcon.get_RI("delete_red.png"));
		this.f_navilist=navilist;
		this._aaa(new ownActionDelete())._ttt("Löschfunktion (löscht direkt, nur im Entwicklermodus zugelassen) !");
	...
	private class ownActionDelete extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			FZ_LIST_btDeleteVektor4Develop oThis = FZ_LIST_btDeleteVektor4Develop.this;
			
			Vector<String>  v_id = oThis.f_navilist.get_vSelectedIDs_Unformated();
			
			if (bibALL.get_RECORD_USER().is_DEVELOPER_NO()) {
				bibMSG.add_MESSAGE(new MyE2_Alarm_Message(new MyE2_String("ACHTUNG! NUR ZU TESTZWECKEN IM ENTWICKLERMODUS !!!")));
				return;
			}
	...
	private Vector<String> delete_row(String id_bewegung_vektor, MyE2_MessageVector  mv_r) throws myException {
		...
		for (RECORD_BEWEGUNG_VEKTOR_POS p: l_p) {
			...
			for (RECORD_BEWEGUNG_ATOM  a: l_a) {
				for (RECORD_BEWEGUNG_STATION s: a.get_DOWN_RECORD_LIST_BEWEGUNG_STATION_id_bewegung_atom()) {
					v_sql.add(s.get_DELETE_STATEMENT());
				}
				v_sql.add(a.get_DELETE_STATEMENT());
			}
			v_sql.add(p.get_DELETE_STATEMENT());
		}
		v_sql.add(v.get_DELETE_STATEMENT());
		...
		if (b.get_DOWN_RECORD_LIST_BEWEGUNG_VEKTOR_id_bewegung().size()==1) {
			v_sql.add(b.get_DELETE_STATEMENT());
	...
	private class ownValidatorOnlyAtom extends XX_ActionValidator_NG {
		...
		public MyE2_MessageVector isValid(Component oComponentWhichIsValidated) throws myException {
			...
			Vector<String> v_ids = FZ_LIST_btDeleteVektor4Develop.this.f_navilist.get_vSelectedIDs_Unformated();

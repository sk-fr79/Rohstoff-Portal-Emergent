...
public class ATOM_LAG_BEW_StatusErmittlung_old implements ICallableTask{
	...
	public ATOM_LAG_BEW_StatusErmittlung_old(boolean bKostenberuecksichtigung) {
		super();
	 ...
	 */
	private void readLagerstatus( boolean bKostenberuecksichtigung ){
		...
		boolean bSelectAllArticles = false;
		...
		String 	sSelect = 
				...
				+ " S.ID_ADRESSE,  "
				+ 	sColArtikel 
				+ " , sum( nvl(A.MENGE_NETTO,0) - nvl(T1.WE_VERSETZT,0) ) as  menge_gesamt "
				+ " , sum ( case when A.E_PREIS_RESULT_NETTO_MGE is null then 0 else nvl(A.MENGE_NETTO,0) - nvl(T1.WE_VERSETZT,0) end ) as menge_bepreist  "
				+ " , sum ( case when A.E_PREIS_RESULT_NETTO_MGE is not null then 0 else  nvl(A.MENGE_NETTO,0) - nvl(T1.WE_VERSETZT,0) end ) as menge_nullpreis "
				+ " , round ( sum ( ((nvl(A.MENGE_NETTO,0) - nvl(T1.WE_VERSETZT,0)) / ART.MENGENDIVISOR) * nvl(" + m_sqlColumPreis + ",0) ),2) as wert_gesamt "
				+ " , round ( sum ( ((nvl(A.MENGE_NETTO,0) - nvl(T1.WE_VERSETZT,0)) / ART.MENGENDIVISOR) * nvl(" + m_sqlColumPreis + ",0) ) / nvl(sum( (nvl(A.MENGE_NETTO,0) - nvl(T1.WE_VERSETZT,0)) / ART.MENGENDIVISOR) ,1),2) as epreis_avg_all "
				+ " , round ( sum ( ((nvl(A.MENGE_NETTO,0) - nvl(T1.WE_VERSETZT,0)) / ART.MENGENDIVISOR) * " + m_sqlColumPreis + ") / nvl(sum ( (case when A.E_PREIS_RESULT_NETTO_MGE is null then 0 else  (nvl(A.MENGE_NETTO,0) - nvl(T1.WE_VERSETZT,0))  end)  / ART.MENGENDIVISOR)  ,1),2) as epreis_avg_bepreist "
		...
		fillDataInRow(sArrDaten, bSelectAllArticles);
	 ...
	 * @param sArrDaten
	 * @param bSelectAllArticles - wird verwendet, wenn alle Status-Daten aller Lager/Sorten auf einmal gelesen werden
	 * 					- wenn true, werden auch gleichzeitig negative Lagerbestände ermittelt und die fehlenden Einträge ersetzt
	 */
	private void fillDataInRow(String[][] sArrDaten, boolean bSelectAllArticles) {
		...
		if(bSelectAllArticles){
			try {
				// lesen der Saldodaten zum Zeitpunkt der Statusermittlung
				oLagersaldo.readSaldoDaten(null, null, m_BuchungsdatumBis);

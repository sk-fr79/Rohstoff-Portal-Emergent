...
public class ATOM_LAG_INV_LIST_FORMATING_Agent extends XX_ComponentMAP_SubqueryAGENT 
{
...
//		BigDecimal bdSaldo = oSaldoDaten.get_Saldo();
		...
		btnOK = (MyE2_ButtonInLIST)oMAP.get__Comp(ATOM_LAG_INV_CONST.HASH_COL_ATOM_LAGERSALDO_OK);
		if (bSaldoOK){
		...
		btnLagerwerte = (MyE2_ButtonInLIST)oMAP.get__Comp(ATOM_LAG_INV_CONST.HASH_COL_ATOM_SETZKASTEN_NEU_AUFBAUEN);
...
//		lblCol.setText(sData);
		...
		// Saldo Differenz
		lblCol = (MyE2_Label)oMAP.get__Comp(ATOM_LAG_INV_CONST.HASH_COL_ATOM_LAGERSALDO_DIFFERENZ);
...
//			oReorg.Reorganize_Lager_WithProcessBar();
			bibMSG.add_MESSAGE(new MyE2_Alarm_Message("Noch nicht implementiert"));
		}
	}
	...
	private class ownActionPopup extends XX_ActionAgent
	{
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException 
		{
			...
			new ATOM_LAG_Umbuchung_Erfassung(m_IDLager, m_IDSorte, m_Menge, m_DateBuchung, ATOM_LAG_INV_LIST_FORMATING_Agent.this.m_navList,m_bKorrektur);
		}

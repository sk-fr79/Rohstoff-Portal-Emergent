 ...
 *
 */
public class ATOM_SALDO_LIST_SqlFieldMAP__OLD extends Project_SQLFieldMAP 
{
	...
	public ATOM_SALDO_LIST_SqlFieldMAP__OLD() throws myException 
	{
		super("JT_BEWEGUNGSATOM", "", false);
		
		this.add_JOIN_Table("JT_ARTIKEL", 
				"JT_ARTIKEL", 
				SQLFieldMAP.LEFT_OUTER, 
				":ANR1:ARTBEZ:ID_ARTIKEL:", true, "JT_BEWEGUNGSATOM.ID_ARTIKEL = JT_ARTIKEL.ID_ARTIKEL", "", 
		...
		this.add_JOIN_Table("JT_ADRESSE", 
				...
				, true, "ID_LAGER=JT_ADRESSE.ID_ADRESSE", "", 
//			bibALL.get_HashMap("ADDRESS_INFO", " NVL(JT_ADRESSE.PLZ,'') ||' '|| NVL(JT_ADRESSE.ORT,'') ||', ' || NVL(JT_ADRESSE.NAME1,'') ||' ' || NVL(JT_ADRESSE.NAME2,'')"));
		...
		// Lagerbestands-ermittlung SQL-Statement aus ATOM_SALDO_LagerSaldoErmittlung entnommen
		...
		String sWhere = " NVL(JT_BEWEGUNGSATOM.STORNIERT,'N') = 'N' AND" +
	   " JT_BEWEGUNGSATOM.ID_BEWEGUNGSATOM = ( " +
       ...
       " )" ;
		...
		/*	
		E2_DropDownSettings oDDWichtigkeit = new E2_DropDownSettings("JT_LAGER_KONTO_WICHTIGKEIT","BESCHREIBUNG","ID_ATOM_SALDO_WICHTIGKEIT","ISTSTANDARD",true);
		this.get_("ID_ATOM_SALDO_WICHTIGKEIT").set_cDefaultValueFormated(oDDWichtigkeit.getDefault());
		*/

...
public class EAKC_LIST_ModuleContainer extends E2_BasicModuleContainer
{
	...
	private EAKC_LIST_COMPONENT_MAP 		oMap = null;  
	
	private E2_NavigationList 				oNaviList = null;
	
	
	private MyE2_Label  					labelBranchenText = new MyE2_Label("");
	private MyE2_Label  					labelGruppenText = new MyE2_Label("");
	
	private MyE2_Label  					labelBranchenCode = new MyE2_Label("");
	private MyE2_Label  					labelGruppenCode = new MyE2_Label("");

	
	public EAKC_LIST_ModuleContainer(EAK_BasicModuleContainer oTopContainer) throws myException
	{
		super();
		//this.set_MODUL_IDENTIFIER(E2_MODULNAMES.NAME_MODUL_EAK+"_CODE");
		//2015-10-08: neue benennung
		this.set_MODUL_IDENTIFIER(E2_MODULNAME_ENUM.MODUL.MODUL_EAK_CODE.get_callKey());
		
		this.set_bVisible_Row_For_Messages(false);

		this.labelBranchenText.setFont(new E2_FontPlain(-2));
		this.labelGruppenText.setFont(new E2_FontPlain(-2));
		this.labelBranchenCode.setFont(new E2_FontPlain(-2));
		this.labelGruppenCode.setFont(new E2_FontPlain(-2));
		...
		gridHelp.add(new MyE2_Label(new MyE2_String("Gruppe:"),MyE2_Label.STYLE_SMALL_PLAIN()));
		gridHelp.add(this.labelGruppenCode,oGL);
		gridHelp.add(this.labelGruppenText,oGL);
		
		this.oNaviList = new E2_NavigationList();

		
		this.oMap = new EAKC_LIST_COMPONENT_MAP(oTopContainer,this.oNaviList);
		...
		this.oBedienPanel.add(new EAKC_ListSearch(oMap,oNaviList));
	...
	public void set_Gruppe(String cID_Gruppe_Unformatiert) throws myException
	{
		SQLFieldMAP oSQLFieldMap = this.oMap.get_oSQLFieldMAP();
		if (!bibALL.isEmpty(cID_Gruppe_Unformatiert) && bibALL.isInteger(cID_Gruppe_Unformatiert))
		{
			((SQLFieldForRestrictTableRange)oSQLFieldMap.get_("ID_EAK_GRUPPE")).set_cRestrictionValue_IN_DB_FORMAT(cID_Gruppe_Unformatiert);
			this.oBasicEditPanel.set_BUTTON_STATUS_VIEW();
		}
		else
		{
			((SQLFieldForRestrictTableRange)oSQLFieldMap.get_("ID_EAK_GRUPPE")).set_cRestrictionValue_IN_DB_FORMAT("NULL");
			this.oBasicEditPanel.set_ALL_ButtonsDisabled();
		}
	...
	public MyE2_Label get_labelGruppenText()
	{
		return labelGruppenText;
	}


	public MyE2_Label get_labelBranchenCode()
	{
		return labelBranchenCode;
	}


	public MyE2_Label get_labelGruppenCode()
	{
		return labelGruppenCode;
	}

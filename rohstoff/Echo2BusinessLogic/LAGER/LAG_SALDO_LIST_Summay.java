...
public class LAG_SALDO_LIST_Summay extends E2_ListSelectorContainer
{
...
//	private cFieldList      m_fieldList_caption   	= new cFieldList();
	...
	private  TreeMap<String, cFieldRow>  m_oRows = new TreeMap<String, LAG_SALDO_LIST_Summay.cFieldRow>();
	...
	public LAG_SALDO_LIST_Summay(E2_NavigationList oNavigationList, LAG_LagerSaldoErmittlung oSaldoErmittlung) throws myException
	{
...
//		}
		...
		for (int i = 0; i< iSegmente; i++){
			...
			String sSql = "";
			sSql = "SELECT l.ID_ADRESSE_LAGER, l.ID_ARTIKEL_SORTE, e.EINHEITKURZ "
				+ " FROM " + bibE2.cTO() + ".JT_LAGER_KONTO l "
				+ " INNER JOIN " + bibE2.cTO() + ".JT_ARTIKEL art on l.ID_ARTIKEL_SORTE = art.ID_ARTIKEL "
				+ " INNER JOIN " + bibE2.cTO() + ".JT_EINHEIT e   on e.ID_EINHEIT = art.ID_EINHEIT "
			    + " WHERE l.ID_LAGER_KONTO in ( "
				+ sIDs
				+ " ) ";
			...
			for (int j = 0; j < cLagerDaten.length; j++) {
				...
				LAG_LagerSaldoDaten oDaten = m_SaldoErmittlung.getData(sID_Adresse, sID_Sorte);
	...
	private class cFieldList {
		...
		public cFieldList() {
			m_fieldList = new ArrayList<LAG_SALDO_LIST_Summay.cField>();
		}

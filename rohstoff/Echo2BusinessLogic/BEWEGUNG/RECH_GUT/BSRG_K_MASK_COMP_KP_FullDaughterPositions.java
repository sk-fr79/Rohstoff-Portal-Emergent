 ...
 */
public class BSRG_K_MASK_COMP_KP_FullDaughterPositions extends XX_FULL_DAUGHTER
{

	private	BSRG_K_MASK__ModulContainer 	oModulContainer_MASK_InTheTOP = null;
	...
	private BSRG_P_LIST__ModulContainer	oModulContainer_LIST_VorgangsPosition = null;
	private BSRG_P_MASK__ModulContainer	oModulContainer_MASK_VorgangsPosition = null;
	
	
	public BSRG_K_MASK_COMP_KP_FullDaughterPositions(	SQLFieldForPrimaryKey 		osqlField, 
											BSRG_K_MASK__ModulContainer 	oMaskContainerInTheTOP) throws myException
	{
		super(osqlField);
	
		this.oModulContainer_MASK_InTheTOP = oMaskContainerInTheTOP;
		
		this.oModulContainer_MASK_VorgangsPosition = new BSRG_P_MASK__ModulContainer(oMaskContainerInTheTOP.get_SETTING());
		
		
		this.oModulContainer_LIST_VorgangsPosition = new BSRG_P_LIST__ModulContainer(oMaskContainerInTheTOP,this.oModulContainer_MASK_VorgangsPosition);
	...
	public Component build_content_for_Mask(String cValueFormated, String cValueUnformated, String cMASK_STATUS) throws myException
	{
		...
		else if (cMASK_STATUS.equals(E2_ComponentMAP.STATUS_EDIT))
		{
			...
			if (this.oModulContainer_MASK_InTheTOP.get_oBSRG_K_ModulContainerLIST() != null)
			{
				bZeigeGeloeschte =this.oModulContainer_MASK_InTheTOP.get_oBSRG_K_ModulContainerLIST().get_oSelektor().get_oCB_ShowDeletedRows().isSelected();
			}
			this.oModulContainer_LIST_VorgangsPosition.get_oComponentMAP_List().get_oSQLFieldMAP().clear_BEDINGUNG_STATIC();
			if (! bZeigeGeloeschte)
				this.oModulContainer_LIST_VorgangsPosition.get_oComponentMAP_List().get_oSQLFieldMAP().add_BEDINGUNG_STATIC("  NVL(JT_VPOS_RG.DELETED,'N')='N'");
			
			this.oModulContainer_LIST_VorgangsPosition.set_ID_VKOPF_RG_IN_Position(cValueUnformated);
	...
	{
		this.removeAll();
		//this.add(new MyE2_Label(new MyE2_String("Positionen kï¿½nen erst erfasst werden, wenn der Kopfsatz gespeichert wurde!")));
	...
	private class ownActionAgent extends XX_ActionAgent
	{
		...
		{
			E2_SaveMaskStandard oSaveMask = new E2_SaveMaskStandard(BSRG_K_MASK_COMP_KP_FullDaughterPositions.this.oModulContainer_MASK_InTheTOP, 
												BSRG_K_MASK_COMP_KP_FullDaughterPositions.this.oNaviList_VKOPF);

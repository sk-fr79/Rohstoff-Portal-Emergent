...
public class BSA_K_LIST_MarkerSubQueryAgent extends XX_ComponentMAP_SubqueryAGENT {
	...
	{ 
		...
		MyE2_Label oLabLocked =   (MyE2_Label)oMAP.get__Comp(BSA__CONST.HASH_KEY_ANZEIGE_LOCKED);

		
		// jetzt den locked-button verarzten
		if (oUsedResultMAP.get_FormatedValue("ABGESCHLOSSEN").equals("Y"))
			oLabLocked.setIcon(BSA__CONST.LABEL_POSITION_LOCKED);
		else
			oLabLocked.setIcon(BSA__CONST.LABEL_EMPTY);
		
		
		

		MyE2_Row oRow = (MyE2_Row)oMAP.get__Comp(BSA__CONST.HASH_KEY_ANZEIGE_POSITIONS);
		oRow.removeAll();
		String cID_VKOPF = oUsedResultMAP.get_cUNFormatedROW_ID();
		
		String cQuery1 =   "SELECT COUNT(*) FROM "+bibE2.cTO()+".JT_VPOS_STD WHERE   NVL(DELETED,'N')='N' AND  ID_VKOPF_STD="+cID_VKOPF+" AND POSITION_TYP<>'"+myCONST.VG_POSITION_TYP_ZUSATZTEXT+"'";
		String cQuery2 =   "SELECT COUNT(*) FROM "+bibE2.cTO()+".JT_VPOS_STD WHERE   NVL(DELETED,'N')='N' AND   ID_VKOPF_STD="+cID_VKOPF+" AND POSITION_TYP<>'"+myCONST.VG_POSITION_TYP_ZUSATZTEXT+"' AND EINZELPREIS IS NOT NULL";

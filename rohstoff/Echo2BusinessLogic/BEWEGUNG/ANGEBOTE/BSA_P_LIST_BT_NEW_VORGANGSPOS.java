...
public class BSA_P_LIST_BT_NEW_VORGANGSPOS extends MyE2_ButtonWithKey
{

	public BSA_P_LIST_BT_NEW_VORGANGSPOS(E2_NavigationList onavigationList,BSA_P_MASK__ModulContainer oPositionModulContainerMASK, BSA_K_MASK__ModulContainer oKopfMaskContainer)
	{
		...
		this.setToolTipText(new MyE2_String("Neues Position eingeben").CTrans());
	}
	...
	private class ownActionAgent extends ButtonActionAgentNEW
	{
		...
		public void do_innerAction() throws myException
		{
			...
			SQLFieldForRestrictTableRange oSQLField_ID_HEAD = (SQLFieldForRestrictTableRange)oSQLFieldMAP.get_("ID_VKOPF_STD");
			
			String cID_VKOPF = oSQLField_ID_HEAD.get_cRestictionValue_IN_DB_FORMAT();
			
			if (bibDB.EinzelAbfrage("SELECT   NVL(DELETED,'N') FROM "+bibE2.cTO()+".JT_VKOPF_STD WHERE ID_VKOPF_STD="+cID_VKOPF).equals("Y"))
			{

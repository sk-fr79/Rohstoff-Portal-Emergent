 ...
 */
public class BST_K_MASK_COMP_KP_FullDaughterPositions extends XX_FULL_DAUGHTER
{

	private	BST_K_MASK__ModulContainer 	oModulContainer_MASK_InTheTOP = null;
	...
	private BST_P_LIST__ModulContainer	oModulContainer_LIST_VorgangsPosition = null;
	private BST_P_MASK__ModulContainer	oModulContainer_MASK_VorgangsPosition = null;
	
	
	public BST_K_MASK_COMP_KP_FullDaughterPositions(	SQLFieldForPrimaryKey 		osqlField, 
														BST_K_MASK__ModulContainer 	oMaskContainerInTheTOP,
														boolean 					bAddOpenTPA_Button) throws myException
	{
		super(osqlField);
	
		this.oModulContainer_MASK_InTheTOP = oMaskContainerInTheTOP;
		this.oModulContainer_MASK_VorgangsPosition = new BST_P_MASK__ModulContainer(oMaskContainerInTheTOP,bAddOpenTPA_Button);
		this.oModulContainer_LIST_VorgangsPosition = new BST_P_LIST__ModulContainer(oMaskContainerInTheTOP,this.oModulContainer_MASK_VorgangsPosition);
		this.set_bIsActive(true);

		//2018-05-28: 
		this.oModulContainer_LIST_VorgangsPosition.get_oListBedienPanel().getoButNEW().add_oActionAgent(new OwnActionAgent());
	...
	public Component build_content_for_Mask(String cValueFormated, String cValueUnformated, String cMASK_STATUS) throws myException
	{
		...
		else if (cMASK_STATUS.equals(E2_ComponentMAP.STATUS_EDIT))
		{
			...
			if (this.oModulContainer_MASK_InTheTOP.get_oBST_K_ModulContainerLIST() != null)
			{
				bZeigeGeloeschte =this.oModulContainer_MASK_InTheTOP.get_oBST_K_ModulContainerLIST().get_oSelektor().get_oCB_ShowDeletedRows().isSelected();
			}
			this.oModulContainer_LIST_VorgangsPosition.get_oComponentMAP_List().get_oSQLFieldMAP().clear_BEDINGUNG_STATIC();
			if (! bZeigeGeloeschte)
				this.oModulContainer_LIST_VorgangsPosition.get_oComponentMAP_List().get_oSQLFieldMAP().add_BEDINGUNG_STATIC("  NVL(JT_VPOS_TPA.DELETED,'N')='N'");

			
			
			this.oModulContainer_LIST_VorgangsPosition.set_ID_VKOPF_TPA_IN_Position(cValueUnformated);
	...
	private class saveButtonAndReload extends MyE2_Button
	{
		...
		{
			super(new MyE2_String("TPA-Positionen können erst erfasst werden, wenn der Kopfsatz gespeichert wurde!"));
	...
	private class ownActionAgent extends XX_ActionAgent
	{
		...
		{
			E2_SaveMaskStandard oSaveMask = new E2_SaveMaskStandard(BST_K_MASK_COMP_KP_FullDaughterPositions.this.oModulContainer_MASK_InTheTOP, 
												BST_K_MASK_COMP_KP_FullDaughterPositions.this.oNaviList_VKOPF);
			...
			try
			{
				...
				if (bibMSG.get_bIsOK())
				{
					bibMSG.add_MESSAGE(new MyE2_Info_Message("Speichern OK"), false);
				}
			}
	...
	public BST_P_LIST__ModulContainer get_oModulContainer_LIST_VorgangsPosition()
	{
		return oModulContainer_LIST_VorgangsPosition;
	}

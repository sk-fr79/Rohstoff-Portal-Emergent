...
public class FU__MASK_CB_OHNE_ABRECHNUNG extends MyE2_DB_CheckBox 
{

	public FU__MASK_CB_OHNE_ABRECHNUNG(SQLField osqlField) throws myException 
	{
		super(osqlField);
		this.add_oActionAgent(new ownActionAgent());
		this.add_GlobalValidator(new ownValidator());
		this.add_GlobalAUTHValidator_AUTO("SETZE_FUHRE_OHNE_ABRECHNUNG");
		this.EXT().set_bDisabledFromBasic(true);
		this.setToolTipText(new MyE2_String("Zeigt an, dass in dieser Fuhre keine Buchungssätze auf die gefahrene Sorte erlaubt sind (z.b. reine Fahraufträge)").CTrans());
	...
	private class ownValidator extends XX_ActionValidator
	{
		...
		{
			FU__MASK_CB_OHNE_ABRECHNUNG oThis = FU__MASK_CB_OHNE_ABRECHNUNG.this;
			...
			if (!bRueck)
			{
				oMV.add_MESSAGE(new MyE2_Alarm_Message(new MyE2_String("Nur möglich, wenn keine Positionen zu dieser Position gebucht wurden !")));
			}
			return oMV;
		}

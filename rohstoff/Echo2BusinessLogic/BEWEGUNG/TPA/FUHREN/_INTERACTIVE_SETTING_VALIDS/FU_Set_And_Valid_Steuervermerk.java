...
import rohstoff.Echo2BusinessLogic.BEWEGUNG.TPA.FUHREN.FU_MASK_SelectTaxViaPopup;
...
public class FU_Set_And_Valid_Steuervermerk extends XX_MAP_Set_And_Valid
{
	...
	public FU_Set_And_Valid_Steuervermerk(E2_ComponentMAP oMAP,String cEK_VK) throws myException
	{
		super();
	...
	private MyE2_MessageVector make_Checks_and_Settings(E2_ComponentMAP oMAP, int ActionType, ExecINFO oExecInfo) throws myException
	{
		...
		// felder beschaffen
		MyE2IF__DB_Component 				oSEL_ID_TAX = 			(MyE2IF__DB_Component)oMAP.get__Comp(RECORD_VPOS_TPA_FUHRE.FIELD__ID_TAX_EK);
		FU__MASK_SELECT_FIELD_STEUERVERMERK oSEL_SteuerVermerk = 	(FU__MASK_SELECT_FIELD_STEUERVERMERK)oMAP.get__Comp(FU___CONST.HASH_KEY_BUTTON_MASK_DROPDOWN_STEUERTEXT_EK);
		MyE2_DB_TextArea   					oTF_SteuerVermerk =  	(MyE2_DB_TextArea)oMAP.get__Comp(RECORD_VPOS_TPA_FUHRE.FIELD__EU_STEUER_VERMERK_EK);
		BS_ComboBox_MWST 					oCBB_Steuer = 			(BS_ComboBox_MWST)oMAP.get__Comp(RECORD_VPOS_TPA_FUHRE.FIELD__STEUERSATZ_EK);
		
		String  							HASHNAME_OF_oSELSTEUERTEXTE = 	FU___CONST.HASH_KEY_BUTTON_MASK_DROPDOWN_STEUERTEXT_EK;
		String  							HASHNAME_OF_oSEL_ID_TAX =  		RECORD_VPOS_TPA_FUHRE.FIELD__ID_TAX_EK;
		
		if (!this.bEK)
		{
			//2018-07-05: neues optionales steuerselektionsobjekt
			oSEL_ID_TAX = 					(MyE2IF__DB_Component)oMAP.get__Comp(RECORD_VPOS_TPA_FUHRE.FIELD__ID_TAX_VK);
			oSEL_SteuerVermerk = 			(FU__MASK_SELECT_FIELD_STEUERVERMERK)oMAP.get__Comp(FU___CONST.HASH_KEY_BUTTON_MASK_DROPDOWN_STEUERTEXT_VK);
			oTF_SteuerVermerk = 			(MyE2_DB_TextArea)oMAP.get__Comp(RECORD_VPOS_TPA_FUHRE.FIELD__EU_STEUER_VERMERK_VK);
			oCBB_Steuer = 					(BS_ComboBox_MWST)oMAP.get__Comp(RECORD_VPOS_TPA_FUHRE.FIELD__STEUERSATZ_VK);
			HASHNAME_OF_oSELSTEUERTEXTE = 	FU___CONST.HASH_KEY_BUTTON_MASK_DROPDOWN_STEUERTEXT_VK;
			HASHNAME_OF_oSEL_ID_TAX = 		RECORD_VPOS_TPA_FUHRE.FIELD__ID_TAX_VK;
		}
		...
		if 		(ActionType==XX_MAP_Set_And_Valid.USE_IN_MASK_KLICK_ACTION)
		{
			...
			if (oExecInfo.get_HASHKEY_of_KLICK_COMPONENT().equals(HASHNAME_OF_oSEL_ID_TAX))
			{
				...
				if (S.isFull(oSEL_ID_TAX.get_cActualDBValueFormated()))
				{
					...
					this.setzeSteuerUndTextWegen_ID_TAX(oSEL_ID_TAX, oTF_SteuerVermerk, 
										new MyDate(oMAP.get_cActualDBValueFormated(this.bEK?VPOS_TPA_FUHRE.datum_aufladen.fn():VPOS_TPA_FUHRE.datum_abladen.fn())), oCBB_Steuer);
				}
				else
				{
				...
				if (S.isFull(oSEL_SteuerVermerk.get_ActualWert()))
				{
					...
					if (oSEL_ID_TAX instanceof MyE2_DB_SelectField) {
					   ((MyE2_DB_SelectField)oSEL_ID_TAX).set_ActiveValue("");
						...
						((FU_MASK_SelectTaxViaPopup)oSEL_ID_TAX).set_cActualMaskValue("");
					}
		...
		else if		(ActionType==XX_MAP_Set_And_Valid.USE_IN_MASK_LOAD_ACTION)
		{
			...
			//2018-07-05: neuer selektor fuer steuersaetze
			if (S.isFull(oSEL_ID_TAX.get_cActualDBValueFormated()))
			{
				oMAP.set_ActiveADHOC(HASHNAME_OF_oSELSTEUERTEXTE, false, false);
			}
		...
		else if (ActionType==XX_MAP_Set_And_Valid.USE_IN_MASK_VALID_ACTION)
		{
			...
			try {
				...
				MyDate 			leistungsDatum  =		new MyDate(oMAP.get_cActualDBValueFormated(this.bEK?VPOS_TPA_FUHRE.datum_aufladen.fn():VPOS_TPA_FUHRE.datum_abladen.fn()));
					...
					if (actuellerSteuersatz.isOK()) {
						...
						if (!bdSteuer.getUfRounded(2).equals(actuellerSteuersatz.getUfRounded(2))) {
							oMV._addAlarm(S.ms("Achtung! Die Steuersetzung ist falsch! ")
												.t(this.bEK?"(Ladeseite) ":"(Abladeseite) ")
				...
				e.printStackTrace();
				oMV._addAlarm("Error reading Tax-value ! <2b490f16-b56a-11ea-b3de-0242ac130004>");
			}

...
public class FU_LIST_BT_EditLieferbedingung extends DBRB_BUTTON_ToEditSomeFields {
 
	private FU_AL_SucheLieferBed  oSucheLieferBed = null;
	private E2_ComponentMAP       oMapFuhre = null;

	public FU_LIST_BT_EditLieferbedingung(	String 									fieldName, 
											FU_AL_SucheLieferBed 					sucheLieferBed, 
											FU_AL_LieferbedZeigeAktuellStatistik 	sucheAktuelleStatistik,  
											E2_ComponentMAP 						map_Fuhre) throws myException {
		super("<lieferbed>",MyE2_Button.StyleButtonCenteredWithDDarkBorder());
		
		this.oSucheLieferBed = sucheLieferBed;
		this.oMapFuhre = map_Fuhre;
		this.set_cFieldName(fieldName);

		FU_AL_Erzeuge_ButtonTextUndToolTips oGenerator = new FU_AL_Erzeuge_ButtonTextUndToolTips(sucheLieferBed, sucheAktuelleStatistik);
		
		MyE2_String cTextAufButton = 	oGenerator.get_cTextAufButton();
		String 		cToolTips = 		oGenerator.get_cToolTips();
		
		boolean bEK = fieldName.equals(_DB.VPOS_TPA_FUHRE$LIEFERBED_ALTERNATIV_EK);
		...
		MyE2_String cBeschriftung = bEK 
									?
						 			new MyE2_String("Lieferbedingung WE-Seite: ")
									:
						 			new MyE2_String("Lieferbedingung WA-Seite: ");

		this.set_Text(cTextAufButton);
		this.setToolTipText(new MyE2_String("Lieferbedingung der Hauptfuhre erfassen ("+(bEK?"WE-Seite)":"WA-Seite)"),true,"  ----  ",false,cToolTips,false).CTrans());
		...
		this.set_oActionAddToSaveButton(new XX_ActionAgent()
		{
			...
			{
				FU_LIST_BT_EditLieferbedingung.this.oMapFuhre._DO_REFRESH_COMPONENTMAP(E2_ComponentMAP.STATUS_VIEW, true, false);
			}
		});				 			  
	...
	private class ownActionAgent extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			...
			if (FU_LIST_BT_EditLieferbedingung.this.oSucheLieferBed.get_bIS_MANDANT()) {
				...
				String cID_VPOS_TPA_FUHRE = FU_LIST_BT_EditLieferbedingung.this.oMapFuhre.get_oInternalSQLResultMAP().get_cUNFormatedROW_ID();
				MyE2_Grid oGrid = FU_LIST_BT_EditLieferbedingung.this.BaueMaskenGrid(new RECORD_VPOS_TPA_FUHRE(cID_VPOS_TPA_FUHRE), oBasicContainer);
	...
	public MyE2_Grid createGrid4PopupWindow(MyE2_String cFeldBeschriftung, Component oComponent4Edit, Component oComponent4UserInfo, MyE2_Button btSAVE, MyE2_Button btCANCEL, E2_BasicModuleContainer oContainer4Popup) throws myException {
		...
		FU_AL_SucheLieferBed oSucheLB =  FU_LIST_BT_EditLieferbedingung.this.oSucheLieferBed;

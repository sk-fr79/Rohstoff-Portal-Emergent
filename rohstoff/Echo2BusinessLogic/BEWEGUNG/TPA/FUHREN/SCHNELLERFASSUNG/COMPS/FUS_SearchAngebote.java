...
public class FUS_SearchAngebote extends MyE2_MaskSearchField implements __FUS_STANDARD_Element
{

	public static  String cSQL4Label = 
						"SELECT NVL(JT_VKOPF_STD.BUCHUNGSNUMMER,'??')||'-'||NVL(JT_VPOS_STD.POSITIONSNUMMER,'')||' '||"+
						"'['||NVL(JT_VPOS_STD.ANR1,'')||'-'||NVL(JT_VPOS_STD.ANR2,'')||'] '||NVL(JT_VKOPF_STD.NAME1,'')||' '||NVL(JT_VKOPF_STD.ORT,'')||' - '||" +
								" TO_CHAR(JT_VPOS_STD.ANZAHL,'999G999G999', 'NLS_NUMERIC_CHARACTERS='',.''')||' '||NVL(JT_VPOS_STD.EINHEITKURZ,'??') ||' '|| NVL(JT_VPOS_STD.ARTBEZ1,'')||''||'   ('||"+
						"    NVL(TO_CHAR(JT_VPOS_STD_ANGEBOT.GUELTIG_VON,'dd.mm.'),'-')||' - '||"+
						"    NVL(TO_CHAR(JT_VPOS_STD_ANGEBOT.GUELTIG_BIS,'dd.mm.'),'-')||')'"+
						" FROM "+
						"    JT_VKOPF_STD, JT_VPOS_STD,JT_VPOS_STD_ANGEBOT "+
						" WHERE "+
						"    JT_VKOPF_STD.ID_VKOPF_STD   = JT_VPOS_STD.ID_VKOPF_STD AND JT_VPOS_STD.ID_VPOS_STD = JT_VPOS_STD_ANGEBOT.ID_VPOS_STD AND " +
						"    JT_VPOS_STD.ID_VPOS_STD=#WERT#";	

	private String cEK_VK = null;
	

	public FUS_SearchAngebote(String EK_VK) throws myException
	{
		super(new FUS_SearchAngebote__SearchBlock(), FUS_SearchAngebote.cSQL4Label, E2_INSETS.I_0_0_0_0, true);
		
		((FUS_SearchAngebote__SearchBlock)this.get_oSeachBlock()).set_oFUS_SearchAngebote(this);
		...
		this.set_oMaskActionAfterMaskValueIsFound(new XX_MaskActionAfterFoundNonDB()
		{
			...
			{
				MyLong ID_VPOS_STD = new MyLong(cMaskValue);
				
				if (!ID_VPOS_STD.get_cErrorCODE().equals(MyLong.ALL_OK))
				{
					throw new myException(this, "Search-Result is not allowed !!!");
				}

				
				new __FUS_ActionAfterFound(FUS_SearchAngebote.this);
				FUS_SearchAngebote.this.do_afterFieldWasFilled(""+ID_VPOS_STD.get_lValue());
	...
	public class actionAgentAfterRadiergummi extends XX_ActionAgent
	{
		...
		{
			new _SEARCH_PreisEingabe().get_found_PreisFeld(FUS_SearchAngebote.this.get_bIS_EK()).set_bEnabled_For_Edit(true);
			new _SEARCH_PreisEingabe().get_found_PreisFeld(FUS_SearchAngebote.this.get_bIS_EK()).setText("");
		}
	}
	...
	public RECORD_VPOS_STD get_Actual_RECORD_VPOS_STD() throws myException
	{
		RECORD_VPOS_STD recRueck = null;
		
		if (this.get_bIsCorrectFilled(false))
		{
			MyInteger  intID_VPOS_STD = new MyInteger(this.get_oTextFieldForSearchInput().getText());
			
			if (intID_VPOS_STD.get_cErrorCODE().equals(MyInteger.ALL_OK))
			{
				recRueck = new RECORD_VPOS_STD(intID_VPOS_STD.get_iValue());
			}
		}
		return recRueck;
	}
	...
	public RECORD_VPOS_STD_ANGEBOT get_Actual_RECORD_VPOS_STD_ANGEBOT() throws myException
	{
		RECORD_VPOS_STD recVPOS_STD = this.get_Actual_RECORD_VPOS_STD();
		
		RECORD_VPOS_STD_ANGEBOT recRueck = null;
		
		if (recVPOS_STD!=null)
		{
			recRueck = recVPOS_STD.get_DOWN_RECORD_LIST_VPOS_STD_ANGEBOT_id_vpos_std().get(0);
		}
		return recRueck;
	}
	...
	public RECORD_VKOPF_STD get_Actual_RECORD_VKOPF_STD() throws myException
	{
		RECORD_VPOS_STD recVPOS_STD = this.get_Actual_RECORD_VPOS_STD();
		
		RECORD_VKOPF_STD recRueck = null;
		
		if (recVPOS_STD!=null)
		{
			recRueck = recVPOS_STD.get_UP_RECORD_VKOPF_STD_id_vkopf_std();
		}
		return recRueck;
	}
	...
	{
		RECORD_VPOS_STD  recAngebot = new RECORD_VPOS_STD(cID);


		if (this.get_bIS_EK())
		{
			new _SEARCH_SearchSortenFields().get_Found_EK_SortenField().fill_MaskText_And_Label(recAngebot.get_ID_ARTIKEL_BEZ_cUF());
			//jetzt die abladesorte definieren, wenn die abladestelle mandant ist
			new _SEARCH_SearchSortenFields().get_Found_EK_SortenField().do_afterFieldWasFilled(recAngebot.get_ID_ARTIKEL_BEZ_cUF_NN(""));
		}
		else
		{
			new _SEARCH_SearchSortenFields().get_Found_VK_SortenField().fill_MaskText_And_Label(this.get_Actual_RECORD_VPOS_STD().get_ID_ARTIKEL_BEZ_cUF());
		}
		
		
		new _SEARCH_PreisEingabe().get_found_PreisFeld(FUS_SearchAngebote.this.get_bIS_EK()).set_bEnabled_For_Edit(false);

		//dann den jeweiligen preis eintragen
		new _SEARCH_PreisEingabe().get_found_PreisFeld(FUS_SearchAngebote.this.get_bIS_EK()).setText(recAngebot.get_EINZELPREIS_cF_NN("0,00"));

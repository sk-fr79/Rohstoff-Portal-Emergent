...
public class FU_MASK_BT_ZAUBERSTAB_MENGEN_LADEMENGE_LIEF extends MyE2_Button 
{

	public FU_MASK_BT_ZAUBERSTAB_MENGEN_LADEMENGE_LIEF() 
	{
		super(E2_ResourceIcon.get_RI("wizard_mini.png"),true);
		this.setToolTipText(new MyE2_String("Mengen übertragen auf (leere) Felder Planmenge Ladeseite, Planmenge Abladeseite, (falls nötig) Ablademenge Ladeseite ...").CTrans());
	...
	private class ownActionAgent extends XX_ActionAgent
	{
		...
		{
			E2_ComponentMAP oMap = FU_MASK_BT_ZAUBERSTAB_MENGEN_LADEMENGE_LIEF.this.EXT().get_oComponentMAP();
...
//			boolean bAblademengeRechnung = S.NN(oMap.get__DBComp("ABLADEMENGE_RECHNUNG").get_cActualMaskValue()).equals("Y");
			...
			if (    S.isFull( hmWerte.get("ANTEIL_LADEMENGE_LIEF")))
			{
				if (!bLademengeGutschrift && S.isEmpty(hmWerte.get("ANTEIL_ABLADEMENGE_LIEF")))
				{
					oMap.set_cActualDatabaseValueToMask("ANTEIL_ABLADEMENGE_LIEF", hmWerte.get("ANTEIL_LADEMENGE_LIEF"));
				}
				
				if (	S.isEmpty(hmWerte.get("ANTEIL_PLANMENGE_LIEF"))) 
				{
					oMap.set_cActualDatabaseValueToMask("ANTEIL_PLANMENGE_LIEF", hmWerte.get("ANTEIL_LADEMENGE_LIEF"));
				}

				if (	S.isEmpty(hmWerte.get("ANTEIL_PLANMENGE_ABN"))) 
				{
					oMap.set_cActualDatabaseValueToMask("ANTEIL_PLANMENGE_ABN", hmWerte.get("ANTEIL_LADEMENGE_LIEF"));
				}

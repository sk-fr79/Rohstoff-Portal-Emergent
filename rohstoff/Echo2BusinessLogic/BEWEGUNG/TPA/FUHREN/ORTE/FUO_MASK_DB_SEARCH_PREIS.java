...
import rohstoff.Echo2BusinessLogic.BEWEGUNG.TPA.FUHREN.FU___RECLIST_VPOS_STD_FOR_PREIS;
import rohstoff.Echo2BusinessLogic.BEWEGUNG.TPA.FUHREN.ALT_LIEFBED.FU_AL_Component;

public class FUO_MASK_DB_SEARCH_PREIS extends MyE2_DB_MaskSearchField
{
	
	private String cEK_VK = null;
	
	public FUO_MASK_DB_SEARCH_PREIS(SQLField osqlField, String EK_VK) throws myException
	{
		...
		this.add_ValidatorStartSearch(new XX_ActionValidator()
		{
			...
			public MyE2_MessageVector isValid(Component oCompWhichIsValidated) throws myException
			{
				...
				FUO_MASK_DB_SEARCH_PREIS oThis = FUO_MASK_DB_SEARCH_PREIS.this;
				
				String cID_Adresse = null;
				String cID_Artikel_Bez = null;
				
				cID_Adresse = ""+oThis.EXT().get_oComponentMAP().get_LActualDBValue("ID_ADRESSE", true,true,new Long(0)).longValue();
				cID_Artikel_Bez = ""+oThis.EXT().get_oComponentMAP().get_LActualDBValue("ID_ARTIKEL_BEZ", true,true,new Long(0)).longValue();
				...
				if (cID_Adresse.equals(bibALL.get_ID_ADRESS_MANDANT()))
				{
					...
					E2_ComponentMAP oMap = FUO_MASK_DB_SEARCH_PREIS.this.EXT().get_oComponentMAP();

					//einzelpreis gleich reinholen, manuell raus
					oThis.prepare_ContentForNew(false);
					((MyE2_DB_TextField) oMap.get_hmRealComponents().get("EINZELPREIS")).setText("0");
					((MyE2_DB_CheckBox) oMap.get_hmRealComponents().get("MANUELL_PREIS")).setSelected(true);
				}
				return oMV;
			}
	...
	private class ownSearchBlock extends XX_SearchBlock
	{
		...
		public Vector<XX_Button4SearchResultList[]> get_vResultButtons(String SearchText) throws myException
		{
			...
			long lID_ADRESSE = oThis.EXT().get_oComponentMAP().get_LActualDBValue("ID_ADRESSE", true, true, new Long(-1)).longValue();
			long lID_ARTBEZ = oThis.EXT().get_oComponentMAP().get_LActualDBValue("ID_ARTIKEL_BEZ", true, true, new Long(-1)).longValue();

			MyDate mdLieferdatum = oThis.EXT().get_oComponentMAP().get_ActualMyDate("DATUM_LADE_ABLADE", true, true, null);
			...
			if (lID_ADRESSE>0 && lID_ARTBEZ>0 && S.isFull(cStartDatum) && S.isFull(cEndDatum))
			{
				...
				FU___RECLIST_VPOS_STD_FOR_PREIS recListAngebote = new FU___RECLIST_VPOS_STD_FOR_PREIS(cSQL_DB_VON,cSQL_DB_BIS,cID_ADRESSE,cID_ARTIKEL_UF,oThis.cEK_VK.equals("EK"),null);
...
//		}
	...
	private class ownActionCleanLieferbed extends  XX_MaskActionAfterPrepareContentForNew  {
		...
		public void doMaskSettingsAfterPrepareContentForNew(MyE2_DB_MaskSearchField oDB_SearchField, boolean bSetDefaults) throws myException {
			...
			boolean bManuellPreis = oMAPFuhreORT.get_bActualDBValue(_DB.VPOS_TPA_FUHRE_ORT$MANUELL_PREIS);
			if (!bManuellPreis) {
				((MyE2_DB_TextField) oMAPFuhreORT.get_hmRealComponents().get(_DB.VPOS_TPA_FUHRE_ORT$EINZELPREIS)).setText("");
			}
	...
	private class ownMaskSettings extends XX_MaskActionAfterFound
	{
		public void doMaskSettingsAfterValueWrittenInMaskField(String cMaskValue, MyE2_DB_MaskSearchField oSearchField, boolean bAfterAction, boolean bIS_PrimaryCall) throws myException
		{
			...
			//einzelpreis gleich reinholen, manuell raus
			((MyE2_DB_TextField) oMap.get_hmRealComponents().get("EINZELPREIS")).setText(recVPOS.get_EINZELPREIS_cF_NN(""));
			((MyE2_DB_CheckBox) oMap.get_hmRealComponents().get("MANUELL_PREIS")).setSelected(false);
			...
			if (recVPOS.get_UP_RECORD_VKOPF_STD_id_vkopf_std().is_ABGESCHLOSSEN_NO()  && bibALL.get_RECORD_MANDANT().is_PREISFIND_ANGEB_NUR_GEDRUCKT_YES()) {
				((MyE2_DB_TextField) oMap.get_hmRealComponents().get("EINZELPREIS")).setText("");
				FUO_MASK_DB_SEARCH_PREIS.this.prepare_ContentForNew(false);
			}

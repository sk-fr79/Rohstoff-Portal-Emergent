...
public class FU_MASK_BT_ZAUBERSTAB_MENGEN_ABLADEMENGE_ABN extends MyE2_Button 
{
	public FU_MASK_BT_ZAUBERSTAB_MENGEN_ABLADEMENGE_ABN() 
	{
...
//		this.setToolTipText(new MyE2_String("Planmenge übertragen auf Planmenge Abladeseite ...").CTrans());
		this.setToolTipText(new MyE2_String("Lade-/Ablademenge der linken Seite auf die passenden Felder der rechten Seite übertragen ...").CTrans());
	...
	private class ownActionAgent extends XX_ActionAgent
	{
		...
		{
			E2_ComponentMAP oMap = FU_MASK_BT_ZAUBERSTAB_MENGEN_ABLADEMENGE_ABN.this.EXT().get_oComponentMAP();
...
//			boolean bLademengeGutschrift = S.NN(oMap.get__DBComp("LADEMENGE_GUTSCHRIFT").get_cActualMaskValue()).equals("Y");
			...
			{
				if (S.isEmpty(hmWerte.get("ANTEIL_ABLADEMENGE_ABN")))
				{
					oMap.set_cActualDatabaseValueToMask("ANTEIL_ABLADEMENGE_ABN", hmWerte.get("ANTEIL_LADEMENGE_LIEF"));
				}
				
				if (!bAblademengeRechnung && S.isEmpty(hmWerte.get("ANTEIL_LADEMENGE_ABN")))
				{
					oMap.set_cActualDatabaseValueToMask("ANTEIL_LADEMENGE_ABN", hmWerte.get("ANTEIL_LADEMENGE_LIEF"));
				}

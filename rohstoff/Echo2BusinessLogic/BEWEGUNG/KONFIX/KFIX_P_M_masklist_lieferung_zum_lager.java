...
import panter.gmbh.Echo2.RB.HIGHLEVEL.RB_HL_SelectLagerMandant;
...
import panter.gmbh.basics4project.DB_ENUMS.VPOS_KON_LAGER;
...
public class KFIX_P_M_masklist_lieferung_zum_lager extends RB_MaskDaughter {

	private String id_vpos_kon_actual = null;
	/**
	 * 
	 */
	public KFIX_P_M_masklist_lieferung_zum_lager() {
		super();
		this._set_width_of_mask_inlay_container_ex(new Extent(800));
		this._set_height_of_mask_inlay_container_ex(new Extent(150));
	}

	@Override
	public KFIX_P_M_masklist_lieferung_zum_lager _mark_components_in_innergrid_neutral() throws myException {
		return this;
	}

	@Override
	public KFIX_P_M_masklist_lieferung_zum_lager _mark_components_in_innergrid_disabled() throws myException {
		return this;
	}

	@Override
	public KFIX_P_M_masklist_lieferung_zum_lager _set_components_in_innergrid_enabled_for_edit(boolean b_enabled_4_edit) throws myException {
		return this;
	}

	@Override
	public KFIX_P_M_masklist_lieferung_zum_lager _rb_Datacontent_to_Component(RB_Dataobject dataObject) throws myException {
	...
	@Override
	public KFIX_P_M_masklist_lieferung_zum_lager _rb_set_db_value_manual(String id_vpos_kon) throws myException {
		...
		if (S.isEmpty(id_vpos_kon)) {
			E2_Button  bt_save_mask_first = new E2_Button()._tr("Kontraktposition speichern, um Lagermengen erfassen zu können ")._s_BorderText()._fsa(2)._center()._i();
			bt_save_mask_first.add_oActionAgent(new ownActionSaveAndReopen_vpos_kon());
			
			//ueberschrift
			this.get_inner_grid_4_data()._setSize(30,30,120,400,80)._bo_dd()
						._a(new RB_lab(),						ld_l_t)
						._a(new RB_lab(),						ld_l_t)
						._a(new RB_lab()._tr("Menge"),			ld_r_t)
						._a(new RB_lab()._tr("Lager"),			ld_m_t)
						._a(new RB_lab()._tr("ID"),				ld_r_t)
						;
			this.get_inner_grid_4_data()._a(bt_save_mask_first, new RB_gld()._span(4));
			...
			if (!l_id_vkopf_kon.isOK()) {
				...
				this.get_inner_grid_4_data()._setSize(30,30,120,400,80)._bo_dd()
							...
							;
				this.get_inner_grid_4_data()._clear();
				this.get_inner_grid_4_data()._a(new RB_lab("<Fehler beim Aufbau der Liste>"), ld_m_d._c()._span(5));
			} else {
				...
				//ueberschrift
				this.get_inner_grid_4_data()._setSize(30,30,120,400,80)._bo_dd()
							._a(v_sammle._ar(new own_bt_edit_lagermengen(MASK_STATUS.NEW,"")),	ld_l_t)
							._a(new RB_lab(),								ld_l_t)
							._a(new RB_lab()._tr("Menge"),					ld_r_t)
							._a(new RB_lab()._tr("Lager"),					ld_m_t)
							._a(new RB_lab()._tr("ID"),						ld_r_t)
				...
				RecList20 reclist_lager = new Rec20(_TAB.vpos_kon)._fill_id(id_vpos_kon).get_down_reclist20(VPOS_KON_LAGER.id_vpos_kon, "", VPOS_KON_LAGER.id_vpos_kon.fn());
				
				for(Rec20 rec_lager_lief : reclist_lager){
					
					Rec20 adresse = rec_lager_lief.get_up_Rec20(VPOS_KON_LAGER.id_adresse_lager, ADRESSE.id_adresse, true);	
					String lager = adresse.get_ufs_kette(" ", true, ADRESSE.plz,ADRESSE.name1, ADRESSE.name2, ADRESSE.ort);
					this.get_inner_grid_4_data()
							._a(v_sammle._ar(new own_bt_edit_lagermengen(MASK_STATUS.EDIT,rec_lager_lief.get_ufs_dbVal(VPOS_KON_LAGER.id_vpos_kon_lager))), ld_l_d)
							._a(v_sammle._ar(new ownDeleteButton(rec_lager_lief.get_ufs_dbVal(VPOS_KON_LAGER.id_vpos_kon_lager))),   						ld_l_d)
							._a(new RB_lab(rec_lager_lief.get_fs_dbVal(VPOS_KON_LAGER.lagermenge, "0")),													ld_r_d)
							._a(new RB_lab(lager),																											ld_m_d)
							._a(new RB_lab(rec_lager_lief.get_fs_dbVal(VPOS_KON_LAGER.id_vpos_kon_lager)),													ld_r_d)
							;
	...
	/**
	 * button zum erfassen und editieren der lagereintraege
	 * @author martin
	 *
	 */
	private class own_bt_edit_lagermengen extends RB_HL_bt_to_open_simple_mask {

		private MASK_STATUS status = 			null;
		private String 		id_vpos_kon_lager =	null;
		 ...
		 */
		public own_bt_edit_lagermengen(MASK_STATUS p_status, String p_id_vpos_kon_lager) {
			super();
			this.status = p_status;
			this.id_vpos_kon_lager = p_id_vpos_kon_lager;
			this._set_mask_status_and_id(this.status, this.id_vpos_kon_lager);
			this._set_height_popup(250)._set_width_popup(550)._set_maskTitle(S.mt("Lagerlieferung erfassen ..."));
		}

		@Override
		public RB_ModuleContainerMASK generate_RB_ModuleContainerMASK() throws myException {
			return new ownModulContainerMask();
		}
		...
		public RB_grid4masks generate_RB_grid4masks(RB_ComponentMapCollector componentMapCollector) throws myException {
			...
			RB_ComponentMap map = componentMapCollector.get(new RB_KM(_TAB.vpos_kon_lager));
			
			mask._setSize(100,300)
				._a(new RB_lab()._tr("ID"), 	new RB_gld()._left_mid()._ins(4))._a(map.getComp(VPOS_KON_LAGER.id_vpos_kon_lager), 	new RB_gld()._left_mid()._ins(4))
				._a(new RB_lab()._tr("Lager"), 	new RB_gld()._left_mid()._ins(4))._a(map.getComp(VPOS_KON_LAGER.id_adresse_lager), 		new RB_gld()._left_mid()._ins(4))
				._a(new RB_lab()._tr("Menge"), 	new RB_gld()._left_mid()._ins(4))._a(map.getComp(VPOS_KON_LAGER.lagermenge), 			new RB_gld()._left_mid()._ins(4))
				;
			return mask;
		}
		...
		private class ownModulContainerMask extends RB_ModuleContainerMASK {
		}
		...
		private class ownDataObjectCollector extends RB_DataobjectsCollector_V2 {
			...
			public ownDataObjectCollector(MASK_STATUS p_status, String p_id_vpos_kon_lager) throws myException {
				super();
				this.status = p_status;
				this.id_vpos_kon_lager = p_id_vpos_kon_lager;
				
				//2017-01-31: formal ein RB_dataobject mit fuehren, der die kopftabelle locked gegen konkurierende aenderungen
				KFIX_P_M_masklist_lieferung_zum_lager oThis =KFIX_P_M_masklist_lieferung_zum_lager.this;
				...
				if (p_status.isStatusNew()) {
					this.registerComponent(new RB_KM(_TAB.vpos_kon_lager), new RB_Dataobject_V2(_TAB.vpos_kon_lager));
				} else {
					this.registerComponent(new RB_KM(_TAB.vpos_kon_lager), new RB_Dataobject_V2(new Rec20(_TAB.vpos_kon_lager)._fill_id(id_vpos_kon_lager),p_status));
				}
			}
			...
			public void execute_final_statements_in_open_transaction(RB_DataobjectsCollector_V2 do_collector,	MyE2_MessageVector mv) throws myException {
				...
				RecList20  	rl_lager = rec_vpos_kon.get_down_reclist20(VPOS_KON_LAGER.id_vpos_kon, null, null);
				RecList20  	rl_zuord_ek = rec_vpos_kon.get_down_reclist20(EK_VK_BEZUG.id_vpos_kon_ek, null, null);
				RecList20  	rl_zuord_vk = rec_vpos_kon.get_down_reclist20(EK_VK_BEZUG.id_vpos_kon_vk, null, null);
				
				BigDecimal  bd_summe1 = rl_lager.get_sum_bigDecimal((r)-> {return r.get_myBigDecimal_dbVal(VPOS_KON_LAGER.lagermenge, new MyBigDecimal(BigDecimal.ZERO)).get_bdWert();});
				BigDecimal  bd_summe2 = rl_zuord_ek.get_sum_bigDecimal((r)-> {return r.get_myBigDecimal_dbVal(EK_VK_BEZUG.anzahl, new MyBigDecimal(BigDecimal.ZERO)).get_bdWert();});
				BigDecimal  bd_summe3 = rl_zuord_vk.get_sum_bigDecimal((r)-> {return r.get_myBigDecimal_dbVal(EK_VK_BEZUG.anzahl, new MyBigDecimal(BigDecimal.ZERO)).get_bdWert();});
				
				BigDecimal bd_summe = bd_summe1.add(bd_summe2).add(bd_summe3);
				...
				boolean      is_ueberliefer_erlaubt = S.NN((((IF_RB_Component_Savable)oThis._find_component_in_neighborhood(_TAB.vpos_kon_trakt.rb_km(), VPOS_KON_TRAKT.ueberliefer_ok.fk())).rb_readValue_4_dataobject())).equals("Y");
				...
				if (db_in_mask.isOK()) { 
					...
					if (!is_ladungs_kontrakt) {
						if (bd_summe.compareTo(db_in_mask_toleranz_inclusive)>0) {
							if (is_ueberliefer_erlaubt) {
								mv.add_MESSAGE(new MyE2_Warning_Message("Achtung! die zugeordnete Menge ist größer als die Positionsmenge (incl. evtl. Toleranz), aber Überlieferung ist erlaubt !"));
							} else {
								mv.add_MESSAGE(new MyE2_Alarm_Message("Achtung! die zugeordnete Menge ist größer als die Positionsmenge (incl. evtl. Toleranz) !"));
							}
						}
					} else {
						if (bd_in_mask.doubleValue()>0 && bd_summe.compareTo(db_in_mask_toleranz_inclusive)>0) {
							mv.add_MESSAGE(new MyE2_Warning_Message("Achtung! die zugeordnete Menge ist evtl. größer als die Positionsmenge, hängt aber von der Ladung ab !"));
						}
					}
				} else {
					mv.add_MESSAGE(new MyE2_Alarm_Message("Achtung! Das Mengenfeld in der Maske der Kontraktposition enthält keine Zahl oder ist leer. Bitte geben Sie eine Menge ein, damit Lagerlieferungen erfaßt werden können !!!"));
				}
			}

			@Override
			public ownDataObjectCollector get_copy() throws myException {
				ownDataObjectCollector v_neu = new ownDataObjectCollector(this.status,this.id_vpos_kon_lager);
		...
		private class ownComponentMapCollector extends RB_ComponentMapCollector {
			public ownComponentMapCollector() throws myException {
				super();
				this.registerComponent(new RB_KM(_TAB.vpos_kon_lager), new ownComponentMAP());
			}
		}
		...
		private class ownComponentMAP extends RB_ComponentMap {
			public ownComponentMAP() throws myException {
				super();
		        this.registerComponent(new RB_KF(VPOS_KON_LAGER.id_adresse_lager),    new RB_HL_SelectLagerMandant(VPOS_KON_LAGER.id_adresse_lager, 350, true));
		        this.registerComponent(new RB_KF(VPOS_KON_LAGER.lagermenge),    		new RB_TextField(200)._bord_ddd());
		        this.registerComponent(new RB_KF(VPOS_KON_LAGER.id_vpos_kon_lager),    new RB_lab());
			...
			public MyE2_MessageVector maskSettings_do_After_Load() throws myException {
				bibALL.setFocus(this.get__CompEcho(VPOS_KON_LAGER.lagermenge));
				return null;
			}
			...
			public MyE2_MessageVector maskSettings_define_own_pre_settings(RB_PreSettingsContainer preSettingsContainer,MASK_STATUS status) throws myException {
				preSettingsContainer.rb_set_forcedValueAtSave(VPOS_KON_LAGER.id_vpos_kon, KFIX_P_M_masklist_lieferung_zum_lager.this.id_vpos_kon_actual);
	...
	private class ownDeleteButton extends RB_bt_delete_single {
		...
		public Vector<String> get_delete_sql_statements(String id_to_delete) throws myException {
			return new VEK<String>()._a(new Rec20(_TAB.vpos_kon_lager)._fill_id(id_to_delete).get_sql_2_delete());
		}
		...
		public String get_warnung_achtung_es_wird_ein_irgendwas_geloescht() {
			return "Achtung! Sie löschen eine Lagerzuordnung!";
		}

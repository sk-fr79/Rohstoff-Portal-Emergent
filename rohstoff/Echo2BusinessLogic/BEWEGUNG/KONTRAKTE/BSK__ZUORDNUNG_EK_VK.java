...
public abstract class BSK__ZUORDNUNG_EK_VK extends Project_BasicModuleContainer implements IF_BasicModuleContaier_Refreshable
{
	public static E2_ResourceIcon ICON_MINI_LOCK = E2_ResourceIcon.get_RI("locked_mini.png");
    ...
    private XX_ActionAgent		ownAgentCancel = new XX_ActionAgent()
    {
		...
		{
			BSK__ZUORDNUNG_EK_VK.this.CLOSE_AND_DESTROY_POPUPWINDOW(false);
	 ...
	 * @throws myException
	 */
	public BSK__ZUORDNUNG_EK_VK(String 		ID_VPOS_KON) throws myException
	{
	...
	public void Refresh(Vector<String> vID_ARTIKEL) throws myException
	{
		...
		labHelp.setToolTipText(BSK__ZUORDNUNG_EK_VK.oTooltipZuordnungsMengeKontrakte.CTrans());
							...
							boolean    				bHauptZeile) throws myException
	{
		...
		oLabLagerMenge.setToolTipText(BSK__ZUORDNUNG_EK_VK.oTooltipZuordnungsMengeLager.CTrans());
		...
		if (bMakeMengeAktiv)
		{
			...
			oButt.setToolTipText(BSK__ZUORDNUNG_EK_VK.oTooltipKontraktMenge.CTrans());
			oGrid.add(oButt,LayOutRechts);
		}
		else
		{
			MyE2_Label oLab = new MyE2_Label(cMenge,MyE2_Label.STYLE_SMALL_ITALIC());
			oLab.setToolTipText(BSK__ZUORDNUNG_EK_VK.oTooltipKontraktMenge.CTrans());
		...
		else
		{
			...
			oLabMengeGes.setToolTipText(BSK__ZUORDNUNG_EK_VK.oTooltipZuordnungsMengeKontrakte.CTrans());
			
			oGrid.add(oLabMengeGes,LayOutRechts);
			if (oKomponentMengeSpalte instanceof MyE2_TextField) 
			{ 
				((MyE2_TextField)oKomponentMengeSpalte).setToolTipText(BSK__ZUORDNUNG_EK_VK.oTooltipZuordnungsMengeKontraktOben.CTrans());
			}
			if (oKomponentMengeSpalte instanceof MyE2_Label) { ((MyE2_Label)oKomponentMengeSpalte).setToolTipText(BSK__ZUORDNUNG_EK_VK.oTooltipZuordnungsMengeKontrakte.CTrans());}
		}
		...
		Font   oFont = BSK__ZUORDNUNG_EK_VK.oButtonFontNormal;
		if (S.isFull(recordVPOS_KON.get_UP_RECORD_VKOPF_KON_id_vkopf_kon().get_BUCHUNGSNUMMER_cUF_NN("")))
		{
			...
			oFont = BSK__ZUORDNUNG_EK_VK.oButtonFontSmallItalic; 
		}
	...
	private class SelektorSortenInKontrakten extends MyE2_SelectField
	{
		...
		public SelektorSortenInKontrakten() throws myException
		{
			...
			if (BSK__ZUORDNUNG_EK_VK.this.recVPOS_KON.get_UP_RECORD_VKOPF_KON_id_vkopf_kon().get_VORGANG_TYP_cUF().equals(myCONST.VORGANGSART_EK_KONTRAKT))
			...
			String cZ12 = BSK__ZUORDNUNG_EK_VK.this.recVPOS_KON.get_UP_RECORD_ARTIKEL_id_artikel().get_ANR1_cUF().substring(0,2);
	...
	private class ActionBaueAuswahl extends XX_ActionAgent
	{
		...
		{
			BSK__ZUORDNUNG_EK_VK oThis = BSK__ZUORDNUNG_EK_VK.this;
			try 
			{

 ...
 */
public class BSK_K_MASK extends MyE2_Column  implements IF_BaseComponent4Mask	
{
	private BSK_K_MASK__ComponentMAP oMaskMAP = null;

	private Vector<IF_ADDING_Allowed>  vMaskGrids = new Vector<IF_ADDING_Allowed>();
	
	public BSK_K_MASK(BSK_K_MASK__ComponentMAP maskMAP) throws myException
	{
		...
		oFill.add_Line(oGrid1,new MyE2_String("Vorgang-ID"),1,			"ID_VKOPF_KON|#   |#Abgeschlossen|ABGESCHLOSSEN|#    |#Druckdatum|DRUCKDATUM",5);
		oFill.add_Line(oGrid1,new MyE2_String("Kontrakt-Nr."),1,		"BUCHUNGSNUMMER",5);
		oFill.add_Line(oGrid1,new MyE2_String("Erstellungsdatum "),1,	"ERSTELLUNGSDATUM",5);
		
		oGrid1.add(new Separator(),6,new Insets(2,1,2,1));
		oFill.add_Line(oGrid1,new MyE2_String("Adressat"),1,			"ID_ADRESSE|#  |"+BSK__CONST.HASH_KEY_BUTTON_MASK_OPEN_EDITADRESS+"|# |"+BSK__CONST.HASH_KEY_BUTTON_MASK_OPEN_NEWADRESS,5);
		oGrid1.add(new Separator(),6,new Insets(2,1,2,1));
		...
		MyE2_Grid oGrid4 = new MyE2_Grid(1,MyE2_Grid.STYLE_GRID_NO_BORDER());
		oGrid4._add(this.oMaskMAP.get_Comp(BSK_K_MASK_CompDaugherTextListe.getKey()), new RB_gld());
		
		
		/*
		 * grid seite 5
		 */
		MyE2_Grid oGrid5 = new MyE2_Grid(6,MyE2_Grid.STYLE_GRID_NO_BORDER());
		oGrid5.add(this.oMaskMAP.get_Comp(BSK__CONST.HASH_KEY_DAUGHTERTABLE_POSITIONS),6,E2_INSETS.I_0_2_2_2);

		/*
		 * grid seite 5
		 */
		MyE2_Grid oGrid6 = new MyE2_Grid(6,MyE2_Grid.STYLE_GRID_NO_BORDER_W100());
		oFill.add_Line(oGrid6,new MyE2_String("Anzahl Entsperrungen"),1,"ZAEHLER_ENTSPERRUNG|#  |#Druckzähler:|DRUCKZAEHLER<W100W>",5);
		oGrid6.add(new MyE2_Label(new MyE2_String("Druckprotokoll")),1,1, new Insets(0,12,10,2), new Alignment(Alignment.LEFT,Alignment.TOP));
		oGrid6.add(this.oMaskMAP.get_Comp(BSK__CONST.HASH_KEY_DAUGHTERTABLE_PRINTPROTOKOLL),5,E2_INSETS.I_0_2_10_2);
		...
		oTabbed.add_Tabb(new MyE2_String("Formulartexte"),oGrid4);
		oTabbed.add_Tabb(new MyE2_String("Positionen"),oGrid5);
		oTabbed.add_Tabb(new MyE2_String("Druckprotokoll"),oGrid6);

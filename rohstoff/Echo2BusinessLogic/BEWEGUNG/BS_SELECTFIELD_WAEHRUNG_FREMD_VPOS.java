...
public class BS_SELECTFIELD_WAEHRUNG_FREMD_VPOS extends BS_SELECTFIELD_WAEHRUNG_FREMD {

	public BS_SELECTFIELD_WAEHRUNG_FREMD_VPOS(SQLFieldMAP oFM, boolean bSmall) throws myException {
		super(oFM,bSmall);
	...
	/**
	 * 
	 * @return s _TAB enum or null on error
	 */
	private _TAB getPosTable() {
		try {
		
			E2_ComponentMAP cMap = this.getMaskMap();
			SQLFieldMAP     fMap = cMap.get_oSQLFieldMAP();
			String 			table = fMap.get_cMAIN_TABLE();  
	...
	/**
	 * holt den kopf aus den restrictions (falls ein kopf vorhanden, dann muss nix mehr passieren, da die waehrung dann schon feststeht
	 * @return
	 */
	private Long getIdVKopf() {
		Long l = null;
		...
		try {
			...
			ENUM_VORGANGSART e_v = ENUM_VORGANGSART.findENUM_VORGANGSART_fromPosTable(tab);
			
			//nachsehen, ob es ein restricted sqlfield gibt
			SQLField f = fMap.get_(e_v.getFieldPosToHead());
			
			if (f instanceof SQLFieldForRestrictTableRange) {
				SQLFieldForRestrictTableRange fr = (SQLFieldForRestrictTableRange)f;
				
				String restrictVal = fr.get_cRestictionValue_IN_DB_FORMAT();
				MyLong lr = new MyLong(restrictVal);
	...
	private class Break4ConfirmWaehrung extends Break4Confirm {
		...
		public Break4ConfirmWaehrung() {
			...
			this.getOwnCloseButton()._aaa(()->{BS_SELECTFIELD_WAEHRUNG_FREMD_VPOS.this.set_ActiveValue(BS_SELECTFIELD_WAEHRUNG_FREMD_VPOS.this.getLastSetValue());});
		...
		protected boolean check4break(MyE2_MessageVector mv) throws myException {
			...
			//nur in einem modul, das noch keinen kopf definiert hat, wird dieses popup aktiviert, sonst steht die waehrung ueber den kopf fest
			Long idKopf = getIdVKopf();
			
			if (idKopf==null && BS_SELECTFIELD_WAEHRUNG_FREMD_VPOS.this.isEnabled()) {
			
				//super.check4break prueft den popup-zaehler
				if (super.check4break(mv)) {     
					
					if (S.isEmpty(BS_SELECTFIELD_WAEHRUNG_FREMD_VPOS.this.get_ActualWert())) {
						return false;          //einschalten leere währung ohne aktion
					}
					
					MyLong _idWaehrung = new MyLong(BS_SELECTFIELD_WAEHRUNG_FREMD_VPOS.this.get_ActualWert());
					Long    idAdresse  = BS_SELECTFIELD_WAEHRUNG_FREMD_VPOS.this.getIdAdressActual();

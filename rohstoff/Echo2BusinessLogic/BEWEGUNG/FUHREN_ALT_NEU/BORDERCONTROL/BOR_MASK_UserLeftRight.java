...
import panter.gmbh.basics4project.DB_RECORDS.RECORDNEW_BORDERCROSSING_USERINFO;
...
public class BOR_MASK_UserLeftRight extends ULR_UserLeftRight implements IF_RB_Component_Complex, LR_IF_wrap_Component {

	public BOR_MASK_UserLeftRight() throws myException {
		super();
		...
		this._init(selUser, BORDERCROSSING_USERINFO.id_bordercrossing);
		this.set_style_and_size(MyE2_Grid.STYLE_GRID_DDARK_BORDER(), 250,430,200);
	}
	...
	public Vector<MyRECORD_IF_FILLABLE> maskContents_Transfer_To_Record_And_Prepare_Save(MyE2_MessageVector oMV, RB_ComponentMap rb_maskThisBelongsTo) throws myException {
		...
		for (LR_CB2 caps: this.get_v_cb_right()) {
			BOR_MASK_UserLeftRight_kapsel u_caps = (BOR_MASK_UserLeftRight_kapsel)caps.get_place_4_everything();
			RECORD_USER rec_user = u_caps.get_rec_user();
			
			RECORDNEW_BORDERCROSSING_USERINFO rn = new RECORDNEW_BORDERCROSSING_USERINFO();
...
//			rn.set_NEW_VALUE_ALLOW_CHANGE(u_caps.get_cb_change().isSelected()?"Y":"N");
	...
	public Vector<MyRECORD> query_rosstable_records_to_main_id(String id_bordercrossing) throws myException {
		Vector<MyRECORD> v = new Vector<MyRECORD>();
		if (S.isFull(id_bordercrossing)) {
			SEL sel = new SEL(_TAB.bordercrossing_userinfo).FROM(_TAB.bordercrossing_userinfo).WHERE(BORDERCROSSING_USERINFO.id_bordercrossing,id_bordercrossing);
			v.addAll(new RECLIST_BORDERCROSSING_USERINFO(sel.s()).values());
		}
		return v;
	}

	public RECORD_USER extract_user_from_crosstable(MyRECORD rec_cross_tab) throws myException {
		return ((RECORD_BORDERCROSSING_USERINFO)rec_cross_tab).get_UP_RECORD_USER_id_user();
	}
	...
	public Component component_4_list(LR_CB2 cb, LR_ObjectExtender place_4_everything) throws myException {
		...
		if (cb.button_is_right_side()) {
			BOR_MASK_UserLeftRight_kapsel kaps = (BOR_MASK_UserLeftRight_kapsel)place_4_everything;
...
//																._add(kaps.get_cb_change(),new RB_gld()._ins(E2_INSETS.I(2,1,2,1)))
	...
	public void rb_set_db_value_manual(String id_main_table) throws myException {
		...
		for (String id: this.get_rl_user_pool().get_vKeyValues()) {
			...
			cb_left.set_place_4_everything(new BOR_MASK_UserLeftRight_kapsel(ru));
		...
		v_loaded.sort(new Comparator<MyRECORD>() {
			...
			public int compare(MyRECORD o1, MyRECORD o2) {
				try {
					RECORD_USER ru1 = ((RECORD_BORDERCROSSING_USERINFO)o1).get_UP_RECORD_USER_id_user();
					RECORD_USER ru2 = ((RECORD_BORDERCROSSING_USERINFO)o2).get_UP_RECORD_USER_id_user();
		...
		for  (MyRECORD rec: v_loaded) {
			...
			RECORD_BORDERCROSSING_USERINFO rec_cross = (RECORD_BORDERCROSSING_USERINFO)rec;
			
			//diesen user im linken "Pool" finden und kopieren, auf beiden seiten anhaken
			for (LR_CB2 cb_left: this.get_v_cb_left()) {
				RECORD_USER user2 = ((BOR_MASK_UserLeftRight_kapsel)cb_left.get_place_4_everything()).get_rec_user();
				BOR_MASK_UserLeftRight_kapsel kaps = (BOR_MASK_UserLeftRight_kapsel)cb_left.get_place_4_everything();
...
//					kaps.get_cb_change().setSelected(rec_cross.is_ALLOW_CHANGE_YES());
	...
	public void add_content_before_left_right_panel() throws myException {
		...
		this.add(new MyE2_Label(new MyE2_String("Benutzerauswahl")),E2_INSETS.I(3,3,3,3));
	}

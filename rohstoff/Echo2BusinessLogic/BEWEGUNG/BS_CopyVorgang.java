...
public class BS_CopyVorgang
{
	 ...
	 * @throws myException
	 */
	public BS_CopyVorgang(	String 				cid_vorgangskopf, 
							...
							HashMap<String, String> hm_ZusatzErsatzVonOben) throws myException
	{
		...
		/*
		 *  falls die tabelle eine zusatztabelle fuer die positionen hat, diese auch abfragen
		 *  und das zugehoerige array fuellen
		 *  !!! wichtig! Zusaetze machen nur sinn, wenn quelle und ziel uebereinstimmen
		 */
		...
		// INSERT INTO ZIEL (<Zielfelder>) SELECT <Quellfelder> from QUELLE WHERE ID_QUELLE = xxx
		...
		{
			BS_CopyVorgang._WRITE_ERSATZFIELDS_IN_KOPF_KON(hmErsatzKopf,this.oTableZiel);   //kleine unterschiede / user wird auch kopiert
		}
		else
		{
			BS_CopyVorgang._WRITE_ERSATZFIELDS_IN_KOPF(hmErsatzKopf,this.oTableZiel);
		}
		
		BS_CopyVorgang._WRITE_ERSATZFIELDS_IN_POS(hmErsatzPOS,true,this.oTableZiel);
		...
		if (this.oTableQuelle==this.oTableZiel)
		{
				...
				this.oTableQuelle.get_cVORGANG_TYP().equals(myCONST.VORGANGSART_VK_KONTRAKT))
			{
				...
				BS_CopyVorgang._WRITE_ERSATZFIELDS_IN_POS_KON(hmErsatzZusatz,this.oTableQuelle);
			}
...
//			}
			...
			{
				hmErsatzZusatz = new HashMap<String,String>();
				BS_CopyVorgang._WRITE_ERSATZFIELDS_IN_POS_STD_ANGEBOT(hmErsatzZusatz,this.oTableQuelle);
			}
		}
		...
		try 
		{
			...
			for (int i=0;i<cID_POS.length;i++)
			{
				...
				{
					hmErsatzZusatz = new HashMap<String,String>();
					BS_CopyVorgang._WRITE_ERSATZFIELDS_IN_POS_TPA_FUHRE(hmErsatzZusatz,this.oTableQuelle, new RECORD_VPOS_TPA(cID_POS[i][0]).get_DOWN_RECORD_LIST_VPOS_TPA_FUHRE_id_vpos_tpa().get(0));
...
//		}
	...
	public static void _WRITE_ERSATZFIELDS_IN_POS(HashMap<String,String> hmErsatzPOS,boolean bReplaceKOPF_ID, VorgangTableNames oTN)
	{
		...
		if (oTN.get_cVORGANG_TYP().equals(myCONST.VORGANGSART_RECHNUNG) || oTN.get_cVORGANG_TYP().equals(myCONST.VORGANGSART_GUTSCHRIFT))
		{
			...
			//2021-08-18: auf wunsch auch die menge in rech/gut leermachen
			boolean setMengeToNull = true;
			try {
				setMengeToNull = ENUM_MANDANT_DECISION.RECHNUNG_GUTSCHRIFT_KOPIE_SET_MENGE_TO_0.is_YES();
			} catch (myException e1) {
				e1.printStackTrace();
			}
			if (setMengeToNull) {
				hmErsatzPOS.put(VPOS_RG.anzahl.fn(),			"NULL");
				hmErsatzPOS.put(VPOS_RG.anzahl_abzug.fn(),		"NULL");
				hmErsatzPOS.put(VPOS_RG.anzahl_abzug_lager.fn(),"NULL");
			}
...
//		hmErsatz.put(RECORD_VPOS_TPA_FUHRE.FIELD__BELEGNR_PROFORMA,"NULL");
		...
		//2021-02-26: gelangensbestaetigung nicht mitkopieren
		hmErsatz.put(VPOS_TPA_FUHRE.gelangensbestaetigung_erhalten.fn(), "NULL");

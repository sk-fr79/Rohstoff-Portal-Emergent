...
public class WF_ListSelektor_3_StatusOpenClosed extends E2_ListSelektorMultiselektionStatusFeld_STD  implements IF_definable_all_or_relevant {
	
	public WF_ListSelektor_3_StatusOpenClosed() throws myException {
		super(bibARR.ia(140,140),false,null,new Extent(80));

		this.ADD_STATUS_TO_Selector(true,	new is_null(LAUFZETTEL.abgeschlossen_am).s(),		new MyE2_String("Offen / in Bearb."), 		new MyE2_String("Zeige offene Laufzettel"));		
		this.ADD_STATUS_TO_Selector(false,	new is_not_null(LAUFZETTEL.abgeschlossen_am).s(),	new MyE2_String("Geschlossen"), 			new MyE2_String("Zeige abgeschlossene Laufzettel"));
		...
		this.set_oNeutralisator(new XX_ListSelektor_Neutralisator() {
			...
			public void set_to_Neutral() throws myException {
				WF_ListSelektor_3_StatusOpenClosed.this.get_vCheckBoxTypen().get(0).setSelected(true);
				WF_ListSelektor_3_StatusOpenClosed.this.get_vCheckBoxTypen().get(1).setSelected(true);
			}
		});
	...
	public void select_relevant_data() throws myException {
		this.get_vCheckBoxTypen().get(0).setSelected(true);   //offen

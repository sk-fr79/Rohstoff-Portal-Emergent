...
import rohstoff.Echo2BusinessLogic.WORKFLOW.LAUFZETTEL.EnumDisplayOptions;
...
@Deprecated
public class WF_HEAD_LIST_SelectionComponentVector_Version1 extends E2_SelectionComponentsVector
{

	

	//
	private WF_CheckBoxOption oCB_OwnOnly 	= new WF_CheckBoxOption("Nur Laufzettel mit eigenen, offenen Aufgaben (-)",EnumDisplayOptions.SHOW_OWN_ONLY);
	private WF_CheckBoxOption oCB_Finished 	= new WF_CheckBoxOption("Laufzettel mit abgeschlossenen Aufgaben (+)", EnumDisplayOptions.SHOW_FINISHED);
	private WF_CheckBoxOption oCB_Deleted 	= new WF_CheckBoxOption("Laufzettel mit gelöschten Aufgaben (+)",EnumDisplayOptions.SHOW_DELETED);
	...
	public WF_HEAD_LIST_SelectionComponentVector_Version1( 	E2_NavigationList oNavigationList ,
		...
		MyE2_CheckBox oCB_DeletedList = new MyE2_CheckBox(new MyE2_String("Gelöschte Laufzettel zeigen (+)"),MyE2_CheckBox.STYLE_NORMAL_NO_BORDER_NO_INSETS());
		MyE2_CheckBox oCB_FinishedList = new MyE2_CheckBox(new MyE2_String("Abgeschlossene Laufzettel zeigen (+)"),MyE2_CheckBox.STYLE_NORMAL_NO_BORDER_NO_INSETS());
		MyE2_CheckBox oCB_OwnOnlyList = new MyE2_CheckBox(new MyE2_String("Nur Laufzettel mit eigener Beteiligung (-)"),MyE2_CheckBox.STYLE_NORMAL_NO_BORDER_NO_INSETS());
		...
		this.add(new E2_ListSelectorStandard(oCB_Deleted,"1=1","") );
		this.add(new E2_ListSelectorStandard(oCB_Finished,"1=1","") );
		this.add(new E2_ListSelectorStandard(oCB_OwnOnly,"1=1","") );
...
//		MyE2_Column oColHead = new MyE2_Column();
		MyE2_Grid   oColHead = new MyE2_Grid(1,MyE2_Grid.STYLE_GRID_NO_BORDER_NO_INSETS());
...
//		MyE2_Column oColEntries = new MyE2_Column();
		MyE2_Grid   oColEntries = new MyE2_Grid(1,MyE2_Grid.STYLE_GRID_NO_BORDER_NO_INSETS());
		...
		oColHead.setWidth(new Extent(320));
		...
		oColZeitraum.add(new E2_ComponentGroupHorizontal(0,  oSelBetrachtungsZeitraum.get_oComponentForSelection(), this.get_oButtonReload(),E2_INSETS.I_0_0_2_0));
		...
		MyE2_Column oColSwitcher = new MyE2_Column();
		oColSwitcher.setLayoutData(oLayoutRow);
		//hier zwei dummys, da der umschalter jetzt anderswo plaziert ist
//		oColSwitcher.add(new MyE2_Label(new MyE2_String("Selektor umschalten")),E2_INSETS.I_10_0_0_0);
//		oColSwitcher.add(oButtonSwitchToNew,E2_INSETS.I_10_0_0_0);
		oColSwitcher.add(new MyE2_Label(""),E2_INSETS.I_10_0_0_0);
		oColSwitcher.add(new MyE2_Label(""),E2_INSETS.I_10_0_0_0);
		...
		oRow.add(oColSwitcher);
		
		
		this.set_oSelComponent(oRow);

		
		
		this.add(new E2_ListSelectorStandard(oCB_DeletedList,"","  NVL(JT_LAUFZETTEL.GELOESCHT,'N')='N'"));
		this.add(new E2_ListSelectorStandard(oCB_FinishedList,""," JT_LAUFZETTEL.ABGESCHLOSSEN_AM IS NULL ") );
...
//							"AND ID_USER_ABGESCHLOSSEN_VON IS NULL " +
		...
		this.add(new E2_ListSelectorStandard(oCB_OwnOnlyList,sWhere,"") );
	...
	private class OptionSelectorAction extends XX_ActionAgent
	{
		...
		{
			E2_NavigationList oList = WF_HEAD_LIST_SelectionComponentVector_Version1.this.oNaviList;
	...
	private class ownSelektorBetrachtungszeitraum extends XX_ListSelektor
	{
		...
		public String get_WhereBlock() throws myException
		{
			...
			if (!WF_HEAD_LIST_SelectionComponentVector_Version1.this.oCB_Deleted.isSelected())
			{
				cZusatzWhere = cZusatzWhere +" AND NVL(JT_LAUFZETTEL_EINTRAG.GELOESCHT,'N')='N' ";
			}
			if (!WF_HEAD_LIST_SelectionComponentVector_Version1.this.oCB_Finished.isSelected())
			{
				cZusatzWhere = cZusatzWhere +" AND JT_LAUFZETTEL_EINTRAG.ABGESCHLOSSEN_AM IS NULL ";
			}
			if (WF_HEAD_LIST_SelectionComponentVector_Version1.this.oCB_OwnOnly.isSelected())
			{

...
public class WF_SIMPLE_LIST_BedienPanel extends E2_Grid {
    
    private WF_SIMPLE_LIST_Selector  LAUFZETTEL_EINTRAG_LIST_Selector = null;
    ...
    private WF_SIMPLE_LIST_DATASEARCH    dataSearch = null;
    
    
    public WF_SIMPLE_LIST_DATASEARCH getDataSearch() {
		return dataSearch;
	}
    
    public WF_SIMPLE_LIST_BedienPanel(RB_TransportHashMap  p_params) throws myException     {
        super();
        this.setSize(2);
        ...
        this.LAUFZETTEL_EINTRAG_LIST_Selector = new WF_SIMPLE_LIST_Selector(tpHashMap);
        this._a(LAUFZETTEL_EINTRAG_LIST_Selector,   	new RB_gld()._ins(0,2,10,2)._span(2)._left_mid());
        this._a(grid4Components,   							new RB_gld()._ins(0,2,10,2)._left_mid());
        this._a(this.dataSearch = new WF_SIMPLE_LIST_DATASEARCH(tpHashMap), 	new RB_gld()._ins(0,2,10,2)._left_mid());
        ...
        grid4Components._a((Component)this.toDisableAtMask._ar(new WF_SIMPLE_LIST_bt_ListToMask(true,tpHashMap)),   new RB_gld()._ins(2,2,10,2)._left_mid());
        grid4Components._a(new WF_SIMPLE_LIST_bt_ListToMask(false,tpHashMap),   new RB_gld()._ins(2,2,10,2)._left_mid());
        
        grid4Components._a((Component)this.toDisableAtMask._ar(new WF_SIMPLE_LIST_bt_New(tpHashMap)),  		new RB_gld()._ins(2,2,10,2)._left_mid());
        grid4Components._a((Component)this.toDisableAtMask._ar(new WF_SIMPLE_LIST_bt_Copy(tpHashMap)),   	new RB_gld()._ins(2,2,10,2)._left_mid());
        grid4Components._a((Component)this.toDisableAtMask._ar(new WF_SIMPLE_LIST_bt_multiDelete(tpHashMap)),new RB_gld()._ins(2,2,10,2)._left_mid());
        ...
        uploadBt.add_GlobalValidator(WF_SIMPLE_VALIDATORS.ATTACHMENT.getValidator());
    ...
    public WF_SIMPLE_LIST_Selector get_list_Selector() 
    {
        return LAUFZETTEL_EINTRAG_LIST_Selector;
    }

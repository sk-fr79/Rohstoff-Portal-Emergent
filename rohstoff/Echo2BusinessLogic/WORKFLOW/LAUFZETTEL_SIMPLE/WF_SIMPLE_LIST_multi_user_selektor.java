...
import rohstoff.Echo2BusinessLogic.WORKFLOW.LAUFZETTEL.SEL_NT.WF_touched_users_dd_array;
 ...
 *
 */
public class WF_SIMPLE_LIST_multi_user_selektor extends XX_ListSelektor  implements IF_definable_all_or_relevant  {
...
//	private WF_ListSelektor_3_super_user_selektor   			super_selektor = null;
	...
	public WF_SIMPLE_LIST_multi_user_selektor(/*WF_ListSelektor_3_super_user_selektor   p_super_selektor*/) throws myException {
		super();
...
//		this.super_selektor = p_super_selektor;
		...
		String[][] array_users = new WF_touched_users_dd_array(false).get_users_that_are_touching() ;
		
		hm_DropDowns.put(WF_SIMPLE_USER_ROLE.BEARBEITER_EINTRAG.get_beschriftung(), new ownSelectField(WF_SIMPLE_USER_ROLE.BEARBEITER_EINTRAG,array_users));
		hm_DropDowns.put(WF_SIMPLE_USER_ROLE.OWNER_EINTRAG.get_beschriftung(), 		new ownSelectField(WF_SIMPLE_USER_ROLE.OWNER_EINTRAG,array_users));
		hm_DropDowns.put(WF_SIMPLE_USER_ROLE.ABGESCHLOSSEN_EINTRAG.get_beschriftung(), 	new ownSelectField(WF_SIMPLE_USER_ROLE.ABGESCHLOSSEN_EINTRAG,array_users));
		
		v_keys.add(WF_SIMPLE_USER_ROLE.BEARBEITER_EINTRAG.get_beschriftung());
		v_keys.add(WF_SIMPLE_USER_ROLE.OWNER_EINTRAG.get_beschriftung());
		v_keys.add(WF_SIMPLE_USER_ROLE.ABGESCHLOSSEN_EINTRAG.get_beschriftung());
	...
	public String get_WhereBlock() throws myException {
		...
		if (v_intersect_querys.size()==0) {
			...
			c_where = "("+LAUFZETTEL_EINTRAG.id_laufzettel_eintrag.tnfn()+" IN ("+v_intersect_querys.get(0)+")"+")";
		} else {
			c_where =  "("+LAUFZETTEL_EINTRAG.id_laufzettel_eintrag.tnfn()+" IN ("+bibALL.Concatenate(v_intersect_querys, mengenoperator, "0")+"))";
		}
	...
	public Component get_oComponentWithoutText() throws myException {
		RB_gld glT = new RB_gld()._ins(E2_INSETS.I(2,2,5,2))._col(new E2_ColorDark());
		RB_gld gl = new RB_gld()._ins(E2_INSETS.I(2,2,2,2));
		E2_Grid g = new E2_Grid();
		E2_Grid g_i = new E2_Grid();
		
		g_i	._a(new RB_lab()._tr("Benutzer der Laufzettelaufgabe")._fsa(0)._i(), new RB_gld()._ins(2))
			._a(this.cb_select_union_or_intersect, 						new RB_gld()._ins(2,2,2,4))
			._setSize(600)
			;
		
		g 	._a(g_i,new RB_gld()._ins(2)._span(3))   
			._a(new RB_lab(this.v_keys.get(0))._i()._fsa(0),  	glT)
			._a(new RB_lab(this.v_keys.get(1))._i()._fsa(0),  	glT)
			._a(new RB_lab(this.v_keys.get(2))._i()._fsa(0),  	glT)
	...
	public void add_ActionAgentToComponent(XX_ActionAgent oAgent) {
		WF_SIMPLE_LIST_multi_user_selektor oThis = WF_SIMPLE_LIST_multi_user_selektor.this;
	...
	private class ownSelectField extends E2_Grid {
		private WF_SIMPLE_USER_ROLE  	role = 		null;
		...
		public ownSelectField(WF_SIMPLE_USER_ROLE p_role, String[][] arrayUserPresent) throws myException {
			super();
			...
			this.selfield.setWidth(new Extent(200));
		}
		
		public WF_SIMPLE_USER_ROLE get_user_role() {
			return this.role;
		}
...
//				if (S.isFull(WF_SIMPLE_LIST_multi_user_selektor.this.super_selektor.get_ActualWert())) {
//					id_targetuser = WF_SIMPLE_LIST_multi_user_selektor.this.super_selektor.get_ActualWert();
//				}
	...
	private class toggleString extends XX_ActionAgent {
		@Override
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			if (WF_SIMPLE_LIST_multi_user_selektor.this.cb_select_union_or_intersect.isSelected()) {
				WF_SIMPLE_LIST_multi_user_selektor.this.cb_select_union_or_intersect._t(new MyE2_String("Mehrfachauswahl erweitert die Anzahl der gewählten Laufzettel"));
			} else {
				WF_SIMPLE_LIST_multi_user_selektor.this.cb_select_union_or_intersect._t(new MyE2_String("Mehrfachauswahl reduziert die Anzahl der gewählten Laufzettel"));
			}
		}
	}

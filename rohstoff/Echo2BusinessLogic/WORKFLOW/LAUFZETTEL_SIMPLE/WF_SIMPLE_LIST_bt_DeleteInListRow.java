...
public class WF_SIMPLE_LIST_bt_DeleteInListRow extends RB_BtV4_Delete  {
    
	
	public WF_SIMPLE_LIST_bt_DeleteInListRow(RB_TransportHashMap p_tpHashMap) {
        super();
        
        this._setShapeDeleteButton();
        this._setTransportHashMap(p_tpHashMap);
        
        this.setToolTipText(new MyE2_String("Workflow in dieser Zeile loeschen").CTrans());
        
        this.add_GlobalValidator(WF_SIMPLE_VALIDATORS.DELETE.getValidator());
	...
	public MyE2_String get_warnung_achtung_es_wird_ein_irgendwas_geloescht() {
		return S.ms("Soll diese Workflow geloescht werden ?");	}
 
	@Override
	public String get_warnung_achtung_es_werden_n_irgendwas_geloescht_mit_platzhalter_fuer_zahl() {
		
		return "Sollen diese #WERT# Workflows geloescht werden ?";
	}
	...
	public Vector<String> get_delete_sql_statements(String id_to_delete, MyE2_MessageVector mv) throws myException {
		...
		if (lid.isOK()) {
			Rec21 rec = new Rec21(_TAB.laufzettel_eintrag)._fill_id(lid.get_oLong());
			String sdel = rec.get_ufs_dbVal(LAUFZETTEL_EINTRAG.geloescht, "N"); 
			if (sdel.equals("N")){
				rec._setNewVal(LAUFZETTEL_EINTRAG.geloescht, "Y", mv);
				v.add(rec.get_SQL_UPDATE_STD());
//				v.add(rec.get_DELETE_STATEMENT());
			} else {
				mv._addAlarm(S.ms("Einen gelöschten Eintrag kann man nicht mehr ändern."));
			}
	...
	public Object get_Copy(Object objHelp) throws myExceptionCopy {
		WF_SIMPLE_LIST_bt_DeleteInListRow copy= new WF_SIMPLE_LIST_bt_DeleteInListRow(this.getTransportHashMap());

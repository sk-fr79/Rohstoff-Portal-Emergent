 ...
 * 
 */
public class STATKD_StatusErmittlung implements ICallableTask{

	private HashMap<String , STATKD_DataRowStatus> m_htDataRows = null;
	private Vector<String> m_vSqlStatements ;
	
	private Long   m_IDAdresseMandant 	= Long.parseLong(bibALL.get_ID_ADRESS_MANDANT());
	private Long   m_IDMandant			= Long .parseLong(bibALL.get_ID_MANDANT());
	...
	public STATKD_StatusErmittlung() {
	...
	public STATKD_DataRowStatus ErmittleKundenStatus_Forderungen(String spIDAdresse) throws myException{
		...
		readKundenStatusAll(spIDAdresse,false,true);
...
//		String sSelect_FIBU_Where = (bForderung ? " >= 0 " : " < 0 ");
	...
	/**
	 * Prepared-Statement für die Fibu-Daten
	 * @author manfred
	 * @date 11.07.2018
	 *
	 * @param bForderung
	 ...
	 * @return
	 */
	private SqlStringExtended getSqlExt_FibuData(boolean bForderung, Long lIDAdresse){
		...
		if (lIDAdresse != null ){
			// PARAM 3: ID_ADRESSE
			sWhereAdresse = " AND F.ID_ADRESSE = ? ";
		} else {
			sWhereAdresse = "";
		}
		...
		SqlStringExtended sqlExt = new SqlStringExtended(
				...
				" FROM  JT_FIBU F " +
				"  " +
				" LEFT OUTER JOIN  JT_VKOPF_RG K ON   " +
				"		F.ID_MANDANT = K.ID_MANDANT AND " +
				"		F.ID_VKOPF_RG = K.ID_VKOPF_RG " +
				"  " +
				" LEFT OUTER JOIN  JT_VPOS_RG RP ON " +
				"		K.ID_MANDANT = RP.ID_MANDANT AND " +
				"		K.ID_VKOPF_RG= RP.ID_VKOPF_RG	AND " +
				"		NVL(RP.DELETED,'N') = 'N' " +
				"  " +
				...
				"				SUM( F2.FAKTOR_BUCHUNG_PLUS_MINUS * F2.ZAHLUNGSBETRAG_FREMD_WAEHRUNG) as SUMME_OFFEN " +
				" 				FROM  JT_FIBU F2 " +
				"				LEFT OUTER JOIN   JT_VKOPF_RG K2 ON   F2.ID_VKOPF_RG = K2.ID_VKOPF_RG " +
				" 				WHERE NVL(F2.VORLAEUFIG,'N')   = 'N' " +
				"				AND NVL(F2.STORNIERT,'N')= 'N' " +
				"				AND NVL(F2.BUCHUNG_GESCHLOSSEN,'N') = 'N' " +
				// PARAM 1: ID_MANDANT
				...
				// Param1. idMandant
				sqlExt.getValuesList().add( new Param_Long(m_IDMandant) );
				// Param2 idMandant
				sqlExt.getValuesList().add( new Param_Long(m_IDMandant));
				// Param3 idAdresse
				if(S.isFull(sWhereAdresse)){
					sqlExt.getValuesList().add( new Param_Long(lIDAdresse) );
				}
	...
	/**
	 * Prepared Statement für die Rechnungen
	 * @author manfred
	 * @date 11.07.2018
	 *
	 * @param bForderung
	 ...
	 * @return
	 */
	private SqlStringExtended getSqlExt_Rechnungen(boolean bForderung, Long lIDAdresse){
		String sWhereAdresse = "";
		
		if (lIDAdresse != null){
			sWhereAdresse = " AND  JT_VKOPF_RG.ID_ADRESSE = ? ";
		} 
		
		String sTyp = bForderung ? "RECHNUNG" : "GUTSCHRIFT";
		...
		SqlStringExtended sqlExt = new SqlStringExtended(
						...
						" FROM JT_VPOS_RG INNER  " +
						" JOIN JT_VKOPF_RG ON (JT_VPOS_RG.ID_VKOPF_RG=JT_VKOPF_RG.ID_VKOPF_RG) " +
						" WHERE NVL(JT_VKOPF_RG.ABGESCHLOSSEN,'N')='N' " +
						" AND NVL(JT_VKOPF_RG.DELETED,'N')='N' " +
						" AND NVL(JT_VPOS_RG.DELETED,'N')='N' " +
						// PARAM 1: RECHNUNG / GUTSCHRIFT
						" AND JT_VKOPF_RG.VORGANG_TYP = ? " +      
						// PARAM 2: IDADRESSE
						sWhereAdresse +
						" GROUP BY  JT_VKOPF_RG.ID_ADRESSE " 
				);
		
		// param1: RECHNUNG / GUTSCHRIFT
		sqlExt.getValuesList().add( new Param_String("",sTyp) );
		if (lIDAdresse != null){
			sqlExt.getValuesList().add( new Param_Long(lIDAdresse));
		}
	...
	/**
	 * Prepared Statement für die Freien Positionen
	 * @author manfred
	 * @date 11.07.2018
	 *
	 * @param bForderung
	 ...
	 * @return
	 */
	private SqlStringExtended getSqlExt_FreiePositionen (boolean bForderung, Long lIDAdresse){
		...
		String sWhereAdresse = "";
		if (lIDAdresse != null){
			sWhereAdresse = " AND  JT_VPOS_RG.ID_ADRESSE = ? ";
		} 
		
		Long lVorzeichen = bForderung ? -1L : 1L ;
		...
		SqlStringExtended sqlExt = new SqlStringExtended(
				...
				" AND NVL(JT_VPOS_RG.DELETED,'N')='N' " +
				//  Param1 lVorzeichen
				" AND JT_VPOS_RG.LAGER_VORZEICHEN = ? " +
				sWhereAdresse + 
				" GROUP BY JT_VPOS_RG.ID_ADRESSE "

				);

		// param1: Lagervorzeichen
		sqlExt.getValuesList().add( new Param_Long(lVorzeichen) );
		if (lIDAdresse != null){
			sqlExt.getValuesList().add( new Param_Long(lIDAdresse));
		}
	...
	/**
	 * Prepared Statement für Forderungen aus Fuhren
	 * @author manfred
	 * @date 11.07.2018
	 *
	 * @param lIDAdresse
	 * @return
	 */
	private SqlStringExtended getSqlExt_FuhreForderungen ( Long lIDAdresse){
		
		String sWhereAdresse = "";
		if (lIDAdresse != null) {
			sWhereAdresse = " AND F.ID_ADRESSE_ZIEL = ? ";
		}
		...
		SqlStringExtended sqlExt = new SqlStringExtended(
				"SELECT ID_ADRESSE_ZIEL, " +
						...
						"           FROM   ( " + 	
						"                    SELECT   R.ID_ARTIKEL as ARTIKEL, R.EPREIS_RESULT_NETTO_MGE  FROM JT_VPOS_RG R " + 	
						...
						" ) AS EINZELPREIS  " +
						" ,F.ID_VPOS_TPA_FUHRE " +
						"  FROM  JT_VPOS_TPA_FUHRE  F " +
						" LEFT OUTER JOIN JT_VPOS_KON K ON (F.ID_VPOS_KON_VK=K.ID_VPOS_KON)" +
						" INNER JOIN JT_ARTIKEL A  ON  F.ID_ARTIKEL = A.ID_ARTIKEL " +
						" LEFT OUTER JOIN ( " +
						"   SELECT NVL(R.ID_VPOS_TPA_FUHRE_ZUGEORD,0) ID_FUHRE_BERECHNET " +  
						"   FROM  JT_VPOS_RG R  LEFT OUTER JOIN  JT_VKOPF_RG RK ON (RK.ID_VKOPF_RG=R.ID_VKOPF_RG) " +
						...
						"         AND  R.ID_VPOS_TPA_FUHRE_ZUGEORD is not null " +
						"		  AND R.LAGER_VORZEICHEN = -1	" +	
						...
						" AND   (F.ID_ADRESSE_ZIEL <> ? ) "  +  //sIDAdresseMandant + " ) " +
						sWhereAdresse 
				);

		// param1: 
		sqlExt.getValuesList().add( new Param_Long(m_IDAdresseMandant) );
	 ...
	 * @return
	 */
	private SqlStringExtended getSqlExt_FuhreVerbindlichkeiten ( Long lIDAdresse){
		
		String sWhereAdresse = "";
		if (lIDAdresse != null) {
			sWhereAdresse = " AND F.ID_ADRESSE_START = ? " ;
		...
		SqlStringExtended sqlExt = new SqlStringExtended(
					...
					" AND   (F.ID_ADRESSE_START<> ? ) " + //sIDAdresseMandant + " ) " +
					sWhereAdresse 
		);
	...
	/**
	 * Prepared Statement für Forderungen aus Fuhrenorten
	 * @author manfred
	 * @date 11.07.2018
	 *
	 * @param bForderung
	 ...
	 * @return
	 */
	private SqlStringExtended getSqlExt_Fuhrenort_Forderungen(Long lIDAdresse){
		String sWhereAdresse = "";
		if (lIDAdresse != null){
			sWhereAdresse = " AND O.ID_ADRESSE = ? ";
//			sWhereAdresse = " AND O.ID_ADRESSE =  " + lIDAdresse.toString() ;
		} 
		...
		SqlStringExtended sqlExt = new SqlStringExtended(
				" SELECT  O.ID_ADRESSE, " +
					...
					" ) AS EINZELPREIS  " +
					" , O.ID_VPOS_TPA_FUHRE " +
					" FROM  JT_VPOS_TPA_FUHRE_ORT O " +
					" INNER JOIN JT_VPOS_TPA_FUHRE  F " +
					" ON ( O.ID_VPOS_TPA_FUHRE = F.ID_VPOS_TPA_FUHRE ) " +
					" LEFT OUTER JOIN JT_VPOS_KON K " +
					" ON (O.ID_VPOS_KON=K.ID_VPOS_KON) " +
					" INNER JOIN JT_ARTIKEL A ON A.ID_ARTIKEL = O.ID_ARTIKEL " +
					...
					"    AND  NVL(R.DELETED,'N')='N' " +
					"    AND  NVL(RK.DELETED,'N')='N' " +
					"    AND  R.ID_VPOS_TPA_FUHRE_ORT_ZUGEORD IS NOT NULL " +
					"    AND  R.LAGER_VORZEICHEN = -1	" +	
					" )) " +
					" AND (NVL(F.DELETED,'N')='N') " +
					...
					" AND O.ID_ADRESSE <> ? " +
//					" AND O.ID_ADRESSE <>  " + bibALL.get_ID_ADRESS_MANDANT() +
					sWhereAdresse 
				);
	 ...
	 * @return
	 */
	private SqlStringExtended getSqlExt_Fuhrenort_Verbindlichkeiten(Long lIDAdresse){
		String sWhereAdresse = "";
		if (lIDAdresse != null){
			sWhereAdresse = " AND O.ID_ADRESSE = ? ";
		} 
	...
	/**
	 * Prepared Statement für Forderungen aus geplanten Fuhren
	 * @author manfred
	 * @date 11.07.2018
	 *
	 * @param lIDAdresse
	 * @return
	 */
	private SqlStringExtended getSqlExt_FuhreGeplant_Forderungen ( Long lIDAdresse){
	 ...
	 * @return
	 */
	private SqlStringExtended getSqlExt_FuhreGeplant_Verbindlichkeiten ( Long lIDAdresse){
	...
	/**
	 * Prepared Statement für Forderungen aus geplanten Fuhrenorten
	 * @author manfred
	 * @date 11.07.2018
	 *
	 * @param bForderung
	 ...
	 * @return
	 */
	private SqlStringExtended getSqlExt_FuhrenortGeplant_Forderungen(Long lIDAdresse){
		String sWhereAdresse = "";
		if (lIDAdresse != null){
			sWhereAdresse = "  AND O.ID_ADRESSE =  ? ";
		} 
	 ...
	 * @return
	 */
	private SqlStringExtended getSqlExt_FuhrenortGeplant_Verbindlichkeiten(Long lIDAdresse){
		String sWhereAdresse = "";
		if (lIDAdresse != null){
			sWhereAdresse = "  AND O.ID_ADRESSE = ? ";
		} 
	...
	private void readKundenStatus(String spIDAdresse){
		readKundenStatusAll(spIDAdresse, true, true);
	}
	 ...
	 */
	private void readKundenStatusAll(String spIDAdresse, boolean bVerbindlichkeiten, boolean bForderungen){
	
		SqlStringExtended 	sqlExt 		= null;
		String [][] 		sArrDaten 	= new String[0][0];
		
		Long   				lIDAdresse 	= S.isFull(spIDAdresse) ? Long.parseLong(spIDAdresse) : null ; 		
		...
		//
		if (bForderungen){
			sqlExt = getSqlExt_FibuData(true, lIDAdresse);
		...
		if(bVerbindlichkeiten){
			sqlExt = getSqlExt_FibuData(false, lIDAdresse);
			sArrDaten = new String[0][0];
			sArrDaten =  bibDB.EinzelAbfrageInArray(sqlExt,(String)null);
		...
		//
		if(bForderungen){
			sqlExt = getSqlExt_Rechnungen(true, lIDAdresse);
			sArrDaten = new String[0][0];
			sArrDaten =  bibDB.EinzelAbfrageInArray(sqlExt,(String)null);
		...
		if (bVerbindlichkeiten){
			sqlExt = getSqlExt_Rechnungen(false, lIDAdresse);
			sArrDaten = new String[0][0];
			sArrDaten =  bibDB.EinzelAbfrageInArray(sqlExt,(String)null);
		...
		//
		if(bForderungen){
			sqlExt = getSqlExt_FreiePositionen(true, lIDAdresse);
			sArrDaten = new String[0][0];
			sArrDaten =  bibDB.EinzelAbfrageInArray(sqlExt,(String)null);
		...
		if (bVerbindlichkeiten){
			sqlExt = getSqlExt_FreiePositionen(false, lIDAdresse);
			sArrDaten = new String[0][0];
			sArrDaten =  bibDB.EinzelAbfrageInArray(sqlExt,(String)null);
		...
		//  FUHREN FORDERUNGEN, 
		if (bForderungen){
			sqlExt = getSqlExt_FuhreGeplant_Forderungen(lIDAdresse);
			sArrDaten = new String[0][0];
			sArrDaten =  bibDB.EinzelAbfrageInArray(sqlExt,(String)null);
			...
			sqlExt = getSqlExt_FuhrenortGeplant_Forderungen(lIDAdresse);
			sArrDaten = new String[0][0];
			sArrDaten =  bibDB.EinzelAbfrageInArray(sqlExt,(String)null);
		...
		if (bVerbindlichkeiten){
			...
			sqlExt = getSqlExt_FuhreGeplant_Verbindlichkeiten(lIDAdresse);
			sArrDaten = new String[0][0];
			sArrDaten =  bibDB.EinzelAbfrageInArray(sqlExt,(String)null);
			fillDataInRow(sArrDaten, ENUM_Values.FUHRE_VERBINDLICHKEIT_GEPLANT );

			// Geplante Fuhrenorte 
			sqlExt = getSqlExt_FuhrenortGeplant_Verbindlichkeiten(lIDAdresse);	
			sArrDaten = new String[0][0];
			sArrDaten =  bibDB.EinzelAbfrageInArray(sqlExt,(String)null);
...
//				" round(avg(EINZELPREIS_FW),4) " +
	 ...
	 * - 
	 *  curl "http://localhost:8080/rohstoff_app/batch?user=batchuser&pw=xxxx&task=kundenstatus"
	 ...
	 * values(seq_batch_task.nextval,1,'kundenstatus','rohstoff.Echo2BusinessLogic.KUNDENSTATUS.STATKD_StatusErmittlung','Ermittelt den Kundenstatus','SYSTEM',sysdate,'SYSTEM',sysdate);
	 * 
	 */

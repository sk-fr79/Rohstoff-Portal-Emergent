...
public class LAG_BEW_LIST_BT_GET_RESTMENGEN_AVG extends MyE2_Button
{
	...
	public LAG_BEW_LIST_BT_GET_RESTMENGEN_AVG(	E2_NavigationList onavigationList, 
											...
											LAG_BEW_LIST_Panel_MengenErmittlung oPanel)
	{
		...
		this.add_GlobalValidator(new E2_ButtonAUTHValidator_AUTO("LAG_BEW_CALC_MENGEN_BELOW_PRICE_AVG"));
		
		this.setToolTipText(new MyE2_String("Ermitteln der Restmengen, die den gewählten Durchschnittspreis ergeben.").CTrans());
	}
	...
	private class ownActionAgent extends XX_ActionAgent
	{
		...
		public void executeAgentCode(ExecINFO execInfo) throws myException {
			...
			LAG_BEW_LIST_BT_GET_RESTMENGEN_AVG oThis = LAG_BEW_LIST_BT_GET_RESTMENGEN_AVG.this;
			...
			Double dblAvgPreis = bibALL.makeDoublefromNumberString(oThis.oPanelParent.getPreisAvg(), ",", ".");
			if (dblAvgPreis == null){
				bibMSG.add_MESSAGE(new MyE2_Alarm_Message(new MyE2_String("Bitte geben Sie eine Zahl im Feld für den Durchschnittspreis ein!")));
				oThis.oNaviList._REBUILD_ACTUAL_SITE("");
				return;
			}
			
			BigDecimal bdAvgBetrag = new BigDecimal(dblAvgPreis.doubleValue() );
			...
			oThis.oPanelParent.get_oMengenErmittlung().ermittleMengeZuDurchschnittspreis(cID_Lager,cID_Hauptartikel, cID_Artikel, bdAvgBetrag);
			if (bibMSG.get_bIsOK())
			{
				String sValue = bibALL.makeDez(oThis.oPanelParent.get_oMengenErmittlung().get_ErmittelteMenge().doubleValue(), 3, true);
				String sValuePreis = bibALL.makeDez(oThis.oPanelParent.get_oMengenErmittlung().get_ErmittelterPreis().doubleValue(), 2, true);
				oThis.oPanelParent.setErgebnisMengeAVG(sValue);
				oThis.oPanelParent.setErgebnisPreisAVG(sValuePreis);			

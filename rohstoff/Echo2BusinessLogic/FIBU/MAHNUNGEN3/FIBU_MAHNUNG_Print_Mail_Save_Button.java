...
public class FIBU_MAHNUNG_Print_Mail_Save_Button extends MyE2_Button 
{
	private FIBU_MAHNUNG_Container oMaskContainer = null;
	...
	public FIBU_MAHNUNG_Print_Mail_Save_Button(FIBU_MAHNUNG_Container MaskContainer, processtype e_typ) throws myException 
	{
		super("");
		this.typ = e_typ;
	...
	private class ownValidator extends XX_ActionValidator{
		...
		public MyE2_MessageVector isValid(Component oComponentWhichIsValidated) throws myException {
			...
			FIBU_MAHNUNG_Container parentComponent = FIBU_MAHNUNG_Print_Mail_Save_Button.this.oMaskContainer;
	...
	private class ownJasperHash extends SE_JasperHASH_4_ArchivPDF{
		...
		public ownJasperHash(String p_id_mahnung) throws myException {
			super("fibu_mahnung_2", _TAB.fibu, p_id_mahnung);

			RECORD_MAHNUNG recMahnung = FIBU_MAHNUNG_Print_Mail_Save_Button.this.oMaskContainer.get_recMahnung();

			this.cID_ADRESSE = recMahnung.get_DOWN_RECORD_LIST_MAHNUNG_POS_id_mahnung().get(0).get_UP_RECORD_FIBU_id_fibu().get_ID_ADRESSE_cUF();

			this.put("id_mahnung",recMahnung.get_ID_MAHNUNG_cUF());

			this.set_bVorschauDruck(false);

			this.get_vExecuters().add(0,new FIBU_MAHNUNG_executer_add_rechnung_orig(FIBU_MAHNUNG_Print_Mail_Save_Button.this));
		...
		public ArrayList<String> get_v_email_targets() throws myException {
			return FIBU_MAHNUNG_Print_Mail_Save_Button.this.oMaskContainer.getSelectedEmails();
		}
	...
	private  class ownActionAgent extends SE_agent_processing_print_mail_preview
	{
		...
		public Vector<String> get_ids_to_print() throws myException {
			...
			if(FIBU_MAHNUNG_Print_Mail_Save_Button.this.oMaskContainer.isbNeu()){
				idList.add(selectedFibuIds.get(0));
			}else{
				RECLIST_MAHNUNG_POS rec = new RECLIST_MAHNUNG_POS(
						MAHNUNG_POS.id_mahnung+"="+
								FIBU_MAHNUNG_Print_Mail_Save_Button.this.oMaskContainer.getMahnungID(),
						MAHNUNG_POS.manhnung_pos.fieldName());
				idList.add(rec.get(0).get_ID_FIBU_cUF());
			}
		...
		public SE_JasperHASH_4_ArchivPDF generate_jasperHash(String p_id_mahnung) throws myException {
			...
			MyE2_MessageVector mv = FIBU_MAHNUNG_Print_Mail_Save_Button.this.oMaskContainer.save_mask();

			if(mv.get_bIsOK()){
				ArrayList<String> emails = FIBU_MAHNUNG_Print_Mail_Save_Button.this.oMaskContainer.getSelectedEmails(); 
				String fax = FIBU_MAHNUNG_Print_Mail_Save_Button.this.oMaskContainer.getSelectedFaxNummer();
				...
				oJh.put("id_mahnung", FIBU_MAHNUNG_Print_Mail_Save_Button.this.oMaskContainer.getMahnungID());
			}	
			return oJh;
	...
	public class ownArchivAgent extends XX_ActionAgent{
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			...
			RECLIST_MAHNUNG_POS fmRecList = new RECLIST_MAHNUNG_POS(MAHNUNG_POS.id_mahnung+"="+oMaskContainer.getMahnungID(),MAHNUNG_POS.manhnung_pos.fieldName());
			if(fmRecList.size()>1){
	...
	private class ownMailBlock extends MailBlock
	{
		...
		{
			return FIBU_MAHNUNG_Print_Mail_Save_Button.this.oMaskContainer.get_oNaviList().get_oContainer_NaviList_BelongsTo().get_MODUL_IDENTIFIER();
		}
	}

	public FIBU_MAHNUNG_Container get_maskContainer() {
	...
	private class ownCloseAction extends XX_ActionAgent{
		...
		private FIBU_MAHNUNG_Container parent;
		private processtype typ;

		public ownCloseAction(FIBU_MAHNUNG_Container c_parentContainer, processtype typ) {

...
public class FIBU_LIST_BT_PrintMail_Zahlungsavis extends MyE2_Button  implements FIBU_LIST_BT_IF_Druck_Mail_Common{
	...
	private static final FORMULARSTYP FORMULAR_TYP =  FORMULARSTYP.ZAHLUNGSAVIS;
	
	private MyE2_TextField_DatePOPUP_OWN zahlungsAmField;
	
	private RECORD_FIBU_FORMULAR formularRecord;
	...
	public FIBU_LIST_BT_PrintMail_Zahlungsavis(E2_NavigationList  p_naviList) throws myException {
		super(new MyE2_String("Druck/Mail Zahlungsavis"));
		...
		this.add_GlobalValidator(new ZahlungavisValidator());
	...
	private void preloadEmailList() throws myException{
		...
		Vector<String> selectedIds = FIBU_LIST_BT_PrintMail_Zahlungsavis.this.naviList.get_vSelectedIDs_Unformated();
		if(selectedIds.size()>0){	
			...
			if(mailSearcher.get_v_MailAdressesFound().size()==0){
				omv.add_MESSAGE(new MyE2_Warning_Message(new MyE2_String("Es gibt keine Email-Adresse mit der Mail-Verwendung FIBU.")));
			}else
				emailList.addAll(mailSearcher.get_v_MailAdressesFound());
		}
	}
	...
	private void preloadFaxNumbers() throws myException{
		...
		if(selectedIds.size()>0){
			...
			if(faxNumberResearch.get_vFax_number_found().size()==0){
				omv.add_MESSAGE(new MyE2_Warning_Message(new MyE2_String("Es ist keine Faxnummer beim Kunden eingetragen.")));
	...
	private class ownBasicPopupContainer extends E2_BasicModuleContainer {
		...
		public ownBasicPopupContainer() throws myException {
			...
			FIBU_LIST_BT_PrintMail_Button  bt_print 	= new FIBU_LIST_BT_PrintMail_Button("Druck", selectedBuchungBlock, formularRecord);
			FIBU_LIST_BT_PrintMail_Button  bt_preview 	= new FIBU_LIST_BT_PrintMail_Button("Vorschau", selectedBuchungBlock, formularRecord);
			FIBU_LIST_BT_PrintMail_Button  bt_mail 		= new FIBU_LIST_BT_PrintMail_Button("eMail", selectedBuchungBlock, formularRecord);
			...
			zahlungsAmField = new MyE2_TextField_DatePOPUP_OWN(new myDateHelper(bibALL.get_cDateNOW()).get_cDateFormatForMask(), 120);
			zahlungsAmField.set_oLastDateKlicked(new myDateHelper(bibALL.get_cDateNOW()));
			...
			E2_Grid4MaskSimple  gm = new E2_Grid4MaskSimple(MyE2_Grid.STYLE_GRID_DDARK_BORDER())
					...
					.def_(E2_INSETS.I(2,5,2,2))
					.add_(new MyE2_Label(new MyE2_String("Zahlung erfolgt am:"),new E2_FontBold(2)))
					.add_(zahlungsAmField)
					.setSize_(i_breite)
					;

			int[] i_button_breite = {200,200,200};
			...
			this.CREATE_AND_SHOW_POPUPWINDOW(new Extent(640), new Extent(480), new MyE2_String("Drucken/Mailen/Vorschau - Zahlungsavis"));
	...
	private class ZahlungavisValidator extends XX_ActionValidator{

		@Override
		public MyE2_MessageVector isValid(Component oComponentWhichIsValidated) throws myException {
			MyE2_MessageVector mv_rueck = new MyE2_MessageVector();

			E2_NavigationList parentNaviList = FIBU_LIST_BT_PrintMail_Zahlungsavis.this.getNaviList();

			selectedFibuIds.clear();
			selectedBuchungBlock.clear();

			Vector<String> selectIdsVector = new Vector<>();
			selectIdsVector.clear();
			selectIdsVector = parentNaviList.get_vSelectedIDs_Unformated_Select_the_one_and_only();

			int nbOfReports = bibDB.EinzelAbfrageInArray(querySelectField.s()).length;
			
			omv.clear();
			
			if(selectIdsVector.size() == 0){
				mv_rueck.add_MESSAGE(new MyE2_Alarm_Message(new MyE2_String("Bitte mindestens einen Eintrag aus einem Buchungsblock auswählen!")));
			}else if(nbOfReports == 0){
				mv_rueck.add(new MyE2_Alarm_Message("Es ist kein Formular vom Typ:"+FORMULAR_TYP.getTyp() + " hinterlegt !"));
			}else {
				...
				for(String selectedId : selectIdsVector){
					if(! buchungBlockList.contains(selectedId)){
						mv_rueck.add_MESSAGE(new MyE2_Alarm_Message(new MyE2_String("Bitte wählen Sie nur einen Buchungsblock aus!")));
					}
				...
				for(RECORD_FIBU record: recBuhungNr){
					...
					if(record.get_BUCHUNGSTYP_cUF().equals(myCONST.BT_DRUCK_RECHNUNG)){
						flag[0]++;
					}else if(record.get_BUCHUNGSTYP_cUF().equals(myCONST.BT_DRUCK_GUTSCHRIFT)){
						flag[1]++;
					}else{
						flag[2]++;
					}
				...
				if(flag[1]==0 && flag[0]==0){
					mv_rueck.add_MESSAGE(new MyE2_Warning_Message(new MyE2_String("Bitte wählen Sie einen Buchungsblock mit mindestens einer Gutschrift oder einer Rechnung!")));
				}
				...
				for(String selectedId : buchungBlockList){
					...
					String idVKopfRg = rec1.get_ID_VKOPF_RG_cUF_NN("");

					if(!bibALL.isEmpty(idVKopfRg)){
						RECLIST_VPOS_RG recVposRg = new RECLIST_VPOS_RG(VPOS_RG.id_vkopf_rg + "=" + idVKopfRg, VPOS_RG.id_vpos_rg.fieldName());
						for(RECORD_VPOS_RG recvpos : recVposRg){
							if(recvpos.get_EINZELPREIS_dValue(0.0d)<0.0){
//								mv_rueck.add_MESSAGE(new MyE2_Warning_Message(new MyE2_String("Es gibt mindestens eine Position mit einem negativen Einzelpreis. (Entsorgungskosten, o.Ä.)")));
								break;
							}
						}		
					}

					String Buchungstyp = rec1.get_BUCHUNGSTYP_cUF();


					if(Buchungstyp.equals(myCONST.BT_DRUCK_GUTSCHRIFT)){
						selectedFibuIds.add(selectedId);
					}else if(Buchungstyp.equals(myCONST.BT_DRUCK_RECHNUNG)){
						selectedFibuIds.add(selectedId);
					}else if(Buchungstyp.equals(myCONST.BT_SCHECKDRUCK)){
						selectedFibuIds.add(selectedId);
					}else if(Buchungstyp.equals(myCONST.BT_ZAHLUNGSEINGANG)){
						selectedFibuIds.add(selectedId);
					}else if(Buchungstyp.equals(myCONST.BT_ZAHLUNGSAUSGANG)){
						selectedFibuIds.add(selectedId);
					}		
				}
				...
				for(String selectedId : buchungBlockList){
					RECORD_FIBU rec1 = new RECORD_FIBU(selectedId);
					double a = rec1.get_FAKTOR_BUCHUNG_PLUS_MINUS_dValue(0.0)*rec1.get_ZAHLUNGSBETRAG_FREMD_WAEHRUNG_dValue(0.0);
					summe = summe + a;
				}

//				if(summe>0.0){
//					mv_rueck.add(new MyE2_Warning_Message(new MyE2_String("Bitte wählen Sie einen Buchungsblock mit einem Saldo zugunsten des Kunden/Lieferanten!")));
//				}
	...
	private class ownActionAgent extends SE_agent_processing_print_mail_preview {
		...
		public SE_JasperHASH_4_ArchivPDF generate_jasperHash(String id) throws myException {
			...
			formularRecord =  new RECORD_FIBU_FORMULAR(formularSelectBox.get_ActualWert());
			...
			String zahlungsDatum = bibALL.BaueDatumSQL(zahlungsAmField.get_oFormatedDateFromTextField(), "DD.MM.YYYY");
			
			oJh.put("ZAHLUNG_AM", zahlungsDatum);
	...
	private class ownJasperHash extends SE_JasperHASH_4_ArchivPDF {
		...
		public ownJasperHash(String id_fibu) throws myException {
			super(formularRecord.get_FORMULARNAME_cUF(), _TAB.fibu, id_fibu);
			
			this.get_vExecuters().add(0,new FIBU_LIST_BT_Rechnung_Anhanger_executer(FIBU_LIST_BT_PrintMail_Zahlungsavis.this));
		...
		public ArrayList<String> get_v_email_targets() throws myException {
			...
			v_email = FIBU_LIST_BT_PrintMail_Zahlungsavis.this.emailListComp.getSelectedKontakt();
		...
		public MAILPROFILE get_used_send_profile() throws myException {
			return MAILPROFILE.FIBUZAHLUNGSAVIS;
		}
	...
	private class dataValidator extends XX_ActionValidator{
		...
		public MyE2_MessageVector isValid(Component oComponentWhichIsValidated) throws myException {
			...
			if(emailListComp.getSelectedKontakt().size()>5){
				bibMSG.add_MESSAGE(new MyE2_Alarm_Message(new MyE2_String("Es können maximal 5 eMail-Adressen gewählt werden")));
			}
			
			if(S.isEmpty(zahlungsAmField.get_oFormatedDateFromTextField())){
				bibMSG.add_MESSAGE(new MyE2_Alarm_Message(new MyE2_String("Bitte geben Sie ein Zahlungsdatum an !")));
			}

...
public class FIBU_LIST_BT_PrintMail_Buchungsblatt extends MyE2_Button 
{

	private E2_NavigationList  oNaviList = null;

	
	public FIBU_LIST_BT_PrintMail_Buchungsblatt(E2_NavigationList NaviList) 
	{
		//super(E2_ResourceIcon.get_RI("print_and_mail.png"));
		super(new MyE2_String("Druck/Mail komplettes Kontoblatt/Gesamtübersicht des Kunden"));
		this.oNaviList =NaviList;
		
		this.setToolTipText(new MyE2_String("Drucken oder Mailen einer Liste aller offenen Buchungspositionen des Kunden ...").CTrans());
		
		this.add_oActionAgent(new ownActionAgent(new MyE2_String("Drucke Kontoblatt"), this.oNaviList));
	...
	private  class ownActionAgent extends ACTIONAGENT_MAIL_AND_REPORT
	{
		...
		{
			super(fensterTitel, "PRINT_KONTO_BLATT", "PRINT_KONTO_BLATT",null,false);
		}
		...
		{
			E2_MassMailer oMassMailer = new E2_MassMailer_STD(	"FIBU_KONTOBLATT_MAIL_BETREFF",
																"FIBU_KONTOBLATT_MAIL_TEXT",
																"FIBU_KONTOBLATT");
		...
		public V_JasperHASH get_VJasperHashMAP(ExecINFO execInfo) throws myException
		{
			...
			this.vIDs.addAll(FIBU_LIST_BT_PrintMail_Buchungsblatt.this.oNaviList.get_vSelectedIDs_Unformated());

			
			//jetzt feststellen, ob es genau eine adresse ist
			VectorSingle vTesteIds = new VectorSingle();
			vTesteIds.addAll(new RECLIST_FIBU(this.vIDs).get_ID_ADRESSE_hmString_UnFormated("").values());
			
			
			if (vTesteIds.size()!=1)
			{
				bibMSG.add_MESSAGE(new MyE2_Alarm_Message(new MyE2_String("Bitte mindestens eine und ausschliesslich Zeilen eines Kunden/Lieferanten wählen !!")));
				return vRueck;
			}
			
			for (int i=0;i<this.vIDs.size();i++)
			{
				vRueck.add(new ownJasperHASH(vTesteIds.get(0)));
			}
		...
		private class ownJasperHASH extends E2_JasperHASH
		{
			...
			public ownJasperHASH(String ID_ADRESSE_UF)	throws myException
			{
				super("FIBU_KONTOBLATT", new JasperFileDef_PDF());

				this.cID_ADRESSE_UF = ID_ADRESSE_UF;
				
				this.put("id_adresse",this.cID_ADRESSE_UF);

				this.set_bVorschauDruck(false);
				

				this.set_cDownloadAndSendeNameStaticPart("FIBU_KONTOBLATT"+"_"+this.cID_ADRESSE_UF);
			...
			protected MailBlock create_MailBlock() throws myException
			{
				...
				oMailBlock.add_VMailAdress4MailBlock(
						new MailAdressSearcher(	this.cID_ADRESSE_UF,
												true,
												true,
												true,
		...
		private class ownMailBlock extends MailBlock
		{
			...
			{
				return FIBU_LIST_BT_PrintMail_Buchungsblatt.this.oNaviList.get_oContainer_NaviList_BelongsTo().get_MODUL_IDENTIFIER();
			}
		}

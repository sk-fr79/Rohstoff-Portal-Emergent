...
public class VerbundenFibuIdsViaFuhre extends AbstractFibuIdSuche{
	
	protected HashSet<String> intermediarErgebnisVektor;
	
	public VerbundenFibuIdsViaFuhre(String fibuId) throws myException {
		super(fibuId);
	}	
		
	public VerbundenFibuIdsViaFuhre() {
		super();
	}
	...
	public HashSet<String> sucheVerbundenVposIds() throws myException {
		...
		while(it.hasNext()){
			...
			if(! ergebnisRecord.get_DELETED_cUF().equals('N')){
				String erg = ergebnisRecord.get_ID_VPOS_TPA_FUHRE_ZUGEORD_cUF_NN(NON_NULL_CHAR);
	...
	public HashSet<String> sucheVerbundenVkopfRgId(){
		...
		while(it.hasNext()){
			...
			if(! vposRgId.equals(NON_NULL_CHAR)){
				try {
					RECLIST_VPOS_RG recordList = new  RECLIST_VPOS_RG("ID_VPOS_TPA_FUHRE_ZUGEORD="+vposRgId, "ID_VKOPF_RG");
					for(RECORD_VPOS_RG rec: recordList){
						String erg = rec.get_ID_VKOPF_RG_cUF_NN(NON_NULL_CHAR);

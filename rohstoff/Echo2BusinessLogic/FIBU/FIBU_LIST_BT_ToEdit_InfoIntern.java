...
public class FIBU_LIST_BT_ToEdit_InfoIntern extends MyE2_DB_Button_To_FastEdit 
{

	public FIBU_LIST_BT_ToEdit_InfoIntern(SQLField osqlField) throws myException
	{
		super(osqlField, new MyE2_String("Buchungsinfo erfassen/ändern"), MyE2_Button.StyleTextButtonSTD(new E2_FontPlain()),
											"AENDERE_BUCHUNGS_INFO_INTERN", new Extent(500), new Extent(250));
		this.set_cSetTextWhenEmpty(new MyE2_String("-"));
		this.setLineWrap(true);
		this.EXT().set_oColExtent(new Extent(300));
		this.setToolTipText(new MyE2_String("Zusatzinfo (intern) zu eine Buchungszeile erfassen ...").CTrans());
	}
	...
	{
		FIBU_LIST_BT_ToEdit_InfoIntern oButtCopy = null;
		
		try
		{
			oButtCopy = new FIBU_LIST_BT_ToEdit_InfoIntern(this.EXT_DB().get_oSQLField());
		}
	...
	private class ownBasicContainer extends E2_BasicModuleContainer
	{
		...
		public ownBasicContainer() throws myException 
		{
			...
			this.oMap = FIBU_LIST_BT_ToEdit_InfoIntern.this.EXT().get_oComponentMAP();
			SQLResultMAP     oResultMap = oMap.get_oInternalSQLResultMAP();
			
			this.cID_FIBU = ""+oResultMap.get_LActualDBValue("ID_FIBU", false);
			
			
			this.oTF_InfoIntern = new UB_TextArea("INTERN_INFO",true,S.NN(oResultMap.get_UnFormatedValue("INTERN_INFO")),400,8,800);
		...
		private class ownActionSave extends XX_ActionAgent
		{
			...
			public void executeAgentCode(ExecINFO oExecInfo) throws myException 
			{
				...
				if (bibMSG.get_bIsOK())
				{
					recFibu.set_NEW_VALUE_INTERN_INFO(cTextField.get_cString4Database());

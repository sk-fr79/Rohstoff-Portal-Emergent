...
# ============================================================
# GESCHÄFTSLOGIK-VALIDIERUNG (portiert aus Java/Echo2)
# ============================================================
...
@app.post("/api/artikel/validieren")
async def validate_artikel(data: dict, user = Depends(get_current_user)):
    """Validiert einen Artikel nach Geschäftslogik"""
...
@app.post("/api/kontrakte/validieren")
async def validate_kontrakt(data: dict, user = Depends(get_current_user)):
    """Validiert einen Kontrakt nach Geschäftslogik"""
...
@app.post("/api/fuhren/validieren")
async def validate_fuhre(data: dict, user = Depends(get_current_user)):
    """Validiert eine Fuhre nach Geschäftslogik"""
...
async def update_kontrakt(
    ...
    if not skip_validation:
        # Merge existing data with update data for validation
        merged = {**existing, **update_data}
        validation_result = await KontraktValidator.validate(merged, db)
        if not validation_result.is_valid:
            return {
                "success": False,
                "error": "Validierungsfehler",

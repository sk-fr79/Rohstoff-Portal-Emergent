...
public class Param_Date extends ParamDataObject {

	private boolean _value_is_sql_plain = false;
	
	
	public Param_Date(String name, Date Value) {
		this.dValue = Value;
	}
	
	public Param_Date(String name){
		super(name);
	}
	
	public Param_Date (Date value){
		this.dValue = value;
	}
	
	

	public Param_Date(String value, String Format) throws myException{
	...
	public void CopyFrom(ParamDataObject o) {
		if (o instanceof Param_Date){
			this._value_is_sql_plain = ((Param_Date)o).is_value_is_sql_plain();
			if (this._value_is_sql_plain){
				this.setValue(o.ValuePlain(),this._value_is_sql_plain);
			} else {
				this.setValue(((Param_Date) o).ValueDate());
			}
		}
	}

...
public class DRUCK_LIST_bt_kopie extends RB_bt_CopySingular {

	private E2_NavigationList naviList;

	public DRUCK_LIST_bt_kopie(E2_NavigationList p_naviList) {
	...
	public RB_DataobjectsCollector generate_DataObjects4Edit() throws myException {
        ...
        if (v_ids.size()==1) {
            collector = new DRUCK_MASK_DataObjectCollector(v_ids.get(0),RB__CONST.MASK_STATUS.NEW_COPY);
        } else {
        	collector=null;
        	bibMSG.add_MESSAGE(new MyE2_Alarm_Message(new MyE2_String("Sie haben keinen Drucker zum Kopieren ausgewählt !")));
        }
        return collector;
	}

	@Override
	public void define_Actions_4_saveButton(RB_bt_CopySingular btEditSingular,
			RB_bt_maskSaveAndClose bt_saveAndClose_In_Mask, RB_ModuleContainerMASK maskPopup) throws myException {
		bt_saveAndClose_In_Mask.add_oActionAgent(new ownActionSave(maskPopup));
	}

	@Override
	public void define_Actions_4_CloseButton(RB_bt_CopySingular btEditSingular, RB_bt_maskClose bt_Close,
	...
	private class ownActionSave extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			DRUCK_LIST_bt_kopie oThis = DRUCK_LIST_bt_kopie.this;
			...
			if (oMV.get_bIsOK()) {
				...
				Rec20 r_new = ((RB_Dataobject_V2)oThis.get_MaskContainer().rb_FirstAndOnlyComponentMapCollector().rb_Actual_DataobjectCollector().rb_hm_DataObjects().get(new _DRUCK_KEY())).rec20().get_rec_after_save_new();

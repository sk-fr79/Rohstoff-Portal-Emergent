...
import panter.gmbh.Echo2.__BASIC_MODULS.SCANNER.SCAN_CONST.SCAN_BUTTONS;
...
public class SCAN_LIST_bt_kopie extends RB_bt_CopySingular {

	private E2_NavigationList naviList;

	public SCAN_LIST_bt_kopie(E2_NavigationList p_naviList) {
		this.naviList = p_naviList;
	    this.add_GlobalValidator(new E2_ButtonAUTHValidator_AUTO(SCAN_BUTTONS.NEW.db_val()));	
	}
	...
	public RB_DataobjectsCollector generate_DataObjects4Edit() throws myException {
        ...
        if (v_ids.size()==1) {
            collector = new SCAN_MASK_DataObjectCollector(v_ids.get(0),RB__CONST.MASK_STATUS.NEW_COPY);
        } else {
        	collector=null;
        	bibMSG.add_MESSAGE(new MyE2_Alarm_Message(new MyE2_String("Sie haben keinen Scanner zum Kopieren ausgewählt !")));
        }
        return collector;
	}
	...
	private class ownActionSave extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			SCAN_LIST_bt_kopie oThis = SCAN_LIST_bt_kopie.this;
			...
			if (oMV.get_bIsOK()) {
				...
				Rec20 r_new = ((RB_Dataobject_V2)oThis.get_MaskContainer().rb_FirstAndOnlyComponentMapCollector().rb_Actual_DataobjectCollector().rb_hm_DataObjects().get(new _SCAN_KEY())).rec20().get_rec_after_save_new();

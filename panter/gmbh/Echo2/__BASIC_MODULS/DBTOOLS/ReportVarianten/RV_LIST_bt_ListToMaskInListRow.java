...
public class RV_LIST_bt_ListToMaskInListRow extends RB_BtV4_List2Mask {
    
    public RV_LIST_bt_ListToMaskInListRow(boolean bEdit, RB_TransportHashMap  p_tpHashMap) {
        super(bEdit);
        this._setTransportHashMap(p_tpHashMap);
   
        this.add_GlobalValidator(bEdit?RV_VALIDATORS.EDIT.getValidator():RV_VALIDATORS.EDIT.getValidator());
        
    }
    @Override
    public RB_ModuleContainerMASK generateMaskContainer() throws myException {
    	this.getTransportHashMap()._setRBModulContainerMask(
    			new RV_MASK_MaskModulContainer(this.getTransportHashMap()));
    ...
    public RB_hm_multi_DataobjectsCollector generateDataObjects4Mask(MyE2_MessageVector mv_sammler) throws myException {
    	...
    	RV_LIST_bt_ListToMaskInListRow oThis = RV_LIST_bt_ListToMaskInListRow.this;
		...
		if (this.EXT().get_oComponentMAP()!=null) {
			...
			if (id.isOK()) {
	            ...
	            collector.put(id.get_cUF_LongString(), new RV_MASK_DataObjectCollector(oThis.getTransportHashMap(), id.get_cUF_LongString(),aktuellerStatus));
	            return collector;
			} else {
    ...
    public MyE2_String generateTitelInfo4MaskWindow() throws myException {
       	MyE2_String bearbeiten = S.ms("Bearbeiten eines Datensatzes vom Typ: ").ut(RV_CONST.RV_NAMES.DATASET_NAME.getUserText().CTrans());
    	MyE2_String anzeigen  = S.ms("Anzeige eines Datensatzes vom Typ: ").ut(RV_CONST.RV_NAMES.DATASET_NAME.getUserText().CTrans());
    ...
    public MyE2_String generateMessagetextForSaveRecord() throws myException {
         return S.ms("Datensatzes vom Typ: ").ut(RV_CONST.RV_NAMES.DATASET_NAME.getUserText().CTrans()).t(" wurde gespeichert");
    }
    ...
    public E2_Break4PopupController getBreak4PopupController4Save() throws myException {
  	   return new E2_Break4PopupController()._registerBreak(
			   new RV_Break4PopupWhenFalseParameter(this.getTransportHashMap().getMaskComponentMapCollector(), this.getTransportHashMap())
			   );
   }
    
    @Override
    public E2_Break4PopupController getBreak4PopupController4Cancel() throws myException {
        return new E2_Break4PopupController()
        		._registerBreak(new Break4MaskCloseWhenSomethingWasChanged(this.getTransportHashMap().getRBModulContainerMask().rb_FirstAndOnlyComponentMapCollector(), null))
			   	._registerBreak(new RV_Break4PopupWhenFalseParameter(this.getTransportHashMap().getMaskComponentMapCollector(), this.getTransportHashMap())
        );
    }
	...
	public Object get_Copy(Object objHelp) throws myExceptionCopy {
		RV_LIST_bt_ListToMaskInListRow copy= new RV_LIST_bt_ListToMaskInListRow(this.isUsedToEdit(), this.getTransportHashMap());

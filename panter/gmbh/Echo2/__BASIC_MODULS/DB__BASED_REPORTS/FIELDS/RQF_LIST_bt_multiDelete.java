...
public class RQF_LIST_bt_multiDelete extends RB_BtV4_Delete {
	
    
    public RQF_LIST_bt_multiDelete(RB_TransportHashMap  p_tpHashMap)    {
        super();
        this._image(E2_ResourceIcon.get_RI("delete.png") , E2_ResourceIcon.get_RI("leer.png"));
        
        this._setTransportHashMap(p_tpHashMap);
        
        this.add_GlobalValidator(RQF_VALIDATORS.DELETE.getValidator());
	...
	public MyE2_String get_warnung_achtung_es_wird_ein_irgendwas_geloescht() {
		return S.ms("Soll diese Listenspalten für abfragebasierte Reports-Definition geloescht werden ?");	
    }
	@Override
	public String get_warnung_achtung_es_werden_n_irgendwas_geloescht_mit_platzhalter_fuer_zahl() {
		return "Sollen diese #WERT# Listenspalten für abfragebasierte Reports-Definitionen geloescht werden ?";
	}
	...
	public Vector<String> get_delete_sql_statements(String id_to_delete, MyE2_MessageVector mv) throws myException {
		...
		if (lid.isOK()) {
			Rec21 rec = new Rec21(_TAB.reporting_query_field)._fill_id(lid.get_oLong());

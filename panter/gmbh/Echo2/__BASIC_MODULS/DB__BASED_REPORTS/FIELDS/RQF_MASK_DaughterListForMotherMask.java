 ...
 */
public class RQF_MASK_DaughterListForMotherMask extends RB_MaskDaughterBasedOnBasicModulContainer {
 	
 	//fieldKey fuer das einbinden dieser komponente in einer mother-mask
	public static RB_KF  keyForMotherMask = (RB_KF)new RB_KF()._setHASHKEY("RQF_REPORTING_QUERY_FIELD_MASK_DaughterListForMotherMask")._setREALNAME("RQF_REPORTING_QUERY_FIELD_MASK_DaughterListForMotherMask");
    ...
    private RQF_LIST_BedienPanel 	bedienPanel = null;
    private VEK<MyE2IF__Component>  toDisableAtMask = new VEK<MyE2IF__Component>();
    private RQF_LIST_DATASEARCH  	searcher = null;
     ...
     * @throws myException
     */
    public RQF_MASK_DaughterListForMotherMask(RB_TransportHashMap p_tpHashMap, boolean useCompactList) throws myException {
        super();
	...
	public IF_Field getLinkfieldToMotherTable() {
		return REPORTING_QUERY_FIELD.id_reporting_query;
	}
	...
	public class InternalLIST_BasicModuleContainer extends Project_BasicModuleContainer    {
	    ...
	    public InternalLIST_BasicModuleContainer(RB_TransportHashMap p_params, boolean useCompactList) throws myException    {
	        ...
	        RQF_MASK_DaughterListForMotherMask oThis = RQF_MASK_DaughterListForMotherMask.this;
	        ...
	        oThis.m_tpHashMap._setLeadingMaskKey(RQF_CONST.getLeadingMaskKey());
	        oThis.m_tpHashMap._setLeadingTableOnMask(RQF_CONST.getLeadingTable());
 
	        this.set_bVisible_Row_For_Messages(false);
 	        
 	        
	        RQF_LIST_ComponentMap listComponentMap  = null;
 	        
	        if (useCompactList) {
                oThis.m_tpHashMap.put(RB_TransportHashMap_ENUM.NAVILIST,new E2_NavigationListCompact());
 
                 
                RQF_LIST_bt_New btNew = new RQF_LIST_bt_New(oThis.m_tpHashMap);
                RQF_LIST_bt_Copy btCopy = new RQF_LIST_bt_Copy(oThis.m_tpHashMap);
                ...
                oThis.searcher = new RQF_LIST_DATASEARCH(oThis.m_tpHashMap);
                ...
                listComponentMap = new RQF_LIST_ComponentMap(oThis.m_tpHashMap);
                ...
                oThis.bedienPanel = new RQF_LIST_BedienPanel(RQF_MASK_DaughterListForMotherMask.this.m_tpHashMap);
		...
		public void fillComponents(E2_ComponentMAP oMAP, SQLResultMAP oUsedResultMAP) throws myException {
			if (!m_enabled) {
				...
				for (MyE2IF__Component c: oMAP.get_hmRealComponents().values()) {
					if (c instanceof RQF_LIST_bt_DeleteInListRow) {
						((RQF_LIST_bt_DeleteInListRow)c).setEnabledForEditOfSuperClass(false);
					}
					if (c instanceof RQF_LIST_bt_ListToMaskInListRow) {
						if ( ((RQF_LIST_bt_ListToMaskInListRow)c).isUsedToEdit()) {
							((RQF_LIST_bt_ListToMaskInListRow)c).setEnabledForEditOfSuperClass(false);
						}
					}

...
public class ListDef
{
	 ...
	 * @param newAllowed
	 */
	public ListDef(		String ctablename, 
			...
			oButtonRueck.add_oActionAgent(new XX_ActionAgent()
					{
						public void executeAgentCode(ExecINFO oExecInfo)  throws myException
						{
							E2_ConfirmBasicModuleContainer oBasicContainer = new E2_ConfirmBasicModuleContainer(
									 ...
									 null,
									 new MyE2_String(ListDef.this.HELPTEXT,false),
									 new MyE2_String("OK"),
									 null,
									 new Extent(900),
									 new Extent(400));
	...
	public E2_ComponentMAP build_ComponentMAP(SQLFieldMAP oSQLFieldMAP) throws myException
	{
		...
		{
			ListDef.ListColumn oCol = (ListDef.ListColumn)this.VECTORCOLUMNS.get(i);
			...
			if (vFields.size()==1)
			{
				 ...
				 */
				ListDef.FieldDef oField = (ListDef.FieldDef) vFields.get(0);
			...
			else
			{
				...
				{
					ListDef.FieldDef oField = (ListDef.FieldDef)vFields.get(k);
					MyE2IF__DB_Component oComp = oField.buildComponent(oSQLFieldMAP);
					oComp.EXT_DB().set_bIsSortable(oField.SORTABLE);
					((MyE2IF__Component)oComp).EXT().set_bDisabledFromBasic(!oField.EDITABLE);
	...
	public E2_ComponentGroupHorizontal build_GridWithSelectors( E2_NavigationList oNaviList, String cMODUL_KENNER) throws myException
	{
		...
		if ((this.VECTORSELECTORSDROPDOWN!=null && this.VECTORSELECTORSDROPDOWN.size()>0) || (this.VECTORSELECTORSCHECKBOX!=null && this.VECTORSELECTORSCHECKBOX.size()>0))
		{
			oCompGrid = 		new E2_ComponentGrid(ListDef.this.NUMBERCOLUMNSINSELECTORGRID);	
			...
			if (this.VECTORSELECTORSDROPDOWN!=null && this.VECTORSELECTORSDROPDOWN.size()>0)
			{
				...
				{
					ListDef.SelectorDefDropDown oSel = (ListDef.SelectorDefDropDown)this.VECTORSELECTORSDROPDOWN.get(i);
			...
			if (this.VECTORSELECTORSCHECKBOX!=null && this.VECTORSELECTORSCHECKBOX.size()>0)
			{
				...
				{
					ListDef.SelectorDefCheckBox oSel = (ListDef.SelectorDefCheckBox)this.VECTORSELECTORSCHECKBOX.get(i);
					E2_ListSelectorStandard oSelStd = oSel.makeSelector();
					oSelVector.add(oSelStd);
											 ...
											 String MODULE_KENNER) throws myException
	{
		...
		if (this.VECTORSEARCHFIELDS!=null && this.VECTORSEARCHFIELDS.size()>0)
		{
			...
			{
				ListDef.SearchDef oSearchDef = (ListDef.SearchDef)this.VECTORSEARCHFIELDS.get(i);
	...
	public class FieldDef
	{
		...
		private MyE2IF__DB_Component buildComponent(SQLFieldMAP oSQLFieldMap) throws myException
		{
			...
			if (this.FIELDTYPE.equals(ListDef.FIELDTYPE_LABEL))
			{
			...
			if (this.FIELDTYPE.equals(ListDef.FIELDTYPE_CHECKBOX))
			{
				SQLField oSQLField = (SQLField)oSQLFieldMap.get(this.FIELDNAME);
				if (oSQLField == null)
					throw new myException("ListDef:FieldDef:buildComponent:FieldName not in SQLFieldMAP: "+this.FIELDNAME);
			...
			*/
			...
			if (this.FIELDTYPE.equals(ListDef.FIELDTYPE_DROPDOWN))
			{
				SQLField oSQLField = (SQLField)oSQLFieldMap.get(this.FIELDNAME);
				if (oSQLField == null)
					throw new myException("ListDef:FieldDef:buildComponent:FieldName not in SQLFieldMAP: "+this.FIELDNAME);
			...
			if (this.FIELDTYPE.equals(ListDef.FIELDTYPE_EDITFIELD))
			{
				SQLField oSQLField = (SQLField)oSQLFieldMap.get(this.FIELDNAME);
				if (oSQLField == null)
					throw new myException("ListDef:FieldDef:buildComponent:FieldName not in SQLFieldMAP: "+this.FIELDNAME);
			...
			if (this.FIELDTYPE.equals(ListDef.FIELDTYPE_EDITPASSWORD))
			{
				SQLField oSQLField = (SQLField)oSQLFieldMap.get(this.FIELDNAME);
				if (oSQLField == null)
					throw new myException("ListDef:FieldDef:buildComponent:FieldName not in SQLFieldMAP: "+this.FIELDNAME);

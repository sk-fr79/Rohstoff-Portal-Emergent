...
public class EXP_bt_export_excel extends MyE2_Button implements IF_components_4_decision{
	...
	public EXP_bt_export_excel(E2_NavigationList p_navigationlist,  String authcode_button) {
		this(p_navigationlist, null, authcode_button);
	}
		
	public EXP_bt_export_excel(E2_NavigationList p_navigationlist) {
		this(p_navigationlist, null, null);
	}

	
	
	public EXP_bt_export_excel(E2_NavigationList p_navigationlist, String authcode_modul, String authcode_button) {
		super(new MyE2_String("Listenexport ins Excelformat"));
		this.navigationlist = p_navigationlist;

		
		if (S.isEmpty(authcode_button)) {
			authcode_button="EXCEL_EXPORT";
		}
		...
		this.add_oActionAgent(new ownAction_start_yes_no(this));
		this.add_oActionAgent(new ownActionAgent());
		
		this.setToolTipText(new MyE2_String("Export der Daten aus der momentanen Selektion in eine Excel-Tabelle").CTrans());
	}

	private class ownActionAgent extends XX_ActionAgent {
		public void executeAgentCode(ExecINFO oExecInfo)	{
			EXP_bt_export_excel oThis = EXP_bt_export_excel.this;
			try	{
				oThis.c_primary_table_name = oThis.navigationlist.get_oComponentMAP__REF().get_oSQLFieldMAP().get_cMAIN_TABLE();
	
				if (S.isFull(oThis.c_primary_table_name)) {
					
					String name_download = oThis.c_primary_table_name+"-export-excel.xls";
					
					
					
					new MyHSSFWorkBookForNavigationList_flat(
							new myTempFile("export","xls",true),name_download,oThis.navigationlist);
	...
	private class ownAction_start_yes_no extends Std_action_popup_frage_yes_no {
		...
		protected void define_buttons_and_fill_grid_in_info_popup(MyE2_Button bt_ok, MyE2_Button bt_cancel,	MyE2_Grid grid_4_info) throws myException {
			...
			grid_4_info.add(new E2_Grid4MaskSimple()
								...
								.add_(new MyE2_Label(new MyE2_String(""+EXP_bt_export_excel.this.navigationlist.get_vectorSegmentation().size()),new E2_FontBold(2)))

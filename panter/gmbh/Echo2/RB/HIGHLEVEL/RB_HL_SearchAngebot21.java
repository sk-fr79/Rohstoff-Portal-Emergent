...
import panter.gmbh.indep.dataTools.TERM.VglDbFieldsContainsDate;
...
public abstract class RB_HL_SearchAngebot21 extends RB_SearchFieldSaveable implements IF_RB_ResultButton_can_edit_searched_record{
	 ...
	 * @throws myException
	 */
	public RB_HL_SearchAngebot21(ENUM_VORGANGSART m_vg_art) throws myException {
		super();
	 ...
	 * @throws myException
	 */
	public RB_HL_SearchAngebot21(ENUM_VORGANGSART m_vg_art, boolean bShowEraser) throws myException {
		super(bShowEraser);
		this.vorgangsart = m_vg_art;
		this._init();
	}
	...
	private RB_HL_SearchAngebot21 _init() throws myException {
		...
		if (this.vorgangsart==null) {
			throw new myException(this,"angebots-typ (EK or VK) undefined!");
		}
		
		this.and_statement_collector_4_basic().and(new vgl(VKOPF_STD.vorgang_typ, this.vorgangsart.db_val()));
		...
		this.and_statement_collector_4_basic().and(new vgl_YN(VPOS_STD.deleted, false));
		this.and_statement_collector_4_basic().and(new vgl_YN(VKOPF_STD.deleted, false));
		...
		this.and_statement_collector_4_search().or(new vgl(new TermSimple("TO_CHAR("+VPOS_STD.id_vpos_std.tnfn()+")"), COMP.EQ, new TermSimple("REPLACE('#WERT#','.','')")));
		this.and_statement_collector_4_search().or(new vgl(new FieldTerm(ATTRIBUTES.UPPER,VKOPF_STD.buchungsnummer), COMP.LIKE, new TermSimple("UPPER('%#WERT#%')")));
		...
		this.set_key_4_save_sorting(ENUM_USER_SAVEKEY.SAVEING_SORTING_SEARCHFIELD_RB_HL_SEARCH_ANGEBOT);
	...
	public  XX_ActionAgent  generate_EraseButtonAction() throws myException {
		return new XX_ActionAgent() {
			...
			public void executeAgentCode(ExecINFO oExecInfo) throws myException {
				bibMSG.add_MESSAGE(RB_HL_SearchAngebot21.this.rb_set_db_value_manual("", true,true));
			}
		};
	}
	...
	private class ActionStartSearch extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			...
			RB_HL_SearchAngebot21 oThis = RB_HL_SearchAngebot21.this;
			...
			Date  			datumFuhre = oThis.getActualDate();
			...
			if (datumFuhre!=null) {
				...
				//!!! and_statement_collector_4_action() ist immer minimum 1 lang, wegen der zeitbeschränkung
				oThis.and_statement_collector_4_action()
						._clear()
						.and(new VglDbFieldsContainsDate(VPOS_STD_ANGEBOT.gueltig_von, datumFuhre, VPOS_STD_ANGEBOT.gueltig_bis))
						;
				//auf jeden fall muss in die suche die zeitbeschraenkung
				...
				if (idAdresse !=null || idArtikel!=null) {
					...
					if (idArtikel != null) {
						oThis.and_statement_collector_4_action().and(new vgl(VPOS_STD.id_artikel,idArtikel.toString()))
																;
					}
					...
					oThis.and_statement_collector_4_action().and(new vgl(ADRESSE.id_adresse,this.pairAdressAndSorte.getVal2().getIdLong().toString()))
															.and(new vgl(VPOS_STD.id_artikel,this.pairAdressAndSorte.getVal1().getIdLong().toString()))
															;
				}
				
				//wenn beides leer ist, dann ein popup vorschalten, das 
				if (S.isEmpty(c_text_4_search) && oThis.and_statement_collector_4_action().size()==1) {
					//dann hier eine vorabfrage nach firmen und sorten und diese via popup starten
					new PopupWithPreSelect(datumFuhre);
				...
				bibMSG.MV()._addAlarm(S.ms("Bitte zuerst das Datum der Warenbewegung festlegen !"));
			}
		}
	...
	public MyE2_MessageVector  execute_searchquery_and_fill_resultbutton_array(String search_text) throws myException {
		...
		if (mv.get_bIsOK()) {
			...
			for (IF_Field f: VPOS_STD.values()) {
				v_names.add(f.tnfn());
			}
			
			SEL  s_query = new SEL(_TAB.vpos_std).FROM(_TAB.vpos_std)
		...
		//	DEBUG.System_println(c_sql_query,DEBUG.DEBUG_FLAGS.MARTINS_EIGENER.name());
			...
			RecList21 rlVposKon = new RecList21(_TAB.vpos_std)._fill(c_sql_query);
	...
	private void fillResultButtonArray(RecList21 rlVPosStd) throws myException {
		
		RB_ResultButtonArrays  v_result_arrays = this.get_rb_ResultButtonArray();
		v_result_arrays.clear();

		
		for (Rec21 r: rlVPosStd) {
			...
			BigDecimal bdMenge = 				O.NN(recVposStd.get_raw_resultValue_bigDecimal(VPOS_STD.anzahl),BigDecimal.ZERO);
			BigDecimal bdGeliefert = 			O.NN(recVposStd.get_gesamt_fuhre_menge().get_bdWert(), BigDecimal.ZERO);
			...
			v_selColumns._a(new TRIPLE<String,String,MutableStyle>(	recVkopf.getFs(VKOPF_STD.buchungsnummer, "<"+recVkopf.getFs(VKOPF_STD.id_vkopf_std)+">")+"-"+recVposStd.getFs(VPOS_STD.positionsnummer,"0"),
																	recVkopf.getFs(VKOPF_STD.buchungsnummer, "<"+recVkopf.getFs(VKOPF_STD.id_vkopf_std)+">")+"-"+recVposStd.getFs(VPOS_STD.positionsnummer,"0"),
																	RB_HL_SearchAngebot21.styleListButtonsLeft()));


			v_selColumns._a(new TRIPLE<String,String,MutableStyle>(	recVkopf.getFs(VKOPF_STD.druckdatum, "-"),
																	recVkopf.getFs(VKOPF_STD.druckdatum, "-"),
																	RB_HL_SearchAngebot21.styleListButtonsMid()));
			...
			v_selColumns._a(new TRIPLE<String,String,MutableStyle>(	new MyBigDecimal(bdMenge).get_FormatedRoundedNumber(3),
																	...
																	RB_HL_SearchAngebot21.styleListButtonsRight()));
			...
			v_selColumns._a(new TRIPLE<String,String,MutableStyle>(	recVposStd.get_ufs_kette(" - " , VPOS_STD.anr1,VPOS_STD.anr2), 
																	recVposStd.get_ufs_kette(" - " , VPOS_STD.anr1,VPOS_STD.anr2),
																	RB_HL_SearchAngebot21.styleListButtonsLeft()));
			 
			v_selColumns._a(new TRIPLE<String,String,MutableStyle>(	recVposStd.getUfs(VPOS_STD.artbez1), 
																	recVposStd.getUfs(VPOS_STD.artbez1),
																	RB_HL_SearchAngebot21.styleListButtonsLeft()));
			...
			v_selColumns._a(new TRIPLE<String,String,MutableStyle>(	recVposStd.getGueltigkeitsZeitraum(),
																	recVposStd.getGueltigkeitsZeitraum(),
																	RB_HL_SearchAngebot21.styleListButtonsMid()));

			int size = v_selColumns.size();
			
			RB_ResultButton[] res_zeile = new RB_ResultButton[size];
			
			for (int i=0;i<size; i++) {
				res_zeile[i]= new own_result_button(this,recVposStd, v_selColumns.get(i).getVal1(),v_selColumns.get(i).getVal2(),v_selColumns.get(i).getVal3());
			}
	...
	public void  fill_grid_4_popup(MyE2_Grid grid_4_popup, RB_ResultButtonArrays vektor_buttons, ExecINFO oExecInfo) throws myException {
		...
		v_buttons.add(new RB_SearchFieldListSortButton(this,0,  new MyE2_String("Buchung-Nr."),	RB_HL_SearchAngebot21.styleTitleButtonsLeft()));
		v_buttons.add(new RB_SearchFieldListSortButton(this,2,  new MyE2_String("Datum"),		RB_HL_SearchAngebot21.styleTitleButtonsLeft()));
		v_buttons.add(new RB_SearchFieldListSortButton(this,3,  new MyE2_String("Firma"),		RB_HL_SearchAngebot21.styleTitleButtonsLeft()));
		v_buttons.add(new RB_SearchFieldListSortButton(this,4,  new MyE2_String("Menge"),		RB_HL_SearchAngebot21.styleTitleButtonsRight()));
		v_buttons.add(new RB_SearchFieldListSortButton(this,5,  new MyE2_String("Offen"),		RB_HL_SearchAngebot21.styleTitleButtonsRight()));
		v_buttons.add(new RB_SearchFieldListSortButton(this,6,  new MyE2_String("Sorte"),		RB_HL_SearchAngebot21.styleTitleButtonsLeft()));
		v_buttons.add(new RB_SearchFieldListSortButton(this,7,  new MyE2_String("Artikelbez"),	RB_HL_SearchAngebot21.styleTitleButtonsLeft()));
		v_buttons.add(new RB_SearchFieldListSortButton(this,8,  new MyE2_String("Preis"),		RB_HL_SearchAngebot21.styleTitleButtonsRight()));
		v_buttons.add(new RB_SearchFieldListSortButton(this,9,  new MyE2_String("Gültig"),		RB_HL_SearchAngebot21.styleTitleButtonsMid()));
		...
		grid._s(9)	._a(v_buttons.get(0), new RB_gld()._col_back_d()._ins(2)._left_mid())
					._a(v_buttons.get(1), new RB_gld()._col_back_d()._ins(2)._center_mid())
					._a(v_buttons.get(2), new RB_gld()._col_back_d()._ins(2)._left_mid())
					._a(v_buttons.get(3), new RB_gld()._col_back_d()._ins(2)._right_mid())
					._a(v_buttons.get(4), new RB_gld()._col_back_d()._ins(2)._right_mid())
					._a(v_buttons.get(5), new RB_gld()._col_back_d()._ins(2)._left_mid())
					._a(v_buttons.get(6), new RB_gld()._col_back_d()._ins(2)._left_mid())
					._a(v_buttons.get(7), new RB_gld()._col_back_d()._ins(2)._right_mid())
					._a(v_buttons.get(8), new RB_gld()._col_back_d()._ins(2)._center_mid())
					;
	...
	public RB_HL_SearchAngebot21 _setTextFieldInputWidth(int textFieldInputWidth) {
	...
	private class PopupWithPreSelect extends E2_BasicModuleContainer {
		...
		public PopupWithPreSelect(Date datumFuhre) throws myException {
			super();
			
			this.add(grid, E2_INSETS.I(2,2,2,2));
			
			SEL  s_query = new SEL(VPOS_STD.id_artikel, VKOPF_STD.id_adresse).ADD_Distinct().FROM(_TAB.vpos_std)
						.INNERJOIN(_TAB.vkopf_std, VPOS_STD.id_vkopf_std, VKOPF_STD.id_vkopf_std)
						.INNERJOIN(_TAB.vpos_std_angebot, VPOS_STD.id_vpos_std, VPOS_STD_ANGEBOT.id_vpos_std)
						.INNERJOIN(_TAB.adresse,   VKOPF_STD.id_adresse,  ADRESSE.id_adresse)
						.WHERE(new vgl(VKOPF_STD.vorgang_typ,RB_HL_SearchAngebot21.this.vorgangsart.db_val()))
						.AND(new VglDbFieldsContainsDate(VPOS_STD_ANGEBOT.gueltig_von, datumFuhre, VPOS_STD_ANGEBOT.gueltig_bis))
						.AND(new vgl_YN(VKOPF_STD.deleted, false))
			...
			this.CREATE_AND_SHOW_POPUPWINDOW(new Extent(800),new Extent(800),S.ms("Auswahl mögliche Angebote ..."));
		}
		...
		private void rebuild() throws myException {
			...
			String typ = RB_HL_SearchAngebot21.this.vorgangsart==ENUM_VORGANGSART.ABNAHMEANGEBOT?"Lieferanten":"Abnehmer";
			this.grid._a(new RB_lab()._t(S.ms("Angebote folgender Kombinationen aus "+typ+" und Sorte sind für den Zeitraum vorhanden:")), new RB_gld()._ins(5,5,5,5)._span(2));

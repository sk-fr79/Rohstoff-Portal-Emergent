...
public abstract class RB_HighLevelSearchAVVCode extends E2_Grid  implements IF_RB_Component_Savable, E2_IF_Copy, MyE2IF_IsMarkable {

	/**
	 * abstracts
	 */
	public abstract Rec21 get_actual_artikel() throws myException;
	...
	public RB_HighLevelSearchAVVCode() throws myException {
		super();
		this._bo(null);
		...
		this.tf_4_id._f(new E2_FontItalic(-2));
		this.tf_4_anzeige._f(new E2_FontItalic(-2));
	...
	private class ownActionErase extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			RB_HighLevelSearchAVVCode.this.tf_4_anzeige.setText("");
			RB_HighLevelSearchAVVCode.this.tf_4_id.setText("");
		}
	}

	public String hole_StandardAVV_Code_kleinanlieferer() throws myException {
		Rec21 art = this.get_actual_artikel();
		if (art == null) {
			bibMSG.add_MESSAGE(new MyE2_Alarm_Message("Bitte zuerst die Sortenbezeichung definieren !"));
			return null;
		} else {
			if (S.isFull(art.getUfs(ARTIKEL.id_eak_code, ""))) {
				this.found_and_stored_code = new RECORD_EAK_CODE(art.getUfs(ARTIKEL.id_eak_code, ""));
				this.tf_4_id.setText(art.getFs(ARTIKEL.id_eak_code));
				this.FillLabelWithDBQuery(art.getUfs(ARTIKEL.id_eak_code));
				return art.getUfs(ARTIKEL.id_eak_code);
			} else {
	...
	public String hole_StandardAVV_Code_ex_mandant() throws myException {
		...
		if (art == null) {
			...
			if (S.isFull(art.getUfs(ARTIKEL.id_eak_code_ex_mandant, ""))) {
				this.found_and_stored_code = new RECORD_EAK_CODE(art.getUfs(ARTIKEL.id_eak_code_ex_mandant, ""));
				this.tf_4_id.setText(art.getFs(ARTIKEL.id_eak_code_ex_mandant));
				this.FillLabelWithDBQuery(art.getUfs(ARTIKEL.id_eak_code_ex_mandant));
				return art.getUfs(ARTIKEL.id_eak_code_ex_mandant);
			} else {
				this.tf_4_id.setText("");
				this.FillLabelWithDBQuery("");
				return null;
	...
	private class actionStartSearch extends XX_ActionAgent {
		...
		public void executeAgentCode(ExecINFO oExecInfo) {
			RB_HighLevelSearchAVVCode oThis = RB_HighLevelSearchAVVCode.this;
		...
		private class actionAgentListBoxCode extends XX_ActionAgent {
			public void executeAgentCode(ExecINFO oExecInfo) {
				...
				try {
					...
					if (RB_HighLevelSearchAVVCode.this.oActionAfterFound != null) {
						RB_HighLevelSearchAVVCode.this.oActionAfterFound.executeAgentCode(oExecInfo);
					}
	...
	public void rb_Datacontent_to_Component(RB_Dataobject dataObject) throws myException {
		if (dataObject.get_RecORD() == null) {
			...
			/*20190318 aenderung Martin  alt: this.tf_4_id.setText(((MyRECORD) dataObject.get_RecORD()).ufs(this.rb_KF().FIELDNAME(), ""));*/
			this.tf_4_id.setText(((MyRECORD) dataObject.get_RecORD()).fs(this.rb_KF().FIELDNAME(), ""));
		}

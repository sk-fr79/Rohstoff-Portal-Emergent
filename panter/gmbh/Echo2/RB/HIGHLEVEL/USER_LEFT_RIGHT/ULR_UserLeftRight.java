...
public abstract class ULR_UserLeftRight extends LR_Container2 implements IF_RB_Component_Complex, LR_IF_wrap_Component {

	
	// der pool an moeglichen usern
	private RECLIST_USER        	       	rl_user_pool = null;
	
	private SEL  							selectUserList =						null;		//SEL-objekt, definiert welche user zur verfuegung gestellt werden (die auswahl, z.b. nur buerouser)
	private IF_Field    					ref_pk_field_main_table_in_crosstable = null;		//z.B. laufzettel_user.id_laufzettel
	
	private Vector<MyRECORD>   				rec_defined_crosstable_after_load = 	new Vector<>();   //hier werden die nach dem laden vorhandenen RECORD_<crosstable> abgelegt
	private Vector<RB_Validator_Component> 	vVALIDATORS_4_INPUT = 					new Vector<RB_Validator_Component>();
	
	private RB_KF       					key = null;

	
	public ULR_UserLeftRight() throws myException {
		super();
	}

	
	// MUSS ausggefuehrt werden
	public void _init(SEL _selectUserList, IF_Field _ref_pk_field_main_table_in_crosstable) throws myException {
		this.selectUserList = 							_selectUserList; 
		this.ref_pk_field_main_table_in_crosstable = 	_ref_pk_field_main_table_in_crosstable;
		this.rl_user_pool = new RECLIST_USER(this.selectUserList.s());
	}
	...
	public Vector<MySqlStatementBuilder> get_vSQL_StatementBuilders_Others(MyE2_MessageVector oMV, RB_ComponentMap rb_maskThisBelongsTo) throws myException {
		...
		for (MyRECORD rec: this.rec_defined_crosstable_after_load) {
			String del_string = "DELETE FROM "+bibE2.cTO()+"."+crossTable.fullTableName()+" WHERE "+crossTable.keyFieldName()+"="+rec.ufs(crossTable.key());
	...
	public abstract Vector<MyRECORD> query_rosstable_records_to_main_id(String id_bordercrossing) throws myException; 
	
	public abstract RECORD_USER extract_user_from_crosstable(MyRECORD rec_cross_tab) throws myException;
	...
	public MyE2_MessageVector makeBindingDaughterRecords_to_MotherTable(RB_Dataobject DataObjectMother, Vector<MyRECORD_IF_FILLABLE> v_DaughterRecords) throws myException {
		...
		if (DataObjectMother.rb_relevant_record_to_fill() instanceof MyRECORD_IF_RECORDS) {
			cAddonVal = ((MyRECORD)DataObjectMother.rb_relevant_record_to_fill()).get_UnFormatedValue(this.rb_KF().get_REALNAME());
		} else {
	...
	public void set_selectUserList(SEL _selectUserList) {
		this.selectUserList = _selectUserList;
	}
	...
	public Vector<MyRECORD> get_vector_defined_crosstable_after_load() {
	...
	public void rb_Datacontent_to_Component(RB_Dataobject dataObject) throws myException {
		...
		if (((MyRECORD)dataObject.get_RecORD())!=null) {
			//sonst neueintrag
			id_main_table = ((MyRECORD)dataObject.get_RecORD()).ufs(this.rb_KF().get_data_field());
		}
	...
	public RECLIST_USER get_rl_user_pool() {
		return rl_user_pool;
	}

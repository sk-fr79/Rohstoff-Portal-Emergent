...
public abstract class RB_HL_SelectLieferAdressenWithFunctionalMainAdress extends RB_SearchFieldSaveable  implements IF_RB_ResultButton_can_edit_searched_record{
...
//	private boolean  		add_edit_button = true;
	...
	public RB_HL_SelectLieferAdressenWithFunctionalMainAdress() throws myException {
		super();
		this._init();
	}

	public RB_HL_SelectLieferAdressenWithFunctionalMainAdress(boolean bShowEraser) throws myException {
		super(bShowEraser);
		this._init();
	}

	//waehrend der laufzeit die basisadresse ermitteln
	public abstract Long readActualBaseAdress();
	...
	private void _init() throws myException {
		...
		this.get_buttonStartSearch().setToolTipText(new MyE2_String("Lieferadressen laden",false).CTrans());
	...
	public  XX_ActionAgent  generate_EraseButtonAction() throws myException {
		return new XX_ActionAgent() {
			...
			public void executeAgentCode(ExecINFO oExecInfo) throws myException {
				RB_HL_SelectLieferAdressenWithFunctionalMainAdress.this.rb_set_db_value_manual("",true,true);
			}
		};
	}
	...
	public MyE2_MessageVector  execute_searchquery_and_fill_resultbutton_array(String p_searchText) throws myException {
		...
		Long idAdresseBasis = this.readActualBaseAdress();
		
		if (idAdresseBasis==null) {
			return bibMSG.getNewMV()._addAlarm(S.ms("Es konnte keine zugehörige Hauptadresse gefunden werden !"));
		}
		
		String s_idAdresseBasis = idAdresseBasis.toString();
		
		And and_statement_collector_4_basic1 = new And().and(new vgl(ADRESSE.adresstyp, ""+myCONST.ADRESSTYP_FIRMENINFO)).and(new vgl(ADRESSE.aktiv, "Y"))
														.and(new vgl(ADRESSE.id_adresse,COMP.EQ,s_idAdresseBasis));
														;
		And and_statement_collector_4_basic2 = new And().and(new vgl(ADRESSE.adresstyp, ""+myCONST.ADRESSTYP_LIEFERADRESSE)).and(new vgl(ADRESSE.aktiv, "Y"));

			
		RECORD_ADRESSE  		rec_eigene = 			new RECORD_ADRESSE(s_idAdresseBasis);
		RECLIST_LIEFERADRESSE 	rleigene = 				rec_eigene.get_DOWN_RECORD_LIST_LIEFERADRESSE_id_adresse_basis();
		Vector<String> 			eigene_lieferadressen = bibVECTOR.get_Vector(rleigene.get_ID_ADRESSE_LIEFER_hmString_UnFormated("-1").values());
		

		and_statement_collector_4_basic2.and(
		...
		if (S.isFull(search_text.trim())) {
			...
			v_wheres1.add("("+and_statement_collector_4_basic1.s()+")");
			v_wheres2.add("("+and_statement_collector_4_basic2.s()+")");
			...
			RECORD_ADRESSE_extend rae = new RECORD_ADRESSE_extend(s_idAdresseBasis);
	...
	private class own_result_button extends RB_ResultButton {
		...
		public own_result_button(	RB_SearchFieldSaveable 		calling_searchField, 
			...
			result_record = 	p_result_record;
			
			this._aaa(()-> {
				String ret_val = calling_searchField.get_result_string_from_record(result_record);
				MyE2_MessageVector mv = calling_searchField.rb_set_db_value_manual(ret_val,true,true);
				if (get_container_4_popupWindow()!=null) {
					get_container_4_popupWindow().CLOSE_AND_DESTROY_POPUPWINDOW(true);
				}
				bibMSG.MV()._add(mv);
			});
		}
	...
	private class ownActionStartSearch extends XX_ActionAgent {
		...
		@Override
		public void executeAgentCode(ExecINFO oExecInfo) throws myException {
			RB_HL_SelectLieferAdressenWithFunctionalMainAdress oThis = RB_HL_SelectLieferAdressenWithFunctionalMainAdress.this;
			...
			try	{
				...
				bibMSG.add_MESSAGE(oThis.execute_searchquery_and_fill_resultbutton_array(S.NN(oThis.get_tf_search_input().getText())));
	...
	private class ownButtonEditAdresse extends E2_BtEditAdress {
		...
		public MyLong find_id_adress() throws myException {
			...
			if (RB_HL_SelectLieferAdressenWithFunctionalMainAdress.this.is_search_done_correct()) {
				c_actual_station = RB_HL_SelectLieferAdressenWithFunctionalMainAdress.this.rb_readValue_4_dataobject();

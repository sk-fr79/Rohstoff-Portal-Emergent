...
public abstract class RB_actionStandardSaveAndReopen_V3 extends XX_ActionAgent {

	private RB_KM 							mask_key_main = null;
	private _TAB  							leading_table_on_mask = null;
	
	private MASK_STATUS   					statusAfterReload = null;
	
	private E2_NavigationList    			m_naviList = null;
	
	
	public MASK_STATUS getStatusAfterReload() {
		return statusAfterReload;
	}


	public RB_actionStandardSaveAndReopen_V3(MASK_STATUS  statusAfterReload, E2_NavigationList   naviList) {
		super();
		this.statusAfterReload = statusAfterReload;
		...
		if (component_map_collector==null) {
			...
			if (status_old.isStatusNew()) {
				...
				if (mv.get_bIsOK()) {
					...
					if (S.isFull(new_id)) {
						RB_DataobjectsCollector od_coll = this.generate_dataObjectsCollector_4_editOrView(new_id,component_map_collector, this.statusAfterReload);
						mv.add_MESSAGE(component_map_collector.rb_COMPLETE_FILL_CYCLE(od_coll));
						if (this.m_naviList!=null) {
				            m_naviList.ADD_NEW_ID_TO_ALL_VECTORS(new_id);
				...
				if (S.isFull(id_edit)) {
					mv.add_MESSAGE(component_map_collector.rb_COMPLETE_SAVE_CYCLE(true));
					if (mv.get_bIsOK()) {
						if (component_map_collector.rb_Actual_DataobjectCollector() instanceof RB_DataobjectsCollector_V2) {
							((RB_DataobjectsCollector_V2)component_map_collector.rb_Actual_DataobjectCollector()).rb_RebuildAllRecords();
							component_map_collector.rb_CompleteMaskReload(statusAfterReload);
							if (this.m_naviList!=null) {
					            m_naviList._REBUILD_ACTUAL_SITE("");
							}

						} else {
							throw new myException(this,"Only in V2-variants useable !");
						}
	...
	public abstract RB_DataobjectsCollector 	generate_dataObjectsCollector_4_editOrView(	String 						id_record
																							,RB_ComponentMapCollector 	componentmapCollectorActual
																							,MASK_STATUS 				statusAfterReload) throws myException;
	...
	public RB_actionStandardSaveAndReopen_V3 _set_mask_key_main(RB_KM p_mask_key_main) {
		this.mask_key_main = p_mask_key_main;
		return this;
	}


	public RB_actionStandardSaveAndReopen_V3 _set_leading_table_on_mask(_TAB p_leading_table_on_mask) {

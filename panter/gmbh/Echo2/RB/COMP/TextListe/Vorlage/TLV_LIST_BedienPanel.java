...
public class TLV_LIST_BedienPanel extends E2_Grid {
    
    private TLV_LIST_Selector  TEXT_LISTE_VORLAGE_LIST_Selector = null;
    ...
    private TLV_LIST_DATASEARCH    dataSearch = null;
    
    
    public TLV_LIST_DATASEARCH getDataSearch() {
		return dataSearch;
	}
    
    public TLV_LIST_BedienPanel(RB_TransportHashMap  p_params) throws myException     {
        super();
        this.setSize(2);
        ...
        this.TEXT_LISTE_VORLAGE_LIST_Selector = new TLV_LIST_Selector(tpHashMap);
       // this._a(TEXT_LISTE_VORLAGE_LIST_Selector,   	new RB_gld()._ins(0,2,10,2)._span(2)._left_mid());
        this._a(grid4Components,   							new RB_gld()._ins(0,2,10,2)._left_mid());
        this._a(this.dataSearch = new TLV_LIST_DATASEARCH(tpHashMap), 	new RB_gld()._ins(0,2,10,2)._left_mid());
        ...
        grid4Components._a((Component)this.toDisableAtMask._ar(new TLV_LIST_bt_ListToMask(true,tpHashMap)),   new RB_gld()._ins(2,2,10,2)._left_mid());
        grid4Components._a(new TLV_LIST_bt_ListToMask(false,tpHashMap),   new RB_gld()._ins(2,2,10,2)._left_mid());
        
        grid4Components._a((Component)this.toDisableAtMask._ar(new TLV_LIST_bt_New(tpHashMap)),  		new RB_gld()._ins(2,2,10,2)._left_mid());
        grid4Components._a((Component)this.toDisableAtMask._ar(new TLV_LIST_bt_Copy(tpHashMap)),   	new RB_gld()._ins(2,2,10,2)._left_mid());
        grid4Components._a((Component)this.toDisableAtMask._ar(new TLV_LIST_bt_multiDelete(tpHashMap)),new RB_gld()._ins(2,2,10,2)._left_mid());
         
        //benutzerdefinierte Reports anzeigen  
        grid4Components._a((Component)new REP__POPUP_Button(E2_MODULNAME_ENUM.MODUL.TEXT_LISTE_VORLAGE_LIST.get_callKey(),this.tpHashMap.getNavigationList()),new RB_gld()._ins(2,2,10,2)._left_mid());
        ...
        uploadBt.add_GlobalValidator(TLV_VALIDATORS.ATTACHMENT.getValidator());
    ...
    public TLV_LIST_Selector get_list_Selector() 
    {
        return TEXT_LISTE_VORLAGE_LIST_Selector;
    }

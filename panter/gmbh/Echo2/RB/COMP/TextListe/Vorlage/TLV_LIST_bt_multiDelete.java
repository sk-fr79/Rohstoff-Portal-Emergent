...
public class TLV_LIST_bt_multiDelete extends RB_BtV4_Delete {
	
    
    public TLV_LIST_bt_multiDelete(RB_TransportHashMap  p_tpHashMap)    {
        super();
        this._image(E2_ResourceIcon.get_RI("delete.png") , E2_ResourceIcon.get_RI("leer.png"));
        
        this._setTransportHashMap(p_tpHashMap);
        
        this.add_GlobalValidator(TLV_VALIDATORS.DELETE.getValidator());
	...
	public MyE2_String get_warnung_achtung_es_wird_ein_irgendwas_geloescht() {
		return S.ms("Soll diese Text-Liste-Vorlagen-Definition geloescht werden ?");	
    }
	@Override
	public String get_warnung_achtung_es_werden_n_irgendwas_geloescht_mit_platzhalter_fuer_zahl() {
		return "Sollen diese #WERT# Text-Liste-Vorlagen-Definitionen geloescht werden ?";
	}
	...
	public Vector<String> get_delete_sql_statements(String id_to_delete, MyE2_MessageVector mv) throws myException {
		...
		if (lid.isOK()) {
			Rec21 rec = new Rec21(_TAB.text_liste_vorlage)._fill_id(lid.get_oLong());
			v.add(rec.get_DELETE_STATEMENT());
			And and = new And(new vgl(TEXT_LISTE.tablename,EnumTableTranslator.TEXT_LISTE_VORLAGE.db_val())).and(new vgl(TEXT_LISTE.id_table,lid.get_oLong().toString()));
			v.add("DELETE FROM "+bibE2.cTO()+"."+_TAB.text_liste.fullTableName()+" WHERE "+and.s());

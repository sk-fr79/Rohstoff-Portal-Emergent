...
public class SearchDialog_vecResultButtons extends Vector<SearchDialog_ResultButton_IF[]> implements IF_Saveable{


	private  Integer    			actual_sort_col = null;
	private  SORTSTATUS  			actual_sort_status = null;
	private  SearchDialog_Base 	calling_searchDialog;
	
	
	public SearchDialog_vecResultButtons() {
		super();
	}
	...
	private class comparator implements Comparator<SearchDialog_ResultButton_IF[]> {
		private int 		i_col = 	0;
		...
		@Override
		public int compare(SearchDialog_ResultButton_IF[] o1, SearchDialog_ResultButton_IF[] o2) {
			try {
	...
	public void set_calling_searchDialog(SearchDialog_Base calling_searchDialog) {
		this.calling_searchDialog = calling_searchDialog;
	}
	
	
	
	public void set_1st_column_focusable() {
		for  (SearchDialog_ResultButton_IF[] arr : this) {
			if (arr!=null && arr.length>0) {
				for (SearchDialog_ResultButton_IF rb: arr) {
			...
			if (arr[0] instanceof MyE2IF__Component) {
				...
				((SearchDialog_ResultButton)arr[0]).setRolloverBorder(new RB_border()._col(Color.RED));
			}
		}
	}

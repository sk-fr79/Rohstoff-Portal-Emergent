...
public abstract class RB_DataobjectsCollector_V2 implements RB_DataobjectsCollector {
...
//	private LinkedHashMap<String, String>           hmCurrValuesNewRecords = new LinkedHashMap<String, String>();
	...
	public RB_DataobjectsCollector_V2() {
		super();
	}
	...
	public RB_Dataobject_V2 get_v2(RB_KM ob) throws myException {
		if (this.hmDataObjects.get(ob)instanceof RB_Dataobject_V2) {
			return (RB_Dataobject_V2)this.hmDataObjects.get(ob);
		}
	...
	public RB_Dataobject registerComponent(RB_K maskHash, RB_Dataobject dataOb) throws myException {
		...
		if (!(dataOb instanceof RB_Dataobject_V2)) {
			throw new myException(this,"In this only RB_dataobjects of type <RB_Dataobject_V2>  are allowed !");
		}
		
		if (maskHash instanceof RB_KM) {
			this.hmDataObjects.put((RB_KM)maskHash, (RB_Dataobject_V2)dataOb);
	 ...
	 */
	@Deprecated
	public RB_Dataobject registerComponent(RB_K maskHash, RB_Dataobject_V21 dataOb) throws myException {
	 ...
	 */
	public abstract void manipulate_filled_records_before_save(RB_DataobjectsCollector_V2 do_collector, MyE2_MessageVector mv) throws myException;
	 ...
	 */
	public abstract void execute_final_statements_in_open_transaction(RB_DataobjectsCollector_V2 do_collector, MyE2_MessageVector mv) throws myException;
	 ...
	 */
	public abstract RB_DataobjectsCollector_V2 get_copy() throws myException;
				...
				if (forceSave || something_has_changed) {
					...
					if (mv.get_bIsOK()) {
						...
						for (RB_KM  key: this.hmDataObjects.keySet()) {
							RB_Dataobject_V2 do_v2 = (RB_Dataobject_V2)this.hmDataObjects.get(key);
						...
						if (mv.get_bIsOK()) {
							...
							if (mv.get_bIsOK()) {
								//jetzt speichern:
								for (RB_KM key: this.hmDataObjects.keySet()) {
									RB_Dataobject_V2 d_ob = (RB_Dataobject_V2)this.hmDataObjects.get(key);
									
									d_ob.get_rec20()._SAVE(false, mv);

									
									//20170816: geaendert: Martin
									if (this.reloadRec20AfterSave && d_ob.get_rec20().is_ExistingRecord()) {
										//2018-08-09: schalter, um fuer die nutzung der Rec21-varianten vorzubereiten
										if (!this.useRec21 ) {
											d_ob.get_rec20()._rebuild();
										} else {
											d_ob.get_rec20()._rebuildRecord();
										}
									}
									if (this.cleanRec20NewValuesAfterSave) {
										d_ob.get_rec20().RESET_ALL_NEWVALUES();
									}
	...
	public void rb_RebuildAllRecords() throws myException	{
		//falls nix da ist, exception
		for (RB_Dataobject d_ob: this) {
			RB_Dataobject_V2 d_ob2 = (RB_Dataobject_V2)d_ob;
			if (d_ob2.get_rec20()!=null) {
				d_ob2.rb_RebuildRecord();
			} else {
				throw new myException(this,"Error: Rebuilding is not possible !");
			}
		}
	}
	...
	public String get_LastWrittenNewID(String cTablename) throws myException	{
		...
		for (RB_Dataobject  d_ob: this.hmDataObjects.values()) {
			...
			if (do_v2.get_rec20().is_newRecordSet()) {
				...
				if (do_v2.get_rec20().get_TABLENAME().equals(cTablename)) {
					v_id_new.add(do_v2.get_rec20().get_rec_after_save_new().get_key_value());
				}
			}
		}
		if (v_id_new.size()>0) {
	 ...
	 * @throws myException
	 */
	public RB_DataobjectsCollector_V2 _addSqlAtStart(String c_sql) throws myException {
		if (S.isFull(c_sql)) {
	 ...
	 * @throws myException
	 */
	public RB_DataobjectsCollector_V2 _addSqlAtEnd(String c_sql) throws myException {
		if (S.isFull(c_sql)) {
	 ...
	 * @throws myException
	 */
	public RB_DataobjectsCollector_V2 _addSqlAtStart(Vector<String> v_sql) throws myException {
		if (v_sql!=null) {
	 ...
	 * @throws myException
	 */
	public RB_DataobjectsCollector_V2 _addSqlAtEnd(Vector<String> v_sql) throws myException {
		if (v_sql!=null) {
	...
	public RB_DataobjectsCollector_V2 setReloadRec20AfterSave(boolean p_reloadRec20AfterSave) {
		this.reloadRec20AfterSave = p_reloadRec20AfterSave;
		return this;
	}

	public RB_DataobjectsCollector_V2 setCleanRec20NewValuesAfterSave(boolean p_cleanRec20NewValuesAfterSave) {
	...
	public RB_DataobjectsCollector_V2  _addMessageTranslator(RB_MessageTranslator mt) {
	...
	//2018-08-09: schalter, um fuer die nutzung der Rec21-varianten vorzubereiten
	public RB_DataobjectsCollector_V2 _setUseRec21(boolean useRec21) {

...
public abstract class RB_DataobjectsCollector_V22 implements RB_DataobjectsCollector {
	...
	//2020-10-16: aktionen vor dem commiten
	private VEK<IF_ExecuterReturnsMV<RB_DataobjectsCollector_V22>>    executersBeforeCommit = new VEK<IF_ExecuterReturnsMV<RB_DataobjectsCollector_V22>>();
	private VEK<IF_ExecuterReturnsMV<RB_DataobjectsCollector_V22>>    executersAfterRollback = new VEK<IF_ExecuterReturnsMV<RB_DataobjectsCollector_V22>>();
	
	
	public RB_DataobjectsCollector_V22() {
		super();
	}
	...
	public RB_Dataobject_V22 getRec22(RB_KM ob) throws myException {
		if (this.hmDataObjects.get(ob)instanceof RB_Dataobject_V22) {
			return (RB_Dataobject_V22)this.hmDataObjects.get(ob);
		}
		throw new myException(this,"No dataobject of type RB_Dataobject_V22!");
	}
	...
	public RB_Dataobject registerComponent(RB_K maskHash, RB_Dataobject dataOb) throws myException {
		...
		if (!(dataOb instanceof RB_Dataobject_V22)) {
			throw new myException(this,"In this only RB_dataobjects of type <RB_Dataobject_V22>  are allowed !");
		}
	 ...
	 */
	public abstract void manipulate_filled_records_before_save(RB_DataobjectsCollector_V22 do_collector, MyE2_MessageVector mv) throws myException;
	 ...
	 */
	public abstract void execute_final_statements_in_open_transaction(RB_DataobjectsCollector_V22 do_collector, MyE2_MessageVector mv) throws myException;
	 ...
	 * 
	 */
	public void executeFinalStatementsAfterCommitTransaction(RB_DataobjectsCollector_V22 do_collector, MyE2_MessageVector mv) {
	}
	
	
	
	
	private void checkIfContentIsCompleteRec22() throws myException {
		//jetzt sicherstellen, dass alle dataobject der collection vom typ RB_Dataobject_V22 sind
		for (RB_Dataobject  d_ob: this.hmDataObjects.values()) {
			if (!(d_ob instanceof RB_Dataobject_V22)) {
				throw new RuntimeException("System-Error: 3cc61441-a393-4df8-93c5-69343458c089: Only Dataobjects Type RB_Dataobject_V22 are allowed !");
			}
		}
	}
	 ...
	 */
	public abstract RB_DataobjectsCollector_V22 get_copy() throws myException;
	
	
	@Override
	public MyE2_MessageVector rb_Dataobjects_to_Database(boolean forceSave) throws myException {
	
		this.checkIfContentIsCompleteRec22();
		...
		if (mv.isOK()) {
			...
			for (RB_Dataobject  d_ob: this.hmDataObjects.values()) {
				RB_Dataobject_V22 do_v2 = (RB_Dataobject_V22)d_ob;
			...
			try {
				...
				if (forceSave || something_has_changed) {
					...
					if (mv.isOK()) {
						...
						for (RB_KM  key: this.hmDataObjects.keySet()) {
							RB_Dataobject_V22 do_v2 = (RB_Dataobject_V22)this.hmDataObjects.get(key);
						...
						if (mv.isOK()) {
							...
							if (mv.isOK()) {
								//jetzt speichern:
								for (RB_KM key: this.hmDataObjects.keySet()) {
									RB_Dataobject_V22 d_ob = (RB_Dataobject_V22)this.hmDataObjects.get(key);
						...
						try {
							...
							if (mv.isOK()) {
								for (IF_ExecuterReturnsMV<RB_DataobjectsCollector_V22> ex: executersBeforeCommit) {
									mv._add(ex.execute(this));
									if (mv.hasAlarms()) {   //beim ersten fehler abbruch
										break;
									}
								}
							}
							if (mv.isOK()) {
								...
								for (IF_ExecuterReturnsMV<RB_DataobjectsCollector_V22> ex: executersAfterRollback) {
									mv._add(ex.execute(this));    //werden alle durchgefuehrt
								}
				...
				if (e instanceof myException) {
					...
					throw new myException(this,"Systemfehler 9c7335b-c11c-4463-b70d-83ff4ee96e29 ! Bitte beim Admin melden !!");
				}
			}
	...
	public void rb_Rebuild_ALL_RECORD(boolean bOnlyWhenExists) throws myException	{
		...
		throw new RuntimeException("System Error: 2c345605-e821-4fc5-92121c-4c4687c57207 Method: rb_Rebuild_ALL_RECORD() is forbidden "); 
	}
	...
	public void rb_RebuildAllRecords() throws myException	{
		...
		for (RB_Dataobject d_ob: this) {
			RB_Dataobject_V22 d_ob2 = (RB_Dataobject_V22)d_ob;
	...
	public String get_LastWrittenNewID(String cTablename) throws myException	{
		...
		for (RB_Dataobject  d_ob: this.hmDataObjects.values()) {
			RB_Dataobject_V22 do_v2 = (RB_Dataobject_V22)d_ob;
			if (do_v2.getIdOfSaveNew()!=null && do_v2.get_TABLENAME().equals(cTablename)) {
				v_id_new.add(do_v2.getIdOfSaveNew().toString());
			}
//			if (do_v2.is_newRecordSet()) {
...
//			}
	 ...
	 * @throws myException
	 */
	public RB_DataobjectsCollector_V22 _addSqlAtStart(String c_sql) throws myException {
		if (S.isFull(c_sql)) {
	 ...
	 * @throws myException
	 */
	public RB_DataobjectsCollector_V22 _addSqlAtEnd(String c_sql) throws myException {
		if (S.isFull(c_sql)) {
	 ...
	 * @throws myException
	 */
	public RB_DataobjectsCollector_V22 _addSqlAtStart(Vector<String> v_sql) throws myException {
		if (v_sql!=null) {
	 ...
	 * @throws myException
	 */
	public RB_DataobjectsCollector_V22 _addSqlAtEnd(Vector<String> v_sql) throws myException {
		if (v_sql!=null) {
	...
	public RB_DataobjectsCollector_V22  _addMessageTranslator(RB_MessageTranslator mt) {
	...
	public RB_Dataobject_V22 get_v22(RB_KM ob) throws myException {
		if (this.hmDataObjects.get(ob)instanceof RB_Dataobject_V22) {
			return (RB_Dataobject_V22)this.hmDataObjects.get(ob);
		}
		throw new myException(this,"No dataobject of type RB_Dataobject_V22!");
	...
	public RB_DataobjectsCollector_V22 _registerExecuterBeforeCommit(IF_ExecuterReturnsMV<RB_DataobjectsCollector_V22> executerBeforeCommit) {
		this.executersBeforeCommit._a(executerBeforeCommit);
		return this;
	}
	
	public RB_DataobjectsCollector_V22 _registerAfterRollback(IF_ExecuterReturnsMV<RB_DataobjectsCollector_V22> executerBeforeCommit) {
		this.executersAfterRollback._a(executerBeforeCommit);
		return this;
	}

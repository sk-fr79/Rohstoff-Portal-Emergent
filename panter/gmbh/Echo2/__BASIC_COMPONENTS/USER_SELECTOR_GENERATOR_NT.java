...
public class USER_SELECTOR_GENERATOR_NT {
	 ...
	 */
	public USER_SELECTOR_GENERATOR_NT(boolean bFormatedIDs, ENUM_USER_TYP... typ) throws myException  {
		super();
		...
		SEL selUser = new SEL("NVL(NAME1,NVL(NAME,'-'))||', '||NVL(VORNAME,'')||' ('|| NVL(KUERZEL,'-')||')'","ID_USER")
		                      .FROM(_TAB.user).WHERE(whereblock).ORDERUP("1");
		
		SEL selUserRest = new SEL("NVL(NAME1,NVL(NAME,'-'))||', '||NVL(VORNAME,'')||' ('|| NVL(KUERZEL,'-')||')'","ID_USER")
        						.FROM(_TAB.user).WHERE(notWhere).ORDERUP("1");
		

		if (bFormatedIDs) {
			this.arrSelected = bibDB.EinzelAbfrageInArrayFormatiert(selUser.s(),"");
			this.arrRest = bibDB.EinzelAbfrageInArrayFormatiert(selUserRest.s(),"");
		} else {

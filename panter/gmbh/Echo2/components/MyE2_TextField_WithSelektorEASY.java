 ...
 */
public class MyE2_TextField_WithSelektorEASY extends	MyE2_TextField_WithSelektor
{
	private String cTextBlockGroup = null;
	private String cEmptyInfo = null;

	public MyE2_TextField_WithSelektorEASY(String cNameTextblockGroup) throws myException
	{
		super(null, null);

		this._makeBasics(cNameTextblockGroup);
	}

	public MyE2_TextField_WithSelektorEASY(String cNameTextblockGroup,int iWidth) throws myException
	{
		super(null, null,iWidth);

		this._makeBasics(cNameTextblockGroup);
	}

	
	private void _makeBasics(String cNameTextblockGroup) throws myException
	{
...
//				" JT_TEXTBLOCK.ID_TEXTBLOCK_KAT=JT_TEXTBLOCK_KAT.ID_TEXTBLOCK_KAT AND UPPER(JT_TEXTBLOCK_KAT.BEZEICHNUNG)="+bibALL.MakeSql(cNameTextblockGroup).toUpperCase()),cEmptyInfo,oLabelInfo);
	}
	...
	{
		this.set_Varianten(bibDB.EinzelAbfrageInArray("SELECT TEXTTITEL,TEXT FROM "+bibE2.cTO()+".JT_TEXTBLOCK,"+bibE2.cTO()+".JT_TEXTBLOCK_KAT WHERE " +
				" JT_TEXTBLOCK.ID_TEXTBLOCK_KAT=JT_TEXTBLOCK_KAT.ID_TEXTBLOCK_KAT AND UPPER(JT_TEXTBLOCK_KAT.BEZEICHNUNG)="+bibALL.MakeSql(this.cTextBlockGroup).toUpperCase()),this.cEmptyInfo,
			...
			public void executeAgentCode(ExecINFO oExecInfo) throws myException 
			{
				if (S.isFull(oTextArea.getText()) && S.isFull(oTextTitel.getText()))
				{
					...
					if (oMV.get_bIsOK())
					{
						...
						MyE2_TextField_WithSelektorEASY.this.Refresh();
					}
					else
					{
						bibMSG.add_MESSAGE(oMV);

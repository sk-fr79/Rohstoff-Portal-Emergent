 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class MyE2_PopUpMenue extends PopUp  implements MyE2IF__Component, E2_IF_Copy
{
	private MyE2EXT__Component 		oEXT = new MyE2EXT__Component(this);
	
    private Vector<MyE2_Button>  	vMenueButtons = new Vector<MyE2_Button>();
    ...
    private MyE2_PopUpMenue.ownAgent oAgentCollapse = null;
     ...
     */
    public MyE2_PopUpMenue(E2_ResourceIcon oiconAktiv, E2_ResourceIcon oiconinaktiv, boolean useContainerEx)
    {
        ...
        this.oColAussen.setStyle(MyE2_PopUpMenue.DEFAULT_STYLE_OUTER_COLUMN());
        this.oColInnen.setStyle(MyE2_PopUpMenue.DEFAULT_STYLE_INNER_COLUMN());
        
        this.setStyle(MyE2_PopUpMenue.DEFAULT_STYLE_POPUP());

        this.oAgentCollapse =	new MyE2_PopUpMenue.ownAgent(this);
     ...
     * @param iTopOffest
     */
    public MyE2_PopUpMenue(		E2_ResourceIcon oiconAktiv, 
    ...
    public void addButton(MyE2_Button oNewButton, boolean bSetDefaultStyle)
    {
        ...
        if (bSetDefaultStyle)
        	oNewButton.setStyle(MyE2_PopUpMenue.DEFAULT_STYLE_BUTTON());
        this.iButtonCounter++;
    }
    ...
    {
    	MyE2_Button oButtonNew = new MyE2_Button(cButtonText);
    	oButtonNew.setStyle(MyE2_PopUpMenue.DEFAULT_STYLE_BUTTON());
	...
	{
		((MyE2_Button)this.vMenueButtons.get(iButtonNr)).setEnabled(bEnabled);
	}
    ...
    public MyE2_Button get_Button(int iPos)
    {
    	MyE2_Button oRueck = null;

        if (iPos < this.vMenueButtons.size())
            oRueck = (MyE2_Button) this.vMenueButtons.get(iPos);
    ...
    public int get_ID_Click(ActionEvent e)
    {
        ...
        {
            if (e.getSource() == (MyE2_Button) this.vMenueButtons.get(i))
            {
                iRueck = i;
                break;
            }
     ...
     * wird -1 zurückgegeben
     */
    public MyE2_Button get_buttonClick(ActionEvent e)
    {
    	MyE2_Button buttonRueck = null;

        for (int i = 0; i < this.vMenueButtons.size(); i++)
        {
            if (e.getSource() == (MyE2_Button) this.vMenueButtons.get(i))
            {
                buttonRueck = (MyE2_Button) this.vMenueButtons.get(i);
    ...
    {
    	MyE2_Button buttonHelp = null;
        if (myListener != null)
        {
	        for (int i = 0; i < this.vMenueButtons.size(); i++)
	        {
	            buttonHelp = (MyE2_Button) this.vMenueButtons.get(i);
	...
	public String find_EventKennung(ActionEvent e)
	{
	    ...
	    if (e.getSource() instanceof MyE2_Button)
	    {
		    for (int i=0;i<this.vMenueButtons.size();i++)
		    {
		        if (e.getSource()==(MyE2_Button)this.vMenueButtons.get(i))
		        {
		            cRueck = ((MyE2_Button)this.vMenueButtons.get(i)).EXT().get_C_EVENT_KENNUNG();
		            break;
		        }
		    }
	    }
	 ...
	 */
	public MyE2_Button find_Button(String cEVENT_KENNUNG)
	{
	    for (int i=0;i<this.vMenueButtons.size();i++)
	    {
	        if (((MyE2_Button)this.vMenueButtons.get(i)).EXT().get_C_EVENT_KENNUNG().equals(cEVENT_KENNUNG))
	        {
	            return (MyE2_Button)this.vMenueButtons.get(i);
	        }
	    }
	    return null;
	}
...
//	
	...
	public Vector<MyE2_Button> get_vMenueButtons()							{		return vMenueButtons;	}
	...
	{
		MyE2_PopUpMenue oPopRueck = new MyE2_PopUpMenue(this.oIconAktiv,this.oIconInactiv, this.bUseContainerEx);
		for (int i=0;i<this.vMenueButtons.size();i++)
		{
			oPopRueck.addButton((MyE2_Button)((MyE2_Button)this.vMenueButtons.get(i)).get_Copy(null), false);
		}
	...
	public static class ownAgent extends XX_ActionAgent
	{
		...
		public ownAgent(MyE2_PopUpMenue popup)
		{
			super();
			oPopUp = popup;
		}

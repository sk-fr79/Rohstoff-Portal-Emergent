...
import panter.gmbh.Echo2.components.MyE2_TextField_WITH_Action;
...
public class MyE2_DB_TextField_WITH_Action extends MyE2_TextField_WITH_Action implements MyE2IF__DB_Component,MyE2IF__Component, E2_IF_Copy
{
	...
	public MyE2_DB_TextField_WITH_Action(SQLField osqlField) throws myException
	{
		super();
		if (osqlField == null)
	...
	{
		MyE2_DB_TextField_WITH_Action oRueck = null;
		
		try
		{
			oRueck =  new MyE2_DB_TextField_WITH_Action(this.EXT_DB().get_oSQLField());
		}
		...
		for (int i=0;i<this.get_vGlobalValidators().size();i++)
			oRueck.add_GlobalValidator((XX_ActionValidator)this.get_vGlobalValidators().get(i));
				
		
		for (int i=0;i<this.get_vIDValidators().size();i++)
			oRueck.add_IDValidator((XX_ActionValidator)this.get_vIDValidators().get(i));

...
public class MyE2_DBC_CrossConnection extends MyE2_Grid_With_CheckBoxes implements MyE2IF__DB_Component,MyE2IF__Component, E2_IF_Copy
{
    ...
    private int CROSSTYP = MyE2_DBC_CrossConnection.CROSSTYP_ALLOW_ALL; // standard: alles erlaubt
    ...
    private Vector<String> vZusatzWhereStatements = null; // zusatz-where-statements, können von oben gesetzt werden
     ...
     * @throws myException
     */
    public MyE2_DBC_CrossConnection(	SQLFieldForPrimaryKey 	osqlField, 
    									...
    									Vector<String> 			vzusatzWhereStatements, 
    									...
    									int 					iCrossTyp) throws myException
    {
        ...
        this.vZusatzWhereStatements = 				vzusatzWhereStatements;
...
//        this.setSize(iAnordnungSpaltenZahl);
        ...
        if (this.vZusatzWhereStatements != null)
            cSQL += " WHERE " + bibALL.Concatenate(this.vZusatzWhereStatements," AND ", null);
         ...
         */
        this.EXT().add_FieldSetters_AND_Validator__BeforeReadInputMAP(new MyE2_DBC_CrossConnection.ownFieldValidator_AfterInput(this));
    ...
    public Object get_Copy(Object objHelp) throws myExceptionCopy
    {
    	try 
    	{
			MyE2_DBC_CrossConnection oRueck = new MyE2_DBC_CrossConnection(
					(SQLFieldForPrimaryKey)this.EXT_DB().get_oSQLField(),
					this.cMittlerTabelle,
					this.cNebenTabelle,
					this.cIndexMittlerTabelle,
					this.cIndexMittlerTabelleZuHaupt ,
					this.cIndexMittlerTabelleZuNeben,
					this.cIndexNebenTabelle ,
					this.cAbfrageAusdruckFuerBeschreibung,
					null,
					null,
					this.vZusatzWhereStatements,
					this.cORDER_STATEMENT,
					this.getSize(),
					this.oFont, this.cErrorMessageString, this.CROSSTYP);
			...
			Vector<MyE2IF__Component> vInFrontNeu = new Vector<MyE2IF__Component>();
			if (this.get_vAddOnComponentsInFront() != null)
			{
				for (int i=0;i<this.get_vAddOnComponentsInFront().size();i++)
				{
					vInFrontNeu.add((MyE2IF__Component)((E2_IF_Copy)this.get_vAddOnComponentsInFront().get(i)).get_Copy(null));
				}
				oRueck.set_vAddOnComponentsInFront(vInFrontNeu);
			}

			Vector<MyE2IF__Component> vInBehindNeu = new Vector<MyE2IF__Component>();
			if (this.get_vAddOnComponentsBehind() != null)
			{
				for (int i=0;i<this.get_vAddOnComponentsBehind().size();i++)
				{
					vInBehindNeu.add((MyE2IF__Component)((E2_IF_Copy)this.get_vAddOnComponentsBehind().get(i)).get_Copy(null));
				}
				oRueck.set_vAddOnComponentsBehind(vInFrontNeu);
			}
			
			oRueck.get_vActionAgent4Checkboxes().addAll(this.vActionAgent4Checkboxes);
			oRueck.set_oAgentToSelektAnzeige(this.get_oAgentToSelektAnzeige());
			oRueck.set_oFormatCheckBoxes(this.get_oFormatCheckBoxes());
    ...
    private boolean fuelle_KreuztabellenGrid(String cIndexWert) throws myException
    {
        ...
        if (this.vZusatzWhereStatements != null)
        {
            for (int i = 0; i < this.vZusatzWhereStatements.size(); i++)
            {
                cSQL += (" AND " + (String) this.vZusatzWhereStatements.get(i));
            }
        }
	...
	private Vector<String> build_InsertVector(String cHauptIDUnformated)
	{
	    Vector<String> vRueck = new Vector<String>();
	    
	    // jetzt das grid aufbauen, das als komponente dient
	    String cSEQUENCE = "SEQ_" + this.cMittlerTabelle.substring(3);

	    vRueck.add("DELETE FROM " + cTO + "." + this.cMittlerTabelle + " WHERE " + this.cIndexMittlerTabelleZuHaupt + "=" + cHauptIDUnformated);
	    ...
	    for (int i = 0; i < this.vCheckBoxKreuzTab.size(); i++)
	    {
	        if (((MyE2_CheckBox) this.vCheckBoxKreuzTab.get(i)).isSelected())
	        {
	            vRueck.add("INSERT INTO " + cTO + "." + this.cMittlerTabelle + "(" + 
	            			this.cIndexMittlerTabelle + "," + 
	            			this.cIndexMittlerTabelleZuHaupt + "," + 
	            			this.cIndexMittlerTabelleZuNeben + ") VALUES (" + 
	            			cSEQUENCE + ".NEXTVAL, " + 
	            			cHauptIDUnformated + "," + 
	            			this.vCheckBoxKreuzTab.get(i).EXT().get_C_MERKMAL() +")");
	         }
	    }
	...
	public static class ownFieldValidator_AfterInput extends XX_FieldSetter_AND_Validator
	{
		...
		private MyE2_DBC_CrossConnection oFieldCrossConnection = null;
		/*
		 *    public static int CROSSTYP_ALLOW_ALL = 100;
			    public static int CROSSTYP_MAX_ONE = 101; // nur 1 knopf erlaubt
			    public static int CROSSTYP_MUST_ONE = 102; // mindestens 1 knopf muss eingegeben werden
		...
		public ownFieldValidator_AfterInput(MyE2_DBC_CrossConnection connection)
		{
			super();
		...
		public MyE2_MessageVector isValid( String cSTATUS_MAP, MyE2EXT__Component EXT_own) throws myException
		{
	        ...
	        if (iCrossTYP == MyE2_DBC_CrossConnection.CROSSTYP_ALLOW_ALL)
	        {
	            // egal
	        }
	        else if (iCrossTYP == MyE2_DBC_CrossConnection.CROSSTYP_EXACT_ONE)
	        {
	            if (iAuswahl != 1)
	        ...
	        else if (iCrossTYP == MyE2_DBC_CrossConnection.CROSSTYP_MUST_ONE)
	        {
	            if (iAuswahl == 0)
	            	if (this.oFieldCrossConnection.get_ErrorMessageForValidationAfterInput()!=null)
	            		vRueck.add_MESSAGE(this.oFieldCrossConnection.get_ErrorMessageForValidationAfterInput(), false);
	        ...
	        else if (iCrossTYP == MyE2_DBC_CrossConnection.CROSSTYP_MAX_ONE)
	        {
	            if (iAuswahl > 1)
	            	if (this.oFieldCrossConnection.get_ErrorMessageForValidationAfterInput()!=null)
	            		vRueck.add_MESSAGE(this.oFieldCrossConnection.get_ErrorMessageForValidationAfterInput(), false);
   ...
   protected Vector<XX_ActionAgent> get_vActionAgent4Checkboxes()
   {

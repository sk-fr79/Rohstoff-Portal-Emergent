 ...
 */
public class MyE2_DB_TextArea_WithSelektorEASY extends	MyE2_DB_TextArea_WithSelektor   implements MyE2IF__Component,MyE2IF__DB_Component, E2_IF_Copy
{

	private String cTextBlockGroup = null;
	private String cEmptyInfo = null;
	
	public MyE2_DB_TextArea_WithSelektorEASY(SQLField osqlField, String cNameTextblockGroup) throws myException
	{
		super(osqlField, null, null, null);
...
//		MyE2_Label oLabelInfo = new MyE2_Label(cInfo,MyE2_Label.STYLE_SMALL_ITALIC());
	...
	/**
	 * Martin
	 * aenderung: liste wird sortiert nach titel 
	 * @date 16.11.2020
	 *
	 * @throws myException
	 */
	private void Refresh() throws myException
	{
		this.set_Varianten(bibDB.EinzelAbfrageInArray("SELECT TEXTTITEL,TEXT FROM "+bibE2.cTO()+".JT_TEXTBLOCK,"+bibE2.cTO()+".JT_TEXTBLOCK_KAT WHERE " +
				...
				+bibALL.MakeSql(this.cTextBlockGroup).toUpperCase()+" ORDER BY UPPER(JT_TEXTBLOCK.TEXTTITEL)"),this.cEmptyInfo,
				this.get_Component_4_add_group_or_text(this.cTextBlockGroup));
	}
			...
			public void executeAgentCode(ExecINFO oExecInfo) throws myException 
			{
				if (S.isFull(oTextArea.getText()) && S.isFull(oTextTitel.getText()))
				{
					...
					if (oMV.get_bIsOK())
					{
						...
						MyE2_DB_TextArea_WithSelektorEASY.this.Refresh();
					}
					else
					{
						bibMSG.add_MESSAGE(oMV);
	...
	{
		MyE2_DB_TextArea_WithSelektorEASY oRueck = null;
		
		try
		{
			oRueck =  new MyE2_DB_TextArea_WithSelektorEASY(this.EXT_DB().get_oSQLField(),this.cTextBlockGroup);
		}
		...
		oRueck.get_oTextArea().set_iMaxInputSize(this.get_oTextArea().get_iMaxInputSize());
//		
		oRueck.get_oTextArea().set_iWidthPixel(this.get_oTextArea().get_iWidthPixel());
		oRueck.get_oTextArea().set_WitdhExtent(this.get_oTextArea().get_WidthExtent());
		
		oRueck.get_oTextArea().setText(this.get_oTextArea().getText());
		oRueck.get_oTextArea().setFont(this.get_oTextArea().getFont());

		oRueck.get_oTextArea().setWidth(this.get_oTextArea().getWidth());
		oRueck.get_oTextArea().setHeight(this.get_oTextArea().getHeight());
		oRueck.get_oTextArea().setAlignment(this.get_oTextArea().getAlignment());
		...
		oRueck.get_oPopUp().set_oIconAktiv(this.get_oPopUp().get_oIconAktiv());
		oRueck.get_oPopUp().set_oIconInactiv(this.get_oPopUp().get_oIconInactiv());
		...
		if (this.get_bBAddTextBehind() != null)
			oRueck.set_bAddTextBehind(new Boolean(this.get_bBAddTextBehind().booleanValue()));

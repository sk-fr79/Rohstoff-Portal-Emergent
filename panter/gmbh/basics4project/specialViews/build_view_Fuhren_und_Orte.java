...
public class build_view_Fuhren_und_Orte extends XXX_ViewBuilder
{
	...
	private String[][] cDefArray = 
			...
			{"STORNO_GRUND",				"",							""},
			{"STORNO_KUERZEL",				"",							""},
			{"ANTEIL_PLANMENGE_LIEF",		"ANTEIL_PLANMENGE",			""},
			{"ANTEIL_LADEMENGE_LIEF",		"ANTEIL_LADEMENGE",			""},
			{"ANTEIL_ABLADEMENGE_LIEF",		"ANTEIL_ABLADEMENGE",		""},
			{"ANTEIL_PLANMENGE_ABN",		"",							"ANTEIL_PLANMENGE"},
			{"ANTEIL_LADEMENGE_ABN",		"",							"ANTEIL_LADEMENGE"},
			{"ANTEIL_ABLADEMENGE_ABN",		"",							"ANTEIL_ABLADEMENGE"},
			{"LIEFERINFO_TPA",				"",							""},
			...
			{"ABZUG_LADEMENGE_LIEF",		"ABZUG_MENGE",				""},
			{"ABZUG_ABLADEMENGE_ABN",		"",							"ABZUG_MENGE"},
			...
			{"TRANSIT_EK",					"TRANSIT",					""},                      
			{"TRANSIT_VK",					"",							"TRANSIT"},   
			//2014-06-27: lieferbedingungen
			{_DB.VPOS_TPA_FUHRE$LIEFERBED_ALTERNATIV_EK,	_DB.VPOS_TPA_FUHRE_ORT$LIEFERBED_ALTERNATIV,	""},
			{_DB.VPOS_TPA_FUHRE$LIEFERBED_ALTERNATIV_VK,	"", 											_DB.VPOS_TPA_FUHRE_ORT$LIEFERBED_ALTERNATIV},
			//2016-04-22: bemerkungen_kunde und verpackungsart
			{VPOS_TPA_FUHRE.bemerkung_fuer_kunde.fn(),	"", 	""},
			{VPOS_TPA_FUHRE.id_verpackungsart.fn(),	"", 		""},
			//2020-08-20: verarbeitung
			{VPOS_TPA_FUHRE.id_verarbeitung.fn(),	"", 		""},
			};
	...
	public boolean build_View_forAll_Mandants() throws myException
	{
		...
		String c_SQL =  	 " CREATE OR REPLACE VIEW V#MANDANT#_FUHREN AS "+	
									"  (SELECT "+ 
		...
		for (int i=0;i<oRecList.get_vKeyValues().size();i++)
		{
			...
			{
				MyE2_String cInfo = new MyE2_String("Fuhren-View V"+oRecList.get(i).get_ID_MANDANT_cUF()+"_FUHREN fuer Mandant: <",true,oRecList.get(i).get_NAME1_cF_NN(""),false, "> erfolgreich erstellt !",true);
				bibMSG.add_MESSAGE(new MyE2_Info_Message(cInfo));
			}
			else
			{
				MyE2_String cInfo = new MyE2_String("Fuhren-View V"+oRecList.get(i).get_ID_MANDANT_cUF()+"_FUHREN fuer Mandant: <",true,oRecList.get(i).get_NAME1_cF_NN(""),false, "> KONNTE NICHT ERSTELLT WERDEN !",true);
